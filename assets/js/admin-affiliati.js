...
function openModal(opts){
    ...
    async function ensureMfaAal2IfNeeded(client, opts){
      ...
      try{
        ...
        // Per questa app supportiamo MFA solo via TOTP.
        // Se NON esiste un fattore TOTP verificato, consideriamo MFA NON attiva (niente prompt).
        // Questo evita falsi positivi (es. fattori incompleti / rimossi) e non blocca l'utente.
        var factor = await getVerifiedTotpFactor(client);
        if (!factor || !factor.id) return true;

        // Se non forziamo, chiediamo MFA solo quando Supabase indica che la sessione è AAL1.
        // Se forziamo (richiesta ad ogni accesso), chiediamo MFA anche se già AAL2.
        if (!force){
          ...
          if (!(cur === 'aal1' && next === 'aal2')) return true;
        }
