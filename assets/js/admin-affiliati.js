  ...
  function init(){
    ...
    async function loadAggregates(){
      ...
      var resp = await client
        ...
        .select('month,sales_count,net_commissions,affiliate_earnings')
      ...
      monthly.forEach(function(m){
        ...
        totalNet += Number(m.total_net_commissions || 0);
        totalAff += Number(m.total_affiliate_earnings || 0);
      });

      safeSetText(elTotalSales, String(totalSales));
      safeSetText(elTotalNet, formatUsd(totalNet));
      safeSetText(elTotalAff, formatUsd(totalAff));
      safeSetText(elTotalTrav, formatUsd(totalNet - totalAff));
    ...
    async function importStay22CSV(file){
      ...
      var rows = parsed.rows.map(function(r){
        ...
        return r.affiliate_slug && r.partner_reference;
      });
