...
Totale: ${total} · Inviate: ${sent} · Errori: ${failed}${extra}`, failed ? 'warn' : 'success');
  ...
  (function setupNewsletterPreview(){
    ...
    function buildPreviewEmailHtml(language, subject, bodyHtml){
      const unsubUrl = '#';
      const safeLang = escapeHtml(language || '');
      const safeUnsub = escapeAttr(unsubUrl);
      const safeSubject = escapeHtml(subject || '');

      const contentHtml = looksLikeHtml(bodyHtml) ? String(bodyHtml) : wrapPlainTextToHtml(bodyHtml);
...
<html>
  <head>
    ...
    <title>${safeSubject || 'Travirae newsletter'}</title>
  </head>
  <body style="margin:0;padding:0;background:#f5f7fb;">
    <div style="padding:24px 12px;">
      <div style="max-width:600px;margin:0 auto;font-family:ui-sans-serif,system-ui,-apple-system,Segoe UI,Roboto,Ubuntu,Cantarell,Noto Sans,sans-serif;background:#ffffff;border:1px solid #eaeef4;border-radius:16px;overflow:hidden;">
        <div style="padding:16px 20px;background:linear-gradient(90deg,#1d4ed8,#3b82f6);color:#fff;">
          <div style="display:flex;align-items:center;gap:10px;">
            <div style="width:34px;height:34px;border-radius:10px;background:rgba(255,255,255,0.18);display:flex;align-items:center;justify-content:center;font-size:18px;">✈️</div>
            <div>
              <div style="font-weight:700;font-size:16px;">Travirae</div>
              <div style="opacity:.9;font-size:12px;">Newsletter</div>
            </div>
          </div>
        </div>
        <div style="padding:22px 20px 14px 20px;">
          <h1 style="margin:0 0 12px 0;font-size:20px;line-height:1.25;color:#111;">${safeSubject || 'Aggiornamenti Travirae'}</h1>
          <div style="font-size:14px;line-height:1.65;color:#111;">${contentHtml}</div>
        </div>
        <div style="padding:14px 20px 20px 20px;border-top:1px solid #eaeef4;color:#666;font-size:12px;line-height:1.5;">
          <div style="margin-bottom:8px;">Lingua: <strong style="color:#111;">${safeLang}</strong></div>
          <div>Se non vuoi più ricevere queste email, puoi <a href="${safeUnsub}" style="color:#1d4ed8;">disiscriverti</a>.</div>
        </div>
      </div>
      <div style="max-width:600px;margin:10px auto 0 auto;color:#8a94a6;font-size:11px;text-align:center;">Anteprima (non è un invio reale)</div>
    </div>
  </body>
</html>`;
    }
