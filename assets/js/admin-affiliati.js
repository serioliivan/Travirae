...
function openModal(opts){
    ...
    async function loadNewsletterStats(){
      if (!newsletterCountEl && !newsletterLangsEl) return;
      if (!state.user || !isAdminEmail(state.user.email)) {
        safeSetText(newsletterCountEl, '—');
        safeSetText(newsletterLangsEl, '—');
        return;
      }
      ...
      try {
        ...
        safeSetText(newsletterCountEl, (typeof count === 'number') ? String(count) : '0');

        // Lingue (best-effort)
        if (newsletterLangsEl) {
          ...
          const counts = {};
          (rows || []).forEach(r => {
            const code = (r && r.language) ? String(r.language) : 'ITA';
            counts[code] = (counts[code] || 0) + 1;
          });
          const parts = Object.keys(counts).sort().map(k => `${k} ${counts[k]}`);
          newsletterLangsEl.textContent = parts.length ? parts.join(' · ') : '—';
        }
      } catch (e) {
        console.warn('Newsletter stats error:', e);
        safeSetText(newsletterCountEl, '—');
        safeSetText(newsletterLangsEl, '—');

        const msg = (e && e.message) ? String(e.message) : '';
        if (msg.toLowerCase().includes('relation') && msg.toLowerCase().includes('newsletter_subscribers')) {
          setNewsletterStatus('Nota: crea la tabella newsletter_subscribers su Supabase per abilitare la newsletter.', 'info');
        }
      }
    }
  ...
  form.addEventListener('submit', async (e)=>{
    ...
    // Richiedo almeno ITA completo (fallback)
    const ita = payload.contentByLang.ITA || { subject:'', body:'' };
    if(!ita.subject || !ita.body){
      setNewsletterStatus('Compila almeno ITA (oggetto e testo). Le altre lingue sono opzionali.', 'error');
      return;
    }

    const totalFilled = NL_LANGS.filter(code=>{
      const c = payload.contentByLang[code];
...
`+
    ...
    try{
      ...
      const res = await fetch(EDGE_NEWSLETTER_SEND, {
        method: 'POST',
        headers: {
