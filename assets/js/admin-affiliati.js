  ...
  function init(){
    ...
    async function getVerifiedTotpFactor(client){
      try{
        ...
        var factors = (res && res.data) ? res.data : null;
        var totpList = (factors && factors.totp) ? factors.totp : [];
        for (var i=0;i<totpList.length;i++){
          var f = totpList[i];
          if (!f) continue;
          if (String(f.status || '').toLowerCase() === 'verified') return f;
        }
        return totpList.length ? totpList[0] : null;
      }catch(_e){
        return null;
      }
    }
    ...
    async function open2faSettings(){
      ...
      if (!client){
        setMessage('Supabase non inizializzato.', 'error');
        return;
      }
      var session = await client.auth.getSession();
      if (!session || session.error || !session.data || !session.data.session){
        setMessage('Devi prima accedere come admin.', 'error');
        return;
      }
      ...
      if (factor && factor.id){
        openModal({
          title: 'Sicurezza (2FA)',
          bodyHtml: '<p class="tva-modal__hint"><b>2FA attiva.</b> Per disattivarla, conferma con un codice dalla tua app.</p>',
          actions: [
            { label: 'Chiudi', className: 'btn secondary', onClick: closeModal },
            { label: 'Disattiva 2FA', className: 'btn', onClick: async function(){
              var ok = await ensureMfaAal2IfNeeded(client);
              if (!ok) return;
              var res = await client.auth.mfa.unenroll({ factorId: factor.id });
              if (res && res.error){
                setMessage(res.error.message || 'Errore disattivando 2FA.', 'error');
                return;
              }
              closeModal();
              setMessage('2FA disattivata.', 'success');
            } }
          ]
        });
        return;
      ...
      openModal({
        title: 'Sicurezza (2FA)',
        bodyHtml: '<p class="tva-modal__hint">Abilita 2FA (TOTP) per proteggere l\'account admin.</p>',
        actions: [
          { label: 'Chiudi', className: 'btn secondary', onClick: closeModal },
          { label: 'Abilita 2FA', className: 'btn', onClick: async function(){
            try{
              var enroll = await client.auth.mfa.enroll({ factorType: 'totp' });
              if (enroll && enroll.error){
                setMessage(enroll.error.message || 'Errore durante enroll.', 'error');
                return;
              }

              var id = enroll && enroll.data ? enroll.data.id : null;
              var totp = enroll && enroll.data ? enroll.data.totp : null;
              var qr = totp ? totp.qr_code : null;
              var uri = totp ? totp.uri : null;
              var qrUrl = qr;
              if (qr && String(qr).trim().indexOf('<svg') === 0){
                qrUrl = 'data:image/svg+xml;utf8,' + encodeURIComponent(qr);
              }

              openModal({
                title: 'Abilita 2FA',
                bodyHtml: [
                  '<p class="tva-modal__hint">1) Scansiona il QR con Google Authenticator / Authy / Microsoft Authenticator.</p>',
                  (qrUrl ? '<div class="tva-modal__qr"><img src="' + qrUrl + '" alt="QR 2FA" /></div>' : ''),
                  (uri ? '<p class="tva-modal__hint">2) Se non puoi scansionare, inserisci manualmente questa chiave/URI:<br><code style="word-break:break-all">' + uri + '</code></p>' : ''),
                  '<p class="tva-modal__hint">3) Inserisci il codice generato dall\'app per completare.</p>',
                  '<div class="tva-modal__field">',
                  '  <label class="tva-modal__label">Codice 2FA</label>',
                  '  <input class="tva-modal__input" id="tvaMfaSetupCode" inputmode="numeric" autocomplete="one-time-code" placeholder="123456" />',
                  '  <div class="tva-modal__error" id="tvaMfaSetupErr"></div>',
                  '</div>'
                ].join(''),
                actions: [
                  { label: 'Annulla', className: 'btn secondary', onClick: function(){ closeModal(); } },
                  { label: 'Verifica & Attiva', className: 'btn', onClick: async function(){
                    var codeEl = document.getElementById('tvaMfaSetupCode');
                    var code = codeEl ? String(codeEl.value || '').trim().replace(/\s+/g,'') : '';
                    if (!/^\d{6}$/.test(code)){
                      var e = document.getElementById('tvaMfaSetupErr');
                      if (e) e.textContent = 'Inserisci un codice valido (6 cifre).';
                      return;
                    }

                    var ok = false;
                    if (client.auth.mfa.challengeAndVerify){
                      var v = await client.auth.mfa.challengeAndVerify({ factorId: id, code: code });
                      ok = !(v && v.error);
                      if (!ok){
                        var ee = document.getElementById('tvaMfaSetupErr');
                        if (ee) ee.textContent = (v && v.error && v.error.message) ? v.error.message : 'Codice non valido.';
                        return;
                      }
                    }else{
                      var ch = await client.auth.mfa.challenge({ factorId: id });
                      if (ch && ch.error){
                        var ee2 = document.getElementById('tvaMfaSetupErr');
                        if (ee2) ee2.textContent = ch.error.message || 'Impossibile creare challenge.';
                        return;
                      }
                      var challengeId = ch && ch.data ? ch.data.id : null;
                      var v2 = await client.auth.mfa.verify({ factorId: id, challengeId: challengeId, code: code });
                      ok = !(v2 && v2.error);
                      if (!ok){
                        var ee3 = document.getElementById('tvaMfaSetupErr');
                        if (ee3) ee3.textContent = (v2 && v2.error && v2.error.message) ? v2.error.message : 'Codice non valido.';
                        return;
                      }
                    }

                    closeModal();
                    setMessage('2FA abilitata.', 'success');
                  } }
                ]
              });

              setTimeout(function(){
                var el = document.getElementById('tvaMfaSetupCode');
                if (el) el.focus();
              }, 50);
            }catch(err){
              setMessage(String(err && err.message ? err.message : err), 'error');
            }
          } }
        ]
      });
