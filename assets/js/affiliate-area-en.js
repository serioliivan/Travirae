...
(function(){
  ...
  const CREATOR_DASHBOARD_I18N = {
    it: {
      ...
      creatorPanelBadge: 'Vlog degli influencer'
    },
    en: {
  ...
  function init(){
    ...
    var elWidgetTotal = $('#affiliate-stat-widget-total');
    ...
    function ensureCreatorStatsWrap(){
      ...
      var insertAfter = $('.affiliate-balance-hero', views.dashboard) || $('.affiliate-stats-grid', views.dashboard) || $('.affiliate-id-block', views.dashboard) || $('.auth-card-header', views.dashboard);
  ...
  return;
    ...
    function renderChart(payouts){
      ...
      state.chart = new Chart(ctx, {
        ...
        data: {
          labels: labels,
          datasets: [{
            label: 'Guadagni mensili maturati (USD)',
            data: values,
            tension: 0.25,
            fill: false
          }]
    ...
    function updatePayoutsTable(payouts){
      ...
      if (!payouts || !payouts.length){
        ...
        td0.textContent = 'Nessun dato disponibile finora.';
        tr0.appendChild(td0);
    ...
    async function loadDashboardStats(){
      ...
      try{
        ...
        var widgetTotalsPromise = (typeof client.rpc === 'function')
          ? client.rpc('get_affiliate_widget_outbound_totals', { p_affiliate_slug: slug })
        ...
        if (widgetTotalsResp && !widgetTotalsResp.error){
          var widgetRows = [];
          if (Array.isArray(widgetTotalsResp.data)) widgetRows = widgetTotalsResp.data;
          else if (widgetTotalsResp.data) widgetRows = [widgetTotalsResp.data];
          if (widgetRows.length){
            var widgetRow = widgetRows[0] || {};
            totalWidgetClicks = Number(widgetRow.widget_clicks_total || 0);
            if (!isFinite(totalWidgetClicks) || totalWidgetClicks <= 0){
              var siteWidgetClicks = Number(widgetRow.widget_clicks_site_total || 0);
              var creatorWidgetClicksFromRpc = Number(widgetRow.widget_clicks_creator_total || 0);
              totalWidgetClicks = (isFinite(siteWidgetClicks) ? siteWidgetClicks : 0) + (isFinite(creatorWidgetClicksFromRpc) ? creatorWidgetClicksFromRpc : 0);
            }
            if (!isFinite(totalWidgetClicks)) totalWidgetClicks = 0;
          }
        }
        if (creatorStats && creatorStats.widget_clicks != null){
          var creatorWidgetClicks = Number(creatorStats.widget_clicks || 0);
          if (isFinite(creatorWidgetClicks) && creatorWidgetClicks > totalWidgetClicks){
            totalWidgetClicks = creatorWidgetClicks;
          }
        }
        if (elWidgetTotal) elWidgetTotal.textContent = String(totalWidgetClicks);
