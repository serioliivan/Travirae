...
// Richiede una re‑autenticazione (password) e, se presente, anche un codice 2FA.
	    ...
	    async function promptPasswordThen2FA(client, opts){
	      ...
	      try{
	              ...
	              buttons: [
	                ...
	                },
	                {
	                  ...
	                  onClick: async function(id){
	                    ...
	                    try{
	                      // Verify password (re-login). Non cambia UX: mantiene l'utente loggato.
	                      ...
	                      console.warn(e);
	                      if (errEl){ errEl.textContent = 'Verifytion error. Please try again.'; errEl.style.display = 'block'; }
	                      if (modal){
	        ...
	        console.warn(e);
	        showToast('Verifytion error. Please try again.', 'error');
	        return false;
	      }
	    }
    ...
    function promptTotpVerifytion(factorId, title, subtitle){
      return new Promise(function(resolve){
        var body = ''+
    ...
    async function open2faSettings(){
      ...
      // Clean stale unverified factors (e.g. user pressed "Cancel" during setup)
      ...
      if (loginForm){
        loginForm.addEventListener('submit', async function(e){
          ...
          try{
            ...
            // (la lingua email viene decisa in fase di registrazione e può essere cambiata dal pulsante "Change language")
      ...
      if (forgotForm){
        forgotForm.addEventListener('submit', async function(e){
          ...
          try{
            ...
            setMessage('If the address exists in our system, you'll receive an email with a link to reset your password.', 'success');
            showView('login');
          }catch(err){
            console.error(err);
  ...
  return;
    ...
    async function loadDashboardStats(){
      ...
      try{
        ...
        if (btnOpenPayoutForm){
          // Non disabilitiamo il bottone: se l'utente ci clicca, mostriamo un popup "Insufficient balance".
