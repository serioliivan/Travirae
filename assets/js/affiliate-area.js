...
(function(){
  ...
  function init(){
    ...
    async function getVerifiedTotpFactor(){
      ...
      var totp = (res.data && res.data.totp) ? res.data.totp : [];
      if (!Array.isArray(totp)) totp = [];
      // Preferisci un fattore già verificato
      var verified = totp.filter(function(f){ return (f && (f.status === 'verified' || f.status === 'active')); });
      return (verified[0] || totp[0] || null);
    }

    async function ensureMfaAal2IfNeeded(){
      // Se l'utente non ha 2FA attiva, non fa nulla.
      if (!client || !client.auth || !client.auth.mfa || !client.auth.mfa.getAuthenticatorAssuranceLevel) return true;
      var level = await client.auth.mfa.getAuthenticatorAssuranceLevel();
      if (level.error) return true;
      var currentLevel = level.data ? level.data.currentLevel : null;
      var nextLevel = level.data ? level.data.nextLevel : null;

      if (currentLevel === 'aal1' && nextLevel === 'aal2'){
        // Richiedi OTP
        var factor = await getVerifiedTotpFactor();
        if (!factor || !factor.id){
          setMessage('2FA attiva ma nessun fattore TOTP disponibile.', 'error');
          return false;
        }

        var ok = await promptTotpVerification(factor.id, 'Verifica 2FA', 'Inserisci il codice a 6 cifre generato dalla tua app Authenticator per completare l’accesso.');
        return ok;
      }
    ...
    function normalizeQrToDataUrl(qr){
      ...
      // Se è già un data URL, usa quello
    ...
    async function open2faSettings(){
      ...
      var factor = await getVerifiedTotpFactor();
      var enabled = !!(factor && factor.id && (factor.status === 'verified' || factor.status === 'active'));

      if (enabled){
        openModal({
          title: 'Sicurezza (2FA)',
          bodyHtml: '<p class="tva-modal__subtitle">La 2FA è <strong>attiva</strong> sul tuo account.</p><p>Se vuoi disattivarla, dovrai confermare con un codice.</p>',
          buttons: [
            { label: 'Chiudi', className: 'btn secondary', onClick: function(){ closeModal(); } },
            { label: 'Disattiva 2FA', className: 'btn', onClick: async function(){
                // Serve AAL2 per unenroll
                var ok = await ensureMfaAal2IfNeeded();
                if (!ok){
                  showToast('Verifica 2FA richiesta.', 'error');
                  return;
                }

                var un = await client.auth.mfa.unenroll({ factorId: factor.id });
                if (un.error){
                  showToast(un.error.message || 'Impossibile disattivare la 2FA.', 'error');
                  return;
                }
                closeModal();
      ...
      openModal({
        title: 'Sicurezza (2FA)',
        bodyHtml: '<p class="tva-modal__subtitle">La 2FA non è attiva. Puoi abilitarla con un’app Authenticator (Google Authenticator, Authy, 1Password, ecc.).</p>',
        buttons: [
          { label: 'Chiudi', className: 'btn secondary', onClick: function(){ closeModal(); } },
          { label: 'Abilita 2FA', className: 'btn', onClick: async function(){
              try{
                var en = await client.auth.mfa.enroll({ factorType: 'totp' });
                if (en.error) throw en.error;

                var id = en.data && en.data.id ? en.data.id : null;
                var qr = en.data && en.data.totp ? en.data.totp.qr_code : '';
                var secret = en.data && en.data.totp ? en.data.totp.secret : '';
                var qrUrl = normalizeQrToDataUrl(qr);

                var body2 = ''+
                  '<p class="tva-modal__subtitle">1) Scansiona il QR con la tua app Authenticator.</p>'+
                  (qrUrl ? '<div class="mfa-qr"><img alt="QR 2FA" src="'+qrUrl+'" /></div>' : '') +
                  (secret ? '<p class="tva-muted" style="margin-top:10px;">Se non puoi scansionare: <code style="user-select:all;">'+secret+'</code></p>' : '') +
                  '<p class="tva-modal__subtitle" style="margin-top:12px;">2) Inserisci il codice generato:</p>'+
                  '<label class="tva-field">Codice 2FA</label>'+
                  '<input id="tvaTotpEnrollCode" class="tva-input" inputmode="numeric" autocomplete="one-time-code" placeholder="123456" maxlength="6" />'+
                  '<div id="tvaTotpEnrollErr" class="tva-error" style="display:none;"></div>';
                ...
                openModal({
                  ...
                  buttons: [
                    ...
                    { label: 'Conferma', className: 'btn', onClick: async function(){
                        var codeEl = document.getElementById('tvaTotpEnrollCode');
                        var errEl = document.getElementById('tvaTotpEnrollErr');
                        var code = (codeEl && codeEl.value) ? codeEl.value.trim() : '';
                        if (!/^\d{6}$/.test(code)){
                          if (errEl){ errEl.textContent = 'Inserisci un codice valido (6 cifre).'; errEl.style.display = 'block'; }
                          return;
                        ...
                        try{
                          // challenge + verify
                          if (client.auth.mfa.challengeAndVerify){
                            var v1 = await client.auth.mfa.challengeAndVerify({ factorId: id, code: code });
                            if (v1.error) throw v1.error;
                          }else{
                            var ch = await client.auth.mfa.challenge({ factorId: id });
                            if (ch.error) throw ch.error;
                            var v = await client.auth.mfa.verify({ factorId: id, challengeId: ch.data.id, code: code });
                            if (v.error) throw v.error;
                          }
                ...
                showToast((err && err.message) ? err.message : 'Impossibile abilitare la 2FA.', 'error');
              }
            } }
        ]
      });
