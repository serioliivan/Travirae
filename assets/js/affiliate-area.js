...
// Richiede una re‑autenticazione (password) e, se presente, anche un codice 2FA.
	        ...
	        var password = await (function(){
	          return new Promise(function(resolve){
	            var modalId = openModal({
	              title: opts.passwordTitle || 'Conferma password',
	              bodyHtml: [
	                '<div class="tva-modal__hint">',
	                  (opts.passwordHint || 'Per continuare, inserisci la password del tuo account.'),
	                '</div>',
	              ...
	              buttons: [
	                ...
	                },
	                {
	                  ...
	                  onClick: async function(id){
	                    ...
	                    pw = pw.trim();
	                    if (!pw){
	                      if (errEl){ errEl.textContent = 'Inserisci la password.'; errEl.style.display = 'block'; }
	                      return;
	                    }
	                    // small loading lock
	                    ...
	                    try{
	                      ...
	                      if (si && si.error){
	                        if (errEl){ errEl.textContent = 'Password non corretta.'; errEl.style.display = 'block'; }
	                        if (modal){
	                      ...
	                      console.warn(e);
	                      if (errEl){ errEl.textContent = 'Errore durante la verifica. Riprova.'; errEl.style.display = 'block'; }
	                      if (modal){
              ...
              openModal({
                ...
                bodyHtml:
                  '<div class="tva-modal__stack">'+
                    '<div style="margin:10px 0 14px; padding:12px; border:1px solid #fde68a; background:#fffbeb; border-radius:12px; font-size:13px; line-height:1.35; color:#111;"><strong>Importante:</strong> se perdi accesso alla tua app Authenticator potresti non riuscire più ad <strong>accedere</strong> al tuo account e non potresti <strong>reimpostare la password</strong> (“Password dimenticata”) finché non recuperi la 2FA. Assicurati di conservare l’accesso al dispositivo/app e valuta di aggiungere la 2FA anche su un secondo dispositivo.</div>'+
                ...
                buttons: [
                  ...
                  { label: 'Verifica & Attiva', className: 'btn btn-primary', onClick: async function(){
                      ...
                      closeModal();
                      showToast('2FA attivata! Ricorda: non perdere l’accesso alla tua app Authenticator, altrimenti potresti non riuscire a recuperare l’account.', 'success');
                    } }
                ],
    ...
    function openChangeLangModal(){
      var client = refreshClient();
      if (!client){
        showToast('Supabase non è pronto. Riprova tra qualche secondo.', 'error');
        return;
      }
      ...
      client.auth.getUser().then(function(res){
        ...
        if (!user){
          showToast('Devi essere autenticato per cambiare lingua.', 'error');
          return;
        }
        ...
        openModal({
          title: 'Cambia Lingua',
          bodyHtml:
            '<p class="tva-modal__subtitle">Seleziona la lingua con cui vuoi ricevere le email di Travirae.</p>' +
            '<div class="tva-field">' +
            ...
            '</div>',
          buttons: [
            { label: 'Annulla', className: 'btn btn-outline', onClick: function(){ closeModal(); } },
            { label: 'Salva', className: 'btn btn-primary', onClick: async function(){
              var selEl = document.getElementById('tvaLangSelect');
              var errEl = document.getElementById('tvaLangError');
              var next = selEl ? selEl.value : '';
              if (errEl){ errEl.style.display = 'none'; errEl.textContent = ''; }
              if (!next){
                if (errEl){ errEl.textContent = 'Seleziona una lingua per continuare.'; errEl.style.display = 'block'; }
                return;
              }
              if (supported.indexOf(next) === -1){
                if (errEl){ errEl.textContent = 'Lingua non valida.'; errEl.style.display = 'block'; }
                return;
              }
              ...
              try{
                ...
                closeModal();
                showToast('Lingua aggiornata correttamente.', 'success');
              }catch(e){
                console.warn(e);
                showToast((e && e.message) ? e.message : 'Errore durante il cambio lingua.', 'error');
              }
            } }
          ]
        });
        ...
        console.warn(e);
        showToast('Impossibile recuperare il profilo utente.', 'error');
      });
    }
          ...
          try{
            ...
            if (result.error){
              ...
              if(msg.includes('already') && msg.includes('registered')){
                setMessage('Mail già registrata, prova con un\'altra mail.', 'error');
              }else{
...
try {
  var mfaOk = await ensureMfaAal2IfNeeded(client, { force: true });
  if (!mfaOk){
    setMessage('Per aggiornare la password è necessaria la verifica 2FA (codice Authenticator).', 'error');
    return;
  }
} catch (e) {
...
if (result.error){
  var em = result.error.message || 'Errore durante l\'aggiornamento della password.';
  if (String(em).includes('AAL2') || String(em).toLowerCase().includes('aal2 session')){
    em = 'Per cambiare password con la 2FA attiva devi inserire anche il codice Authenticator (2FA). Riprova e completa la verifica 2FA.';
  }
  setMessage(em, 'error');
  return;
}
          ...
          if ((Number(state.availableBalance || 0) || 0) < 50){
            ...
            return;
	          ...
	          var okAuth = await promptPasswordThen2FA(refreshClient(), {
	            title: 'Conferma richiesta pagamento',
	            message: 'Per continuare, inserisci la password del tuo account. Se hai attiva la 2FA, ti verrà richiesto anche il codice.',
	            passwordTitle: 'Conferma password'
	          });
	          if (!okAuth){
	            return;
	          }
