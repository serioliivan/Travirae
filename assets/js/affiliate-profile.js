...
(function(){
  ...
  function getProfileChangeRequestInfo(){
    if (state.profilePendingRevision && normalizeDraftStatus(state.profilePendingRevision.status) === 'changes_requested'){
      return { status: state.profilePendingRevision.status, note: String(state.profilePendingRevision.review_notes || '').trim() || getDefaultChangeRequestText('profile') };
    }
    if (state.profile && normalizeDraftStatus(state.profile.status) === 'changes_requested'){
      return { status: state.profile.status, note: String(state.profile.review_notes || '').trim() || getDefaultChangeRequestText('profile') };
    }
    return null;
  }
  ...
  function getPostChangeRequestInfo(post){
    ...
    if (pending && normalizeDraftStatus(pending.status) === 'changes_requested'){
      return { status: pending.status, note: String(pending.review_notes || '').trim() || getDefaultChangeRequestText('post') };
    }
    if (normalizeDraftStatus(post.status) === 'changes_requested'){
      return { status: post.status, note: String(post.review_notes || '').trim() || getDefaultChangeRequestText('post') };
    }
    return null;
  }
