...
(function(){
  ...
  function renderApp(){
    ...
    APP.innerHTML = '' +
      ...
      '<div id="if-affiliate-app-shell" hidden>' +
        ...
        '</section>' +
      '</div>' +
      '<datalist id="if-stay22-suggestions"></datalist>' +
      '<datalist id="if-airport-suggestions"></datalist>' +
  ...
  function populateAutocompleteDatalists(){
    if (typeof I.ensureWidgetAutocompleteCatalog !== 'function') return;
    I.ensureWidgetAutocompleteCatalog().then(function(catalog){
      var stay22List = $('#if-stay22-suggestions');
      var airportList = $('#if-airport-suggestions');
      if (stay22List && !stay22List.dataset.loaded){
        stay22List.innerHTML = (catalog.stay22 || []).map(function(label){
          return '<option value="' + I.escapeAttr(label) + '"></option>';
        }).join('');
        stay22List.dataset.loaded = '1';
      }
      if (airportList && !airportList.dataset.loaded){
        airportList.innerHTML = (catalog.airports || []).map(function(item){
          return '<option value="' + I.escapeAttr(item.label) + '"></option>';
        }).join('');
        airportList.dataset.loaded = '1';
      }
    }).catch(function(err){
      console.error(err);
    });
    ...
    blocksWrap.innerHTML = (editor.blocks || []).map(function(block, index){
      ...
      if (block.type === 'heading'){
        ...
        body = '' +
          ...
          '<div class="form-group"><label>Address / destination</label><input autocomplete="off" list="if-stay22-suggestions" placeholder="Type a city or destination" type="text" value="' + I.escapeAttr(block.destination || '') + '" data-block-field="destination" data-index="' + index + '"/></div>' +
        ...
        body = '' +
          ...
          '<div class="form-group"><label>Description</label><textarea rows="3" data-block-field="description" data-index="' + index + '">' + I.escapeHtml(block.description || '') + '</textarea></div>' +
          '<p class="muted small">Select a suggestion or type a valid 3-letter IATA code.</p>';
      }
