...
(function(){
  ...
  var TEXT = {
    it: {
      ...
      noProfileLive: 'Il profilo pubblico live resta online fino all’approvazione admin della nuova revisione.',
      noPostLive: 'Se modifichi un post già pubblicato, la versione live resta online fino all’approvazione admin.',
      uploadAvatar: 'Carica avatar',
      uploadCover: 'Carica cover',
      emptyEditor: 'Usa blocchi controllati: heading, paragrafo, immagine, lista, quote, callout, separatore, widget Stay22, widget Aviasales.',
      ...
      widgetHint: 'Travirae resta partner principale. Il creator è tracciato come sotto-affiliato interno.',
      status_draft: 'Bozza',
    ...
    en: {
      publicProfile: 'Public profile', myPosts: 'My posts', privateStats: 'Private creator stats', composer: 'Post composer', avatar: 'Profile image', displayName: 'Profile name', publicSlug: 'Public slug', bio: 'Biography', socialLinks: 'Social links', saveProfileDraft: 'Save profile', sendProfileReview: 'Send profile to review', liveProfile: 'Live published version', pendingProfile: 'Pending draft / review', title: 'Post title', postSlug: 'Post slug', coverImage: 'Cover image', excerpt: 'Card caption / excerpt', livePreview: 'Live card preview', postContent: 'Post content', addBlock: 'Add block', startNewPost: 'Create new post', cancelEdit: 'Cancel edit', noProfileLive: 'The live public profile stays online until the admin approves the new revision.', noPostLive: 'If you edit a published post, the live version stays online until admin approval.', uploadAvatar: 'Upload avatar', uploadCover: 'Upload cover', emptyEditor: 'Use controlled blocks only: heading, paragraph, image, list, quote, callout, separator, Stay22 widget, Aviasales widget.', currentEditing: 'Editing post', reviewNotes: 'Review notes', noReviewNotes: 'No review notes.', viewPublicPage: 'Open public page', creatorStatsHint: 'These statistics are private and visible only to you.', widgetHint: 'Travirae remains the main partner. The creator is tracked as an internal sub-affiliate.', status_draft: 'Draft', status_pending_review: 'In review', status_published: 'Published', status_changes_requested: 'Changes requested', status_rejected: 'Rejected', status_archived: 'Archived', status_deleted: 'Deleted', saveDraftBtn: 'Save draft', submitReviewBtn: 'Send to review'
    }
  };
  function L(key){ return (TEXT[LANG] && TEXT[LANG][key]) || (TEXT.en && TEXT.en[key]) || key; }
    ...
    APP.innerHTML = '' +
      ...
      '<div id="if-affiliate-app-shell" hidden>' +
        '<div class="if-creator-grid">' +
          '<section class="auth-card if-panel">' +
            '<div class="if-section-head"><div><h2>' + I.escapeHtml(L('publicProfile')) + '</h2><p class="muted small">' + I.escapeHtml(L('noProfileLive')) + '</p></div><a class="btn secondary" id="if-open-public-page" href="' + I.escapeAttr(I.buildPagePath('influencers-vlogs.html')) + '?creator=' + encodeURIComponent(state.profileDraft && state.profileDraft.public_slug ? state.profileDraft.public_slug : state.affiliateSlug || '') + '" target="_blank" rel="noopener">' + I.escapeHtml(L('viewPublicPage')) + '</a></div>' +
            '<div id="if-profile-review-note"></div>' +
            '<form class="auth-form" id="if-profile-form">' +
              '<div class="if-avatar-upload"><div id="if-avatar-preview"></div><div><label class="small muted">' + I.escapeHtml(L('avatar')) + '</label><input accept="image/png,image/jpeg,image/webp" id="if-profile-avatar-input" type="file"/><p class="muted small">WEBP · max 8MB · output 400×400</p></div></div>' +
              '<div class="form-row-2"><div class="form-group"><label for="if-profile-display-name">' + I.escapeHtml(L('displayName')) + '</label><input id="if-profile-display-name" required type="text"/></div><div class="form-group"><label for="if-profile-public-slug">' + I.escapeHtml(L('publicSlug')) + '</label><input id="if-profile-public-slug" required type="text"/></div></div>' +
        ...
        '</section>' +
        '<section class="auth-card if-panel" id="if-editor-panel">' +
          ...
          '<form class="auth-form" id="if-post-form">' +
            ...
            '<div class="form-row-2"><div class="form-group"><label for="if-post-title">' + I.escapeHtml(L('title')) + '</label><input id="if-post-title" required type="text"/></div><div class="form-group"><label for="if-post-slug">' + I.escapeHtml(L('postSlug')) + '</label><input id="if-post-slug" required type="text"/></div></div>' +
            '<div class="if-cover-upload"><div id="if-cover-preview"></div><div><label class="small muted">' + I.escapeHtml(L('coverImage')) + '</label><input accept="image/png,image/jpeg,image/webp" id="if-post-cover-input" type="file"/><p class="muted small">WEBP · max 8MB · output 1600×900</p></div></div>' +
            ...
            '<div class="if-card-preview-wrap"><label class="small muted">' + I.escapeHtml(L('livePreview')) + '</label><div id="if-post-card-preview"></div></div>' +
            '<div class="if-section-head if-section-head--blocks"><div><h3>' + I.escapeHtml(L('postContent')) + '</h3><p class="muted small">' + I.escapeHtml(L('widgetHint')) + '</p></div></div>' +
  ...
  function bindStaticUi(){
    ...
    var profileSlug = $('#if-profile-public-slug');
    if (profileName && profileSlug){
      profileName.addEventListener('input', function(){
        if (profileSlug && (!profileSlug.dataset.touched || profileSlug.dataset.touched !== '1')){
          profileSlug.value = slugify(profileName.value || state.affiliateSlug || 'creator');
        }
      });
      profileSlug.addEventListener('input', function(){ profileSlug.dataset.touched = '1'; });
    }

    var postTitle = $('#if-post-title');
    var postSlug = $('#if-post-slug');
    if (postTitle && postSlug){
      postTitle.addEventListener('input', function(){
        if (state.autoSlug) postSlug.value = slugify(postTitle.value || '');
        updateEditorStateFromForm();
      });
      postSlug.addEventListener('input', function(){ state.autoSlug = false; updateEditorStateFromForm(); });
    }
    ...
    if (newPost) newPost.addEventListener('click', function(){ resetEditor(); });
    var cancelEdit = $('#if-cancel-edit-post');
    if (cancelEdit) cancelEdit.addEventListener('click', function(){ resetEditor(); });
  }
  ...
  function renderProfileForm(){
    ...
    var draft = state.profileDraft || emptyProfileDraft();
    ...
    var reviewEl = $('#if-profile-review-note');
    if (reviewEl){
      var noteText = state.profilePendingRevision && state.profilePendingRevision.review_notes ? state.profilePendingRevision.review_notes : '';
      reviewEl.innerHTML = '' +
        '<div class="if-note-box">' +
          '<div class="if-note-box__head">' + (state.profile && state.profile.current_published_revision_id ? badge('published') + '<span>' + I.escapeHtml(L('liveProfile')) + '</span>' : badge(state.profilePendingRevision ? state.profilePendingRevision.status : 'draft') + '<span>' + I.escapeHtml(L('pendingProfile')) + '</span>') + '</div>' +
          '<p class="muted small">' + I.escapeHtml(noteText || L('noReviewNotes')) + '</p>' +
        '</div>';
    }
    var displayName = $('#if-profile-display-name'); if (displayName) displayName.value = draft.display_name || '';
    var publicSlug = $('#if-profile-public-slug'); if (publicSlug){ publicSlug.value = draft.public_slug || ''; publicSlug.dataset.touched = '0'; }
  ...
  function updateEditorStateFromForm(){
    ...
    var slug = $('#if-post-slug');
    var excerpt = $('#if-post-excerpt');
    state.editorData.title = title ? String(title.value || '').trim() : '';
    state.editorData.post_slug = slug ? slugify(slug.value || '') : '';
    if (slug) slug.value = state.editorData.post_slug;
  ...
  function renderEditor(){
    ...
    var slug = $('#if-post-slug'); if (slug){ slug.value = editor.post_slug || ''; slug.disabled = !!(state.currentPost && state.currentPost.current_live_revision_id); }
    ...
    if (reviewNote){
      ...
      reviewNote.innerHTML = '<div class="if-note-box"><div class="if-note-box__head">' + badge(state.currentRevision ? state.currentRevision.status : (state.currentPost ? state.currentPost.status : 'draft')) + '<span>' + I.escapeHtml(note || L('noReviewNotes')) + '</span></div></div>';
    }
    ...
    blocksWrap.innerHTML = (editor.blocks || []).map(function(block, index){
      ...
      if (block.type === 'heading'){
        ...
        body = '' +
          ...
          '<p class="muted small">' + I.escapeHtml(L('widgetHint')) + '</p>';
  ...
  async function deletePost(postId){
    ...
    try{
      ...
      if (state.currentPost && state.currentPost.id === postId) resetEditor();
    }catch(err){
      console.error(err);
  ...
  function readProfileForm(){
    var draft = emptyProfileDraft();
    draft.display_name = String(($('#if-profile-display-name') || {}).value || '').trim();
    draft.public_slug = slugify(String(($('#if-profile-public-slug') || {}).value || '').trim()) || slugify(draft.display_name || state.affiliateSlug || 'creator');
    draft.bio = String(($('#if-profile-bio') || {}).value || '').trim();
    draft.avatar_path = (state.profileDraft && state.profileDraft.avatar_path) || '';
    draft.social_links = {};
    var invalid = false;
    Object.keys(I.SOCIALS).forEach(function(platform){
      var input = $('#if-social-' + platform);
  ...
  async function savePost(mode){
    ...
    try{
      ...
      if (!editor.post_slug) editor.post_slug = slugify(editor.title);
      if (!editor.post_slug) throw new Error('Slug post obbligatorio.');
  ...
  async function loadDashboardData(){
    ...
    renderStats();
    renderPostsList();
    if (!state.currentPost) renderEditor();
  }
  ...
  async function init(){
    ...
    state.editorData = basePostEditor();
    await loadDashboardData();
    if (!state.currentPost) resetEditor();
  }
