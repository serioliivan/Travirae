...
(function(){
  ...
  function canonicalizePostBlocks(blocks){
    return (Array.isArray(blocks) ? blocks : []).map(function(block){
      ...
      if (out.type === 'heading'){
        out.level = String(block.level || 'h2');
  ...
  function renderAdminNoteBox(info){
    ...
    return '' +
      '<div class="if-note-box if-note-box--admin-request">' +
        ...
        '<div class="if-note-box__body">' + I.escapeHtml(info.note).replace(/\n/g, '<br/>') + '</div>' +
      '</div>';
  }

  function getProfileChangeRequestInfo(){
    if (state.profilePendingRevision && normalizeDraftStatus(state.profilePendingRevision.status) === 'changes_requested' && state.profilePendingRevision.review_notes){
      return { status: state.profilePendingRevision.status, note: String(state.profilePendingRevision.review_notes || '') };
    }
    if (state.profile && normalizeDraftStatus(state.profile.status) === 'changes_requested' && state.profile.review_notes){
      return { status: state.profile.status, note: String(state.profile.review_notes || '') };
    }
    return null;
  }
  ...
  function getPostChangeRequestInfo(post){
    ...
    if (pending && normalizeDraftStatus(pending.status) === 'changes_requested' && pending.review_notes){
      return { status: pending.status, note: String(pending.review_notes || '') };
    }
    if (normalizeDraftStatus(post.status) === 'changes_requested' && post.review_notes){
      return { status: post.status, note: String(post.review_notes || '') };
    }
    return null;
  }
  ...
  function syncBodyPopupState(){
    document.body.classList.toggle('if-popup-open', isProfilePopupOpen() || isExitPopupOpen() || isDeletePopupOpen());
  }
