...
(function(){
  ...
  function syncComposerLayout(){
    var panel = $('#if-editor-panel');
    if (!panel) return;
    var isNewPostComposer = !!(state.editorOpen && !state.currentPost);
    panel.classList.toggle('has-side-preview', !!(state.previewVisible || state.postPreviewVisible));
    panel.classList.toggle('is-new-post-composer', isNewPostComposer);
    document.body.classList.toggle('if-composer-modal-open', isNewPostComposer);

    var cancelBtn = $('#if-cancel-edit-post');
    if (cancelBtn){
      cancelBtn.classList.toggle('if-composer-close', isNewPostComposer);
      cancelBtn.setAttribute('aria-label', L('cancelEdit'));
      cancelBtn.setAttribute('title', L('cancelEdit'));
      cancelBtn.textContent = isNewPostComposer ? '×' : L('cancelEdit');
    }
  }
  ...
  function syncBodyPopupState(){
    document.body.classList.toggle('if-popup-open', isProfilePopupOpen() || isExitPopupOpen());
  }
