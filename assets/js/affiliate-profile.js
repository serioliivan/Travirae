...
(function(){
  ...
  var CREATOR_SHARE_AND_BLOCK_TEXT_V1 = {
    it: {
      addBlock: '+',
      noBlocksBody: 'Clicca “+” per inserire testi, immagini, mappe e widget nel post.',
      shareProfile: 'Condividi profilo',
      profileLinkCopied: 'Link profilo copiato con successo.',
      profileLinkCopyError: 'Impossibile copiare il link. Riprova.',
      separatorSizeThin: 'Sottile',
      separatorSizeMedium: 'Normale',
      separatorSizeLarge: 'Grande',
      separatorSizeXLarge: 'Molto grande',
      moveBlockUp: 'Sposta blocco su',
      moveBlockDown: 'Sposta blocco giù',
      deleteBlock: 'Elimina blocco'
    },
    en: {
      addBlock: '+',
      noBlocksBody: 'Click “+” to insert text, images, maps and widgets into the post.',
      shareProfile: 'Share profile',
      profileLinkCopied: 'Profile link copied successfully.',
      profileLinkCopyError: 'Unable to copy the link. Try again.',
      separatorSizeThin: 'Thin',
      separatorSizeMedium: 'Normal',
      separatorSizeLarge: 'Large',
      separatorSizeXLarge: 'Extra large',
    ...
    de: {
      addBlock: '+',
      noBlocksBody: 'Klicke auf „+“, um Text, Bilder, Karten und Widgets in den Beitrag einzufügen.',
      shareProfile: 'Profil teilen',
      profileLinkCopied: 'Profillink erfolgreich kopiert.',
      profileLinkCopyError: 'Link konnte nicht kopiert werden. Bitte erneut versuchen.',
      separatorSizeThin: 'Dünn',
      separatorSizeMedium: 'Normal',
      separatorSizeLarge: 'Groß',
      separatorSizeXLarge: 'Sehr groß',
    ...
    es: {
      addBlock: '+',
      noBlocksBody: 'Haz clic en “+” para insertar texto, imágenes, mapas y widgets en el post.',
      shareProfile: 'Compartir perfil',
      profileLinkCopied: 'Enlace del perfil copiado correctamente.',
      profileLinkCopyError: 'No se pudo copiar el enlace. Inténtalo de nuevo.',
      separatorSizeThin: 'Fino',
      separatorSizeMedium: 'Normal',
      separatorSizeLarge: 'Grande',
      separatorSizeXLarge: 'Muy grande',
    ...
    fr: {
      addBlock: '+',
      noBlocksBody: 'Clique sur « + » pour insérer du texte, des images, des cartes et des widgets dans le post.',
      shareProfile: 'Partager le profil',
      profileLinkCopied: 'Lien du profil copié avec succès.',
      profileLinkCopyError: 'Impossible de copier le lien. Réessayez.',
      separatorSizeThin: 'Fin',
      separatorSizeMedium: 'Normal',
      separatorSizeLarge: 'Grand',
      separatorSizeXLarge: 'Très grand',
    ...
    nl: {
      addBlock: '+',
      noBlocksBody: 'Klik op “+” om tekst, afbeeldingen, kaarten en widgets aan de post toe te voegen.',
      shareProfile: 'Profiel delen',
      profileLinkCopied: 'Profiellink gekopieerd.',
      profileLinkCopyError: 'Kan de link niet kopiëren. Probeer opnieuw.',
      separatorSizeThin: 'Dun',
      separatorSizeMedium: 'Normaal',
      separatorSizeLarge: 'Groot',
      separatorSizeXLarge: 'Zeer groot',
      moveBlockUp: 'Blok omhoog verplaatsen',
      moveBlockDown: 'Blok omlaag verplaatsen',
      deleteBlock: 'Blok verwijderen'
    },
    ru: {
      addBlock: '+',
      noBlocksBody: 'Нажмите «+», чтобы вставить текст, изображения, карты и виджеты в пост.',
      shareProfile: 'Поделиться профилем',
      profileLinkCopied: 'Ссылка на профиль успешно скопирована.',
      profileLinkCopyError: 'Не удалось скопировать ссылку. Попробуйте ещё раз.',
      separatorSizeThin: 'Тонкий',
      separatorSizeMedium: 'Обычный',
      separatorSizeLarge: 'Большой',
      separatorSizeXLarge: 'Очень большой',
      moveBlockUp: 'Переместить блок вверх',
      moveBlockDown: 'Переместить блок вниз',
      deleteBlock: 'Удалить блок'
    },
    ar: {
      addBlock: '+',
      noBlocksBody: 'اضغط على «+» لإدراج نصوص وصور وخرائط وويدجت داخل المنشور.',
      shareProfile: 'مشاركة الملف',
      profileLinkCopied: 'تم نسخ رابط الملف بنجاح.',
      profileLinkCopyError: 'تعذّر نسخ الرابط. حاول مرة أخرى.',
      separatorSizeThin: 'رفيع',
      separatorSizeMedium: 'عادي',
      separatorSizeLarge: 'كبير',
      separatorSizeXLarge: 'كبير جدًا',
      moveBlockUp: 'نقل الكتلة لأعلى',
      moveBlockDown: 'نقل الكتلة لأسفل',
      deleteBlock: 'حذف الكتلة'
    },
    zh: {
      addBlock: '+',
      noBlocksBody: '点击“+”即可向文章中插入文字、图片、地图和小组件。',
      shareProfile: '分享资料',
      profileLinkCopied: '资料链接已成功复制。',
      profileLinkCopyError: '无法复制链接，请重试。',
      separatorSizeThin: '细',
      separatorSizeMedium: '普通',
      separatorSizeLarge: '大',
      separatorSizeXLarge: '超大',
      moveBlockUp: '上移区块',
      moveBlockDown: '下移区块',
      deleteBlock: '删除区块'
    }
  };
  Object.keys(CREATOR_SHARE_AND_BLOCK_TEXT_V1).forEach(function(lang){
    TEXT[lang] = Object.assign({}, TEXT[lang] || {}, CREATOR_SHARE_AND_BLOCK_TEXT_V1[lang]);
  });
  ...
  function canonicalizePostBlocks(blocks){
    return (Array.isArray(blocks) ? blocks : []).map(function(block){
      ...
      if (out.type === 'heading'){
        ...
        out.size = String(block.size || 'medium').trim() || 'medium';
        out.align = String(block.align || 'left').trim() || 'left';
      } else if (out.type === 'separator'){
        out.size = sanitizeSeparatorSize(block.size || 'medium');
  ...
  function getTrackedProfileUrl(){
    var url = new URL(I.buildPagePath('influencers-vlogs.html'), window.location.href);
    var creatorSlug = String(getPublicProfileSlugForLink() || '').trim();
    var affiliateSlug = String(state.affiliateSlug || '').trim();
    if (creatorSlug) url.searchParams.set('creator', creatorSlug);
    if (affiliateSlug) url.searchParams.set('ref', affiliateSlug);
    return url.toString();
  }
  ...
  function renderProfileSummary(){
    ...
    var shareBtn = $('#if-share-summary-profile');
    ...
    if (shareBtn){
      shareBtn.hidden = !canOpenPublic;
      shareBtn.style.display = canOpenPublic ? '' : 'none';
      shareBtn.setAttribute('data-share-url', getTrackedProfileUrl());
    }
  }
  ...
  function separatorSizeOptions(selected){
    var current = sanitizeSeparatorSize(selected || 'medium');
    var options = [
      { value: 'thin', label: L('separatorSizeThin') },
      { value: 'medium', label: L('separatorSizeMedium') },
      { value: 'large', label: L('separatorSizeLarge') },
      { value: 'xlarge', label: L('separatorSizeXLarge') }
    ];
    return options.map(function(item){
      return '<option value="' + I.escapeAttr(item.value) + '"' + (item.value === current ? ' selected' : '') + '>' + I.escapeHtml(item.label) + '</option>';
    }).join('');
  }
  ...
  function buildAddBlockMenuHtml(){
    return getBlockButtons().map(function(item){
  ...
  function renderBlockControlIcon(name){
    if (name === 'up'){
      return '<svg class="if-block-action__icon" viewBox="0 0 24 24" aria-hidden="true" focusable="false"><path d="M12 5l-6 6"></path><path d="M12 5l6 6"></path><path d="M12 6v13"></path></svg>';
    }
    if (name === 'down'){
      return '<svg class="if-block-action__icon" viewBox="0 0 24 24" aria-hidden="true" focusable="false"><path d="M12 19l-6-6"></path><path d="M12 19l6-6"></path><path d="M12 5v13"></path></svg>';
    }
    return '<svg class="if-block-action__icon" viewBox="0 0 24 24" aria-hidden="true" focusable="false"><path d="M4 7h16"></path><path d="M10 11v6"></path><path d="M14 11v6"></path><path d="M6 7l1 14h10l1-14"></path><path d="M9 7V4h6v3"></path></svg>';
  }

  function renderBlockControls(index){
    var upLabel = L('moveBlockUp');
    var downLabel = L('moveBlockDown');
    var deleteLabel = L('deleteBlock');
    return '<div class="if-block-card__actions">' +
      '<button class="if-block-action if-block-action--move" type="button" data-block-up="' + index + '" aria-label="' + I.escapeAttr(upLabel) + '" title="' + I.escapeAttr(upLabel) + '">' + renderBlockControlIcon('up') + '</button>' +
      '<button class="if-block-action if-block-action--move" type="button" data-block-down="' + index + '" aria-label="' + I.escapeAttr(downLabel) + '" title="' + I.escapeAttr(downLabel) + '">' + renderBlockControlIcon('down') + '</button>' +
      '<button class="if-block-action if-block-action--delete" type="button" data-block-delete="' + index + '" aria-label="' + I.escapeAttr(deleteLabel) + '" title="' + I.escapeAttr(deleteLabel) + '">' + renderBlockControlIcon('delete') + '</button>' +
    '</div>';
  }

  function closeAllAddBlockMenus(){
    $all('.if-add-menu.is-open').forEach(function(menuRoot){
      menuRoot.classList.remove('is-open');
      var insertRoot = menuRoot.closest ? menuRoot.closest('.if-block-insert') : null;
      if (insertRoot) insertRoot.classList.remove('is-add-menu-open');
      var toggle = $('[data-add-menu-toggle="1"]', menuRoot);
      if (toggle) toggle.setAttribute('aria-expanded', 'false');
      var dropdown = $('.if-add-menu__dropdown', menuRoot);
  ...
  function insertBlockAt(type, index){
    ...
    if (!Array.isArray(state.editorData.blocks)) state.editorData.blocks = [];
    var pos = parseInt(index, 10);
    if (!isFinite(pos) || pos < 0 || pos > state.editorData.blocks.length) pos = state.editorData.blocks.length;
    state.editorData.blocks.splice(pos, 0, defaultBlock(type));
    closeAllAddBlockMenus();
    renderEditor();
  }

  function renderAddBlockControl(insertIndex, extraClass){
    var rawIndex = insertIndex === 'end' ? 'end' : String(parseInt(insertIndex, 10) || 0);
    var label = L('addBlock');
    return '' +
      '<div class="if-block-insert ' + I.escapeAttr(extraClass || '') + '">' +
        '<div class="if-add-menu" data-add-menu="1" data-insert-index="' + I.escapeAttr(rawIndex) + '">' +
          '<button class="btn secondary if-add-menu__toggle" type="button" data-add-menu-toggle="1" aria-label="' + I.escapeAttr(label) + '" aria-expanded="false" title="' + I.escapeAttr(label) + '">' + I.escapeHtml(label) + '</button>' +
          '<div class="if-add-menu__dropdown if-add-menu__dropdown--insert" hidden></div>' +
        '</div>' +
      '</div>';
  }

  function bindAddBlockMenus(root){
    root = root || document;
    var menuRoots = [];
    if (root.matches && root.matches('[data-add-menu="1"]')) menuRoots.push(root);
    menuRoots = menuRoots.concat($all('[data-add-menu="1"]', root));
    menuRoots.forEach(function(menuRoot){
      var dropdown = $('.if-add-menu__dropdown', menuRoot);
      var toggle = $('[data-add-menu-toggle="1"]', menuRoot);
      if (dropdown && !dropdown.innerHTML) dropdown.innerHTML = buildAddBlockMenuHtml();
      if (menuRoot.__ifAddBlockMenuBound) return;
      menuRoot.__ifAddBlockMenuBound = true;
      if (toggle){
        toggle.addEventListener('click', function(e){
          e.preventDefault();
          e.stopPropagation();
          var willOpen = !!(dropdown && dropdown.hidden);
          closeAllAddBlockMenus();
          if (willOpen){
            menuRoot.classList.add('is-open');
            var insertRoot = menuRoot.closest ? menuRoot.closest('.if-block-insert') : null;
            if (insertRoot) insertRoot.classList.add('is-add-menu-open');
            if (toggle) toggle.setAttribute('aria-expanded', 'true');
            if (dropdown) dropdown.hidden = false;
          }
        });
      }
      $all('[data-add-block]', dropdown || menuRoot).forEach(function(btn){
        btn.addEventListener('click', function(e){
          e.preventDefault();
          e.stopPropagation();
          var rawIndex = menuRoot.getAttribute('data-insert-index');
          var index = rawIndex === 'end' ? (state.editorData && Array.isArray(state.editorData.blocks) ? state.editorData.blocks.length : 0) : parseInt(rawIndex || '0', 10);
          insertBlockAt(btn.getAttribute('data-add-block'), index);
        });
      });
    });
    if (!document.body.__ifAddMenusGlobalBound){
      document.body.__ifAddMenusGlobalBound = true;
      document.addEventListener('click', function(e){
        if (!e.target || !e.target.closest || !e.target.closest('.if-add-menu')) closeAllAddBlockMenus();
      });
      document.addEventListener('keydown', function(e){
        if (e.key === 'Escape') closeAllAddBlockMenus();
      });
    }
  }
        ...
        '<section class="auth-card if-panel if-creator-summary" id="if-profile-summary-panel" hidden>' +
          ...
          '<div class="if-creator-summary__content">' +
            ...
            '<div class="auth-actions if-creator-summary__actions">' +
              ...
              '<button class="btn secondary" hidden id="if-share-summary-profile" type="button">' + I.escapeHtml(L('shareProfile')) + '</button>' +
            '</div>' +
          '</div>' +
        '</section>' +
          ...
          '<form class="auth-form if-post-form" id="if-post-form">' +
            '<div class="if-post-form__layout">' +
              '<div class="if-post-form__main">' +
                ...
                '<div class="if-block-toolbar" id="if-block-toolbar">' + renderAddBlockControl('end', 'if-block-insert--toolbar') + '</div>' +
  ...
  function bindStaticUi(){
    ...
    var shareProfileBtn = $('#if-share-summary-profile');
    if (shareProfileBtn && !shareProfileBtn.__bound){
      shareProfileBtn.__bound = true;
      shareProfileBtn.addEventListener('click', async function(){
        var shareUrl = shareProfileBtn.getAttribute('data-share-url') || getTrackedProfileUrl();
        var copied = await copyTextToClipboard(shareUrl);
        notify(copied ? L('profileLinkCopied') : L('profileLinkCopyError'), copied ? 'success' : 'error');
      });
    }
    ...
    bindAddBlockMenus($('#if-editor-panel') || APP || document);
  ...
  function renderEditor(){
    ...
    var editorBlocks = Array.isArray(editor.blocks) ? editor.blocks : [];
    var hasBlocks = editorBlocks.length > 0;
    var blockToolbar = $('#if-block-toolbar');
    if (blockToolbar){
      blockToolbar.hidden = hasBlocks;
      blockToolbar.style.display = hasBlocks ? 'none' : '';
    }
    if (!hasBlocks){
    ...
    var blockHtml = [renderAddBlockControl(0, 'if-block-insert--between if-block-insert--first')];
    editorBlocks.forEach(function(block, index){
      var controls = renderBlockControls(index);
      var body = '';
      if (block.type === 'heading'){
        ...
        body = '<div class="form-group"><label>' + I.escapeHtml(L('fieldSize')) + '</label><select class="if-select" data-block-field="size" data-index="' + index + '">' + separatorSizeOptions(block.size || 'medium') + '</select></div>';
      ...
      blockHtml.push('<article class="if-block-card" data-block-type="' + I.escapeAttr(block.type) + '"><div class="if-block-card__head"><strong>' + I.escapeHtml(t('block' + block.type.charAt(0).toUpperCase() + block.type.slice(1)) || block.type) + '</strong>' + controls + '</div><div class="if-block-card__body">' + body + '</div></article>');
      blockHtml.push(renderAddBlockControl(index + 1, 'if-block-insert--between'));
    });
    blocksWrap.innerHTML = blockHtml.join('');

    bindAddBlockMenus(blocksWrap);
  ...
  async function loadDashboardData(){
    ...
    try{
      if (typeof state.client.rpc === 'function'){
        var widgetTotalsResp = await state.client.rpc('get_affiliate_widget_outbound_totals', { p_affiliate_slug: state.affiliateSlug });
        var widgetRows = widgetTotalsResp && !widgetTotalsResp.error && Array.isArray(widgetTotalsResp.data) ? widgetTotalsResp.data : [];
        if (widgetRows.length){
          var widgetRow = widgetRows[0] || {};
          var totalWidgetClicks = Number(widgetRow.widget_clicks_total || 0);
          if (!isFinite(totalWidgetClicks) || totalWidgetClicks <= 0){
            var siteWidgetClicks = Number(widgetRow.widget_clicks_site_total || 0);
            var creatorWidgetClicks = Number(widgetRow.widget_clicks_creator_total || 0);
            totalWidgetClicks = (isFinite(siteWidgetClicks) ? siteWidgetClicks : 0) + (isFinite(creatorWidgetClicks) ? creatorWidgetClicks : 0);
          }
          var currentWidgetClicks = Number(state.stats.widget_clicks || 0);
          if (isFinite(totalWidgetClicks) && totalWidgetClicks > currentWidgetClicks) state.stats.widget_clicks = totalWidgetClicks;
        }
      }
    }catch(_widgetTotalsErr){ }
