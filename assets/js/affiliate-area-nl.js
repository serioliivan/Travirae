      ...
      if (toastEl){
        ...
        toastEl.innerHTML = ''+
          '<div class="tva-toast__inner" role="alert" aria-live="polite">'+
            ...
            '<button type="button" class="tva-toast__close" aria-label="Sluiten">×</button>'+
          '</div>';
      ...
      modalEl.innerHTML = ''+
        '<div class="tva-modal__card" role="dialog" aria-modal="true">'+
          '<div class="tva-modal__header">'+
            ...
            '<button type="button" class="tva-modal__close" aria-label="Sluiten">×</button>'+
          '</div>'+
...
function openModal(opts){
    ...
    async function ensureMfaAal2IfNeeded(client, opts = {}){
      try{
        ...
        var ok = await promptAndVerifyTotpCode(client, factor.id, '2FA verifiëren');
        if(!ok) return false;

        showToast('2FA geverifieerd.', 'success');
        return true;
      }catch(e){
        console.warn(e);
        return true;
...
// Richiede una re‑autenticazione (password) e, se presente, anche un codice 2FA.
	    ...
	    async function promptPasswordThen2FA(client, opts){
	      ...
	      try{
	        ...
	        if (!email){
	          showToast('Ongeldige sessie. Log opnieuw in.', 'error');
	          return false;
	        }
	        ...
	        var password = await (function(){
	          return new Promise(function(resolve){
	            var modalId = openModal({
	              title: opts.passwordTitle || 'Wachtwoord bevestigen',
	              bodyHtml: [
	                '<div class="tva-modal__hint">',
	                  (opts.passwordHint || 'To continue, enter your account password.'),
	                '</div>',
	                '<input id="tvaReauthPassword" type="password" class="tva-modal__input" placeholder="Wachtwoord" autocomplete="current-password" />',
	                ...
	                },
	                {
	                  ...
	                  onClick: async function(id){
	                    ...
	                    try{
	                      ...
	                      console.warn(e);
	                      if (errEl){ errEl.textContent = 'Verificatiefout. Probeer het opnieuw.'; errEl.style.display = 'block'; }
	                      if (modal){
	        ...
	        console.warn(e);
	        showToast('Verificatiefout. Probeer het opnieuw.', 'error');
	        return false;
	      }
	    }
    ...
    async function promptAndVerifyTotpCode(client, factorId, promptTitle){
      var code = await promptTotpCodeModal(promptTitle || '2FA verifiëren');
            ...
            },
            {
              ...
              onClick: async function(){
                ...
                try{
                  ...
                  if (errEl){ errEl.textContent = (err && err.message) ? err.message : 'Ongeldige code. Probeer het opnieuw.'; errEl.style.display = 'block'; }
                }
              }
            }
          ]
    ...
    async function promptTotpCodeModal(promptTitle){
      promptTitle = promptTitle || '2FA verifiëren';
      return new Promise(function(resolve){
        var done = false;
        function finish(val){
          if (done) return;
        ...
        openModal({
          ...
          bodyHtml: ''+
            ...
            '</div>',
          buttons: [
            { label: 'Annuleren', className: 'btn btn-outline', onClick: function(){ closeModal(); finish(null); } },
    ...
    async function open2faSettings(){
      ...
      if (verifiedFactor && verifiedFactor.id){
        openModal({
          ...
          bodyHtml: '<p class="tva-modal__subtitle">2FA is actief. Om uit te schakelen, bevestig met een code uit je app.</p>',
          buttons: [
    ...
    function openChangeLangModal(){
      var client = refreshClient();
      if (!client){
        showToast('Supabase is nog niet klaar. Probeer het over een paar seconden opnieuw.', 'error');
        return;
      }
      ...
      client.auth.getUser().then(function(res){
        ...
        openModal({
          ...
          bodyHtml:
            ...
            '</div>',
          buttons: [
            { label: 'Annuleren', className: 'btn btn-outline', onClick: function(){ closeModal(); } },
    ...
    function bindAuthForms(){
      ...
      if (loginForm){
        loginForm.addEventListener('submit', async function(e){
          ...
          if (!client || !client.auth){
            setMessage('Authenticatieservice niet beschikbaar. Probeer het zo meteen opnieuw.', 'error');
            return;
          }
          ...
          try{
            ...
            console.error(err);
            setMessage('Inlogfout. Probeer het zo meteen opnieuw.', 'error');
          }
        });
      }
      ...
      if (registerForm){
        registerForm.addEventListener('submit', async function(e){
          ...
          if (!email || !password){
            setMessage('Voer e‑mail en wachtwoord in.', 'error');
            return;
          }
          if (password !== confirm){
            setMessage('Wachtwoorden komen niet overeen.', 'error');
            return;
          }
          ...
          try{
            ...
            if (result.error){
              ...
              if(msg.includes('already') && msg.includes('registered')){
                setMessage('Dit e‑mailadres is al geregistreerd. Probeer een ander e‑mailadres.', 'error');
              }else{
...
try {
  var mfaOk = await ensureMfaAal2IfNeeded(client, { force: true });
  if (!mfaOk){
    setMessage('Om je wachtwoord bij te werken is 2FA-verificatie vereist (Authenticator-code).', 'error');
    return;
  }
} catch (e) {
...
if (result.error){
  ...
  if (String(em).includes('AAL2') || String(em).toLowerCase().includes('aal2 session')){
    em = 'Om je wachtwoord te wijzigen met 2FA ingeschakeld, moet je ook de Authenticator (2FA)-code invullen. Probeer het opnieuw en voltooi de 2FA-verificatie.';
  }
  setMessage(em, 'error');
  return;
}
        ...
        elCopyBtn.addEventListener('click', function(){
          ...
          if (navigator.clipboard && navigator.clipboard.writeText){
            navigator.clipboard.writeText(elLinkInput.value)
              .then(function(){ setMessage('Link gekopieerd naar het klembord.', 'success'); })
              .catch(function(){ setMessage('Copy failed; select and copy manually.', 'error'); });
          }else{
            try{
              var ok = document.execCommand('copy');
              setMessage(ok ? 'Link gekopieerd naar het klembord.' : 'Copy failed; select and copy manually.', ok ? 'success' : 'error');
            }catch(e){
      ...
      if (payoutForm){
        payoutForm.addEventListener('submit', async function(e){
          ...
          if (!fiscal.legal_name || !fiscal.tax_residence_country || !fiscal.address_country || !fiscal.address_city){
            setMessage('Vul de vereiste fiscale gegevens in.', 'error');
            return;
          }

          if (method === 'paypal'){
            if (!payout.paypal_email){
              setMessage('Voer je PayPal e-mailadres in.', 'error');
              return;
            }
          } else {
    ...
    async function initDashboard(){
      ...
      if (!aff){
        setMessage('Affiliateprofiel kon niet worden opgehaald. Probeer het later opnieuw.', 'error');
        showView('login');
        return;
      }
