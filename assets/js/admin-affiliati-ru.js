  ...
  function init(){
    ...
    function ensureModal(){
      ...
      modalEl.innerHTML = [
        ...
        '    <button type="button" class="tva-modal__close" aria-label="Закрыть" id="tvaModalClose">×</button>',
        '  </div>',
...
function openModal(opts){
    ...
    async function promptAndVerifyTotpCode(client, factorId, promptTitle){
      var code = await promptTotpCodeModal({ title: (promptTitle || 'Проверка 2FA') });
      if (!code) return false;
      var ok = await verifyTotpCode(client, factorId, code);
      if (!ok) showToast('Неверный код 2FA.', 'error');
      return ok;
    }
    ...
    function promptTotpCodeModal(opts){
      ...
      return new Promise(function(resolve){
        openModal({
          title: opts.title || 'Проверка 2FA',
          bodyHtml: [
            '<p class="tva-modal__hint">Введите 6‑значный код из приложения Authenticator.</p>',
            '<div class="tva-modal__field">',
            '  <label class="tva-modal__label">Код 2FA</label>',
          ...
          actions: [
            { label: 'Отмена', className: 'btn secondary', onClick: function(){ closeModal(); resolve(null); } },
            { label: 'Подтвердить', className: 'btn', onClick: function(){
    ...
    async function ensureMfaAal2IfNeeded(client){
      try{
        ...
        if (cur === 'aal1' && next === 'aal2'){
          ...
          if (!factor || !factor.id){
            setMessage('2FA is enabled but no TOTP factor was found.', 'error');
            return false;
          }

          while (true){
            var code = await promptTotpCodeModal({ title: 'Проверка 2FA' });
            if (code === null){
            ...
            if (!/^\d{6}$/.test(code)){
              ...
              if (errEl) errEl.textContent = 'Введите корректный 6‑значный код.';
              continue;
            }
            ...
            if (client.auth.mfa.challengeAndVerify){
              ...
              if (!ok){
                ...
                if (e1) e1.textContent = (v1 && v1.error && v1.error.message) ? v1.error.message : 'Неверный код.';
              }
            }else{
              var ch = await client.auth.mfa.challenge({ factorId: factor.id });
              if (ch && ch.error){
                var e2 = document.getElementById('tvaMfaErr');
                if (e2) e2.textContent = ch.error.message || 'Не удалось создать проверку.';
                continue;
              }
              ...
              if (!ok){
                ...
                if (e3) e3.textContent = (v2 && v2.error && v2.error.message) ? v2.error.message : 'Неверный код.';
              }
            }
    ...
    async function open2faSettings(){
      ...
      if (!user){
        showToast('Чтобы управлять 2FA, нужно войти как администратор.', 'error');
        return;
      }
      ...
      if (verifiedFactor && verifiedFactor.id){
        openModal({
          title: 'Безопасность (2FA)',
          bodyHtml: '<p class="tva-modal__subtitle">2FA включена. Чтобы отключить, подтвердите кодом из вашего приложения.</p>',
          actions: [
            { label: 'Закрыть', className: 'btn btn-outline', onClick: closeModal },
            { label: 'Отключить 2FA', className: 'btn btn-danger', onClick: async function(){
                var ok = await promptAndVerifyTotpCode(client, verifiedFactor.id, 'Отключить 2FA');
                if (!ok) return;

                var un = await client.auth.mfa.unenroll({ factorId: verifiedFactor.id });
                if (un.error){
                  showToast(un.error.message || 'Ошибка при отключении 2FA', 'error');
                  return;
                }
                closeModal();
      ...
      openModal({
        title: 'Безопасность (2FA)',
        bodyHtml: '<p class="tva-modal__subtitle">Защитите доступ администратора вторым фактором (Authenticator).</p>',
        actions: [
          { label: 'Закрыть', className: 'btn btn-outline', onClick: closeModal },
          { label: 'Включить 2FA', className: 'btn btn-primary', onClick: async function(){

              // Ensure no verified factor already
              var existing = await getVerifiedTotpFactor(client);
              if (existing && existing.id){
                showToast('2FA уже включена.', 'success');
                open2faSettings();
                return;
              }
              ...
              if (en.error){
                showToast(en.error.message || 'Ошибка при включении 2FA', 'error');
                return;
              }
              ...
              openModal({
                ...
                bodyHtml:
                  '<div class="tva-modal__stack">'+
                    '<p class="tva-modal__subtitle">1) Отсканируйте QR‑код в Google Authenticator / Authy / Microsoft Authenticator.</p>'+
                    '<div class="mfa-qr" id="tvaMfaQrWrap"></div>'+
                    '<p class="tva-muted" id="tvaMfaSecretWrap" style="margin-top:10px; display:none;">Если не удаётся отсканировать: <code style="user-select:all; word-break:break-all;" id="tvaMfaSecret"></code></p>'+
                    '<p class="tva-modal__subtitle" style="margin-top:12px;">2) Введите сгенерированный код:</p>'+
                    '<div class="tva-modal__field">'+
                      '<label class="tva-modal__label">Код 2FA</label>'+
                  ...
                  '</div>',
                actions: [
                  { label: 'Отмена', className: 'btn btn-outline', onClick: cleanupEnroll },
                  { label: 'Подтвердить и включить', className: 'btn btn-primary', onClick: async function(){
                      ...
                      if (!code){
                        if (errEl){ errEl.style.display = 'block'; errEl.textContent = 'Введите код 2FA.'; }
                        return;
                      }

                      var ok = await verifyTotpCode(client, factorId, code);
                      if (!ok){
                        if (errEl){ errEl.style.display = 'block'; errEl.textContent = 'Неверный код. Пожалуйста, попробуйте ещё раз.'; }
                        return;
                      }
                ...
                try{
                  ...
                  if (qrWrap){
                    ...
                    if (qrUrl){
                      ...
                      p.textContent = 'QR‑код недоступен. Используйте ключ ниже.';
    ...
    function renderChart(monthly){
      ...
      state.chart = new Chart(ctx, {
        ...
        data: {
          labels: labels,
          datasets: [
            { label: 'Affiliate payouts', data: affValues, tension: 0.25, fill: false },
            { label: 'Travirae margin', data: travValues, tension: 0.25, fill: false },
          ]
        },
        options: {
          responsive: true,
  ...
  form.addEventListener('submit', async (e)=>{
    ...
    if(missing.length){
      const msg = 'Fill in all fields to continue.';
...
Totale: ${total} · Inviate: ${sent} · Errori: ${failed}${extra}`, failed ? 'warn' : 'success');
      ...
      setNewsletterStatus('Newsletter send error: ' + (err?.message || String(err)), 'error');
    }finally{
      if(sendBtn){ sendBtn.disabled = false; sendBtn.textContent = 'Send newsletter'; }
    }
  });
    ...
    const clearBtn = q('#admin-newsletter-clear-btn');
      if(clearBtn){
        clearBtn.addEventListener('click', () => {
          ...
          setNewsletterStatus('Drafts cleared.', 'success');
          updatePreview();
        });
      }
...
function clearTbody(tbody){
    ...
    function renderPayoutRequestsTable(rows){
      ...
      if (!rows || !rows.length){
        ...
        td0.colSpan = 7;
        td0.className = 'muted small';
        td0.textContent = 'Нет запросов на выплату.';
        tr0.appendChild(td0);
      ...
      rows.forEach(function(r){
        ...
        if (s === 'pending'){
          actionsCell.appendChild(makeAction('Одобрить', 'approved'));
          actionsCell.appendChild(makeAction('Отклонить', 'rejected'));
          actionsCell.appendChild(makeAction('Отметить как выплачено', 'paid'));
        } else if (s === 'approved'){
          actionsCell.appendChild(makeAction('Отметить как выплачено', 'paid'));
          actionsCell.appendChild(makeAction('Отклонить', 'rejected'));
        } else {
    ...
    async function updatePayoutRequestStatus(id, newStatus){
      ...
      var note = window.prompt('Примечание администратора (необязательно):', '') || '';
      ...
      setMessage('Updating request...', null);
      try{
        var resp = await client.from('payout_requests').update(patch).eq('id', id);
        if (resp.error){
          console.error(resp.error);
          setMessage('Error updating request: ' + (resp.error.message || 'unknown'), 'error');
          return;
        }
        setMessage('Request updated.', 'success');
        await loadAll();
      }catch(e){
        console.error(e);
        setMessage('Unexpected error while updating request.', 'error');
      }
    }
    ...
    async function runImportTravelpayouts(){
      ...
      setMessage('Import in progress (Aviasales)...', null);
      try{
        // days = 45 default
        var resp = await client.functions.invoke('import-travelpayouts', { body: { days: 45, campaign_id: 100 } });
        if (resp.error){
          console.error(resp.error);
          setMessage('Import error: ' + (resp.error.message || 'unknown'), 'error');
          return;
        }
        setMessage('Import completed. Updating dashboard...', 'success');
        await loadAll();
      }catch(e){
        console.error(e);
        setMessage('Unexpected error during import.', 'error');
      }
    }
    ...
    async function runRefreshPayouts(){
      ...
      setMessage('Refreshing payouts...', null);
      try{
        var resp = await client.rpc('refresh_monthly_affiliate_payouts');
        if (resp && resp.error){
          setMessage('Refresh error: ' + (resp.error.message || 'unknown'), 'error');
          return;
        }
        setMessage('Payouts refreshed.', 'success');
        await loadAll();
      }catch(e){
        console.error(e);
    ...
    async function importStay22CSV(file){
      ...
      if (!parsed.rows.length){
        setMessage('Empty or invalid CSV.', 'error');
        return;
      }
      ...
      if (!rows.length){
        setMessage('Unsupported CSV: no valid rows found. Make sure the CSV includes at least Campaign ID (or affiliate_slug) and Transaction ID (or partner_reference).', 'error');
        return;
      }
      setMessage('Importing Stay22 CSV: upsert in progress...', null);
      ...
      for (var i=0;i<rows.length;i+=chunkSize){
        ...
        if (up.error){
          ...
          setMessage('CSV import error: ' + (up.error.message || 'unknown'), 'error');
          return;
        }
      }
      ...
      setMessage('CSV imported. Dashboard updated.', 'success');
      await loadAll();
    }
    ...
    if (loginForm){
      loginForm.addEventListener('submit', async function(e){
        ...
        if (!client || !client.auth){
          setMessage('Supabase Auth not available.', 'error');
          return;
        }
        ...
        try{
          ...
          if (res.error){
            setMessage(res.error.message || 'Ошибка входа.', 'error');
            return;
          }
          ...
          if (!u || !isAdminEmail(u.email)){
            ...
            setMessage('This account is not an admin account.', 'error');
            return;
          }
          state.user = u;
