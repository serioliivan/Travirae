...
function openModal(opts){
    ...
    async function ensureMfaAal2IfNeeded(client, opts = {}){
      try{
        ...
        showToast('2FA vérifiée.', 'success');
        return true;
      }catch(e){
        console.warn(e);
        return true;
...
// Richiede una re‑autenticazione (password) e, se presente, anche un codice 2FA.
	    ...
	    async function promptPasswordThen2FA(client, opts){
	      ...
	      try{
	        ...
	        if (!email){
	          showToast('Session invalide. Reconnecte-toi.', 'error');
	          return false;
	        }
	            ...
	            var modalId = openModal({
	              ...
	              bodyHtml: [
	                '<div class="tva-modal__hint">',
	                  (opts.passwordHint || 'Pour continuer, saisis le mot de passe de ton compte.'),
	                '</div>',
	                '<input id="tvaReauthPassword" type="password" class="tva-modal__input" placeholder="Mot de passe" autocomplete="current-password" />',
	              ...
	              buttons: [
	                ...
	                },
	                {
	                  ...
	                  onClick: async function(id){
	                    ...
	                    pw = pw.trim();
	                    if (!pw){
	                      if (errEl){ errEl.textContent = 'Saisis le mot de passe.'; errEl.style.display = 'block'; }
	                      return;
	                    }
	                    // small loading lock
	                    ...
	                    try{
	                      ...
	                      console.warn(e);
	                      if (errEl){ errEl.textContent = 'Erreur de vérification. Réessaie.'; errEl.style.display = 'block'; }
	                      if (modal){
	        ...
	        console.warn(e);
	        showToast('Erreur de vérification. Réessaie.', 'error');
	        return false;
	      }
	    }
          ...
          buttons: [
            ...
            },
            {
              ...
              onClick: async function(){
                ...
                if (!/^\d{6}$/.test(code)){
                  if (errEl){ errEl.textContent = 'Saisis un code valide à 6 chiffres.'; errEl.style.display = 'block'; }
                  return;
                }
                ...
                try{
                  ...
                  if (errEl){ errEl.textContent = (err && err.message) ? err.message : 'Code invalide. Réessaie.'; errEl.style.display = 'block'; }
                }
              }
            }
          ]
    ...
    async function promptTotpCodeModal(promptTitle){
      promptTitle = promptTitle || 'Vérifier la 2FA';
      return new Promise(function(resolve){
        var done = false;
        function finish(val){
          if (done) return;
        ...
        openModal({
          title: promptTitle,
          bodyHtml: ''+
            '<p class="tva-modal__subtitle">Saisis le code à 6 chiffres généré par ton application Authenticator.</p>'+
            '<div class="tva-field">'+
              '<label class="tva-label">Code 2FA</label>'+
            ...
            '</div>',
          buttons: [
            { label: 'Annuler', className: 'btn btn-outline', onClick: function(){ closeModal(); finish(null); } },
            { label: 'Vérifier', className: 'btn btn-primary', onClick: function(){
                var codeEl = document.getElementById('tvaMfaPromptCode');
                var errEl = document.getElementById('tvaMfaPromptError');
                var code = (codeEl && codeEl.value ? codeEl.value : '').trim();
                if (errEl){ errEl.style.display = 'none'; errEl.textContent = ''; }
                if (!code){
                  if (errEl){ errEl.style.display = 'block'; errEl.textContent = 'Saisis le code 2FA.'; }
                  return;
                }
                closeModal();
                finish(code);
    ...
    async function open2faSettings(){
      ...
      if (!user){
        showToast('Tu dois être connecté pour gérer la 2FA.', 'error');
        return;
      }
      ...
      if (verifiedFactor && verifiedFactor.id){
        openModal({
          title: 'Sécurité (2FA)',
          bodyHtml: '<p class="tva-modal__subtitle">2FA activée. Pour la désactiver, confirme avec un code depuis ton appli.</p>',
          buttons: [
      ...
      openModal({
        title: 'Sécurité (2FA)',
        bodyHtml: '<p class="tva-modal__subtitle">Ajoute un second niveau de sécurité à ton compte (Authenticator).</p>',
        buttons: [
          { label: 'Fermer', className: 'btn btn-outline', onClick: closeModal },
          { label: 'Activer la 2FA', className: 'btn btn-primary', onClick: async function(){
              // Double-check
              ...
              openModal({
                ...
                bodyHtml:
                  '<div class="tva-modal__stack">'+
                    '<div style="margin:10px 0 14px; padding:12px; border:1px solid #fde68a; background:#fffbeb; border-radius:12px; font-size:13px; line-height:1.35; color:#111;"><strong>Important :</strong> si tu perds l’accès à ton application Authenticator, tu risques de ne plus pouvoir <strong>te connecter</strong> à ton compte et tu pourrais ne plus pouvoir <strong>réinitialiser ton mot de passe</strong> (« Mot de passe oublié ») tant que tu n’as pas retrouvé l’accès 2FA. Assure-toi de garder l’accès à l’appareil/l’appli et pense aussi à configurer la 2FA sur un second appareil.</div>'+
                    '<p class="tva-modal__subtitle">1) Scanne le QR code avec ton application Authenticator.</p>'+
                    '<div class="mfa-qr" id="tvaMfaQrWrap"></div>'+
                    '<p class="tva-muted" id="tvaMfaSecretWrap" style="margin-top:10px; display:none;">Si tu ne peux pas scanner : <code style="user-select:all; word-break:break-all;" id="tvaMfaSecret"></code></p>'+
                    '<p class="tva-modal__subtitle" style="margin-top:12px;">2) Saisis le code généré :</p>'+
                    '<div class="tva-field">'+
                      '<label class="tva-label">Code 2FA</label>'+
                  ...
                  '</div>',
                buttons: [
                  { label: 'Annuler', className: 'btn btn-outline', onClick: cleanupEnroll },
                  { label: 'Vérifier et activer', className: 'btn btn-primary', onClick: async function(){
                      ...
                      closeModal();
                      showToast('2FA activée ! Attention : si tu perds l’accès à ton application Authenticator, tu risques de ne plus pouvoir récupérer ton compte.', 'success');
                    } }
                ],
                ...
                try{
                  ...
                  if (qrWrap){
                    ...
                    if (qrUrl){
                      ...
                      p.textContent = 'QR indisponible. Utilise la clé ci-dessous.';
    ...
    function openChangeLangModal(){
      var client = refreshClient();
      if (!client){
        showToast('Supabase n’est pas prêt. Réessaie dans quelques secondes.', 'error');
        return;
      }
      ...
      client.auth.getUser().then(function(res){
        ...
        if (!user){
          showToast('Tu dois être authentifié pour changer de langue.', 'error');
          return;
        }
        ...
        openModal({
          ...
          bodyHtml:
            '<p class="tva-modal__subtitle">Choisis la langue dans laquelle tu veux recevoir les e-mails Travirae.</p>' +
            '<div class="tva-field">' +
              '<label class="tva-label" for="tvaLangSelect">Langue</label>' +
              '<select class="tva-input" id="tvaLangSelect">' + optionsHtml + '</select>' +
              '<div class="tva-error" id="tvaLangError" style="display:none;"></div>' +
            '</div>',
          buttons: [
            { label: 'Annuler', className: 'btn btn-outline', onClick: function(){ closeModal(); } },
            { label: 'Enregistrer', className: 'btn btn-primary', onClick: async function(){
              var selEl = document.getElementById('tvaLangSelect');
              var errEl = document.getElementById('tvaLangError');
              var next = selEl ? selEl.value : '';
              if (errEl){ errEl.style.display = 'none'; errEl.textContent = ''; }
              if (!next){
                if (errEl){ errEl.textContent = 'Sélectionne une langue pour continuer.'; errEl.style.display = 'block'; }
                return;
              }
              if (supported.indexOf(next) === -1){
                if (errEl){ errEl.textContent = 'Langue invalide.'; errEl.style.display = 'block'; }
                return;
              }
              ...
              try{
                ...
                console.warn(e);
                showToast((e && e.message) ? e.message : 'Erreur lors du changement de langue.', 'error');
              }
            } }
          ]
        });
    ...
    function bindAuthForms(){
      ...
      if (loginForm){
        loginForm.addEventListener('submit', async function(e){
          ...
          if (!client || !client.auth){
            setMessage('Service d’authentification indisponible. Réessaie bientôt.', 'error');
            return;
          }
          ...
          try{
            ...
            if (result.error){
              setMessage(result.error.message || 'Identifiants invalides.', 'error');
              return;
            }
            ...
            if (!state.user){
              setMessage('Connexion réussie, mais impossible de récupérer les données utilisateur.', 'error');
              return;
            }
            ...
            console.error(err);
            setMessage('Erreur de connexion. Réessaie bientôt.', 'error');
          }
        });
      }
      ...
      if (registerForm){
        registerForm.addEventListener('submit', async function(e){
          ...
          if (!email || !password){
            setMessage('Saisis l’e-mail et le mot de passe.', 'error');
            return;
          }
          if (password !== confirm){
          ...
          try{
            ...
            if (result.error){
              ...
              if(msg.includes('already') && msg.includes('registered')){
                setMessage('Cet e-mail est déjà utilisé. Essaie une autre adresse.', 'error');
              }else{
            ...
            setMessage('Inscription terminée ! Vérifie tes e-mails pour confirmer ton adresse avant de te connecter.', 'success');
            showView('login');
          }catch(err){
            console.error(err);
            setMessage('Erreur inattendue lors de l’inscription.', 'error');
          }
        });
      }
      ...
      if (forgotForm){
        forgotForm.addEventListener('submit', async function(e){
          ...
          if (!email){
            setMessage('Saisis ton e-mail.', 'error');
            return;
          }
          ...
          try{
            ...
            console.error(err);
            setMessage('Erreur inattendue lors de la réinitialisation du mot de passe.', 'error');
          }
        });
      }
      ...
      if (recoveryForm){
        recoveryForm.addEventListener('submit', async function(e){
          ...
          if (!password){
            setMessage('Saisis un nouveau mot de passe.', 'error');
            return;
          }
          if (password !== confirm){
            setMessage('Les mots de passe ne correspondent pas.', 'error');
...
try {
  var mfaOk = await ensureMfaAal2IfNeeded(client, { force: true });
  if (!mfaOk){
    setMessage('Pour mettre à jour ton mot de passe, une vérification 2FA est requise (code Authenticator).', 'error');
    return;
  }
} catch (e) {
...
if (result.error){
  ...
  if (String(em).includes('AAL2') || String(em).toLowerCase().includes('aal2 session')){
    em = 'Pour changer ton mot de passe avec la 2FA activée, tu dois aussi saisir le code Authenticator (2FA). Réessaie et termine la vérification 2FA.';
  }
  setMessage(em, 'error');
  return;
}

            setMessage('Mot de passe mis à jour ! Tu peux maintenant te connecter avec tes nouveaux identifiants.', 'success');
            showView('login');
          }catch(err){
            console.error(err);
            setMessage('Erreur inattendue lors de la mise à jour du mot de passe.', 'error');
          }
        });
      }
        ...
        elCopyBtn.addEventListener('click', function(){
          ...
          if (navigator.clipboard && navigator.clipboard.writeText){
            navigator.clipboard.writeText(elLinkInput.value)
              .then(function(){ setMessage('Lien copié dans le presse-papiers.', 'success'); })
              .catch(function(){ setMessage('Copie échouée ; sélectionne et copie manuellement.', 'error'); });
          }else{
            try{
              var ok = document.execCommand('copy');
              setMessage(ok ? 'Lien copié dans le presse-papiers.' : 'Copie échouée ; sélectionne et copie manuellement.', ok ? 'success' : 'error');
            }catch(e){
      ...
      if (btnOpenPayoutForm){
        btnOpenPayoutForm.addEventListener('click', async function(){
          ...
          if ((Number(state.availableBalance || 0) || 0) < 50){
            setMessage('Solde insuffisant : tu dois avoir au moins 50 USD disponibles pour demander un versement.', 'error');
            return;
          }
	          ...
	          var okAuth = await promptPasswordThen2FA(refreshClient(), {
	            title: 'Confirmer la demande de versement',
	            message: 'Pour continuer, saisis le mot de passe de ton compte. Si la 2FA est activée, on te demandera aussi le code.',
	            passwordTitle: 'Confirmer le mot de passe'
	          });
	          if (!okAuth){
	            return;
	          }
      ...
      if (payoutForm){
        payoutForm.addEventListener('submit', async function(e){
          ...
          if (!client || !client.functions){
            setMessage('Service de versement indisponible (Functions).', 'error');
            return;
          }
          ...
          if (amount > availableNow){
            setMessage('Solde insuffisant : le montant demandé dépasse ton solde disponible.', 'error');
            return;
          }

          if (amount < 50){
            setMessage('Erreur : le seuil minimum de versement est de 50 USD.', 'error');
            return;
          }

          if (confirmEl && !confirmEl.checked){
            setMessage('Tu dois confirmer que les informations sont correctes.', 'error');
            return;
          }
          ...
          if (!fiscal.legal_name || !fiscal.tax_residence_country || !fiscal.address_country || !fiscal.address_city){
            setMessage('Merci de compléter les informations fiscales obligatoires.', 'error');
            return;
          }

          if (method === 'paypal'){
            if (!payout.paypal_email){
              setMessage('Saisis ton e-mail PayPal.', 'error');
              return;
            }
          } else {
            if (!payout.beneficiary_name || !payout.bank_country){
              setMessage('Saisis le titulaire du compte et le pays de la banque.', 'error');
              return;
            }
            // IBAN è richiesto per SEPA; per SWIFT puro potrebbe essere diverso, ma per semplicità lo chiediamo.
            if (!payout.iban){
              setMessage('Saisis un IBAN (pour les virements SEPA).', 'error');
              return;
            }
          }
          ...
          try{
            ...
            setMessage('✅ Demande envoyée ! Tu la verras dans ton historique et l’admin recevra aussi un e-mail.', 'success');
            if (payoutFormWrap) payoutFormWrap.hidden = true;
            ...
            setMessage('❌ Erreur de demande de versement : ' + msg, 'error');
          }finally{
    ...
    async function initDashboard(){
      ...
      if (!aff){
        setMessage('Impossible de récupérer le profil affilié. Réessaie plus tard.', 'error');
        showView('login');
        return;
      }
