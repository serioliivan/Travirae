  ...
  function init(){
    ...
    function ensureModal(){
      ...
      modalEl.innerHTML = [
        ...
        '    <button type="button" class="tva-modal__close" aria-label="Close" id="tvaModalClose">×</button>',
        '  </div>',
...
function openModal(opts){
    ...
    async function ensureMfaAal2IfNeeded(client){
      try{
        ...
        if (cur === 'aal1' && next === 'aal2'){
          ...
          while (true){
            var code = await promptTotpCodeModal({ title: 'Verify 2FA' });
            if (code === null){
    ...
    async function open2faSettings(){
      ...
      if (!user){
        showToast('You must be logged in as admin to manage 2FA.', 'error');
        return;
      }
