      ...
      if (toastEl){
        ...
        toastEl.innerHTML = ''+
          '<div class="tva-toast__inner" role="alert" aria-live="polite">'+
            ...
            '<button type="button" class="tva-toast__close" aria-label="关闭">×</button>'+
          '</div>';
      ...
      modalEl.innerHTML = ''+
        '<div class="tva-modal__card" role="dialog" aria-modal="true">'+
          '<div class="tva-modal__header">'+
            ...
            '<button type="button" class="tva-modal__close" aria-label="关闭">×</button>'+
          '</div>'+
...
// Richiede una re‑autenticazione (password) e, se presente, anche un codice 2FA.
	        ...
	        var password = await (function(){
	          return new Promise(function(resolve){
	            var modalId = openModal({
	              title: opts.passwordTitle || '确认密码',
	              bodyHtml: [
	                '<div class="tva-modal__hint">',
	                  (opts.passwordHint || '继续操作前请输入账户密码。'),
	                '</div>',
	                '<input id="tvaReauthPassword" type="password" class="tva-modal__input" placeholder="密码" autocomplete="current-password" />',
	                ...
	                },
	                {
	                  ...
	                  onClick: async function(id){
	                    ...
	                    try{
	                      ...
	                      if (si && si.error){
	                        if (errEl){ errEl.textContent = '密码不正确。'; errEl.style.display = 'block'; }
	                        if (modal){
	                      ...
	                      console.warn(e);
	                      if (errEl){ errEl.textContent = '验证出错。请重试。'; errEl.style.display = 'block'; }
	                      if (modal){
    ...
    async function promptAndVerifyTotpCode(client, factorId, promptTitle){
      var code = await promptTotpCodeModal(promptTitle || '验证 2FA');
    ...
    function promptTotpVerification(factorId, title, subtitle){
      return new Promise(function(resolve){
        var body = ''+
          ...
          '<label class="tva-field">2FA 验证码</label>'+
            ...
            },
            {
              ...
              onClick: async function(){
                ...
                try{
                  ...
                  if (errEl){ errEl.textContent = (err && err.message) ? err.message : '验证码无效。请重试。'; errEl.style.display = 'block'; }
                }
              }
            }
          ]
    ...
    async function promptTotpCodeModal(promptTitle){
      promptTitle = promptTitle || '验证 2FA';
      return new Promise(function(resolve){
        var done = false;
        function finish(val){
          if (done) return;
        ...
        openModal({
          title: promptTitle,
          bodyHtml: ''+
            '<p class="tva-modal__subtitle">请输入验证器应用生成的 6 位验证码。</p>'+
            '<div class="tva-field">'+
              '<label class="tva-label">2FA 验证码</label>'+
            ...
            '</div>',
          buttons: [
            { label: '取消', className: 'btn btn-outline', onClick: function(){ closeModal(); finish(null); } },
            { label: '验证', className: 'btn btn-primary', onClick: function(){
    ...
    async function open2faSettings(){
      ...
      if (!user){
        showToast('你需要登录后才能管理 2FA。', 'error');
        return;
      }
              ...
              openModal({
                ...
                bodyHtml:
                  '<div class="tva-modal__stack">'+
                    '<div style="margin:10px 0 14px; padding:12px; border:1px solid #fde68a; background:#fffbeb; border-radius:12px; font-size:13px; line-height:1.35; color:#111;"><strong>重要提示：</strong>如果你失去对验证器应用的访问权限，你可能将无法<strong>登录</strong>，并且在恢复 2FA 之前可能无法<strong>重置密码</strong>（“忘记密码”）。请确保你能可靠地访问该设备/应用，并考虑配置备用访问方式。</div>'+
                    '<p class="tva-modal__subtitle">1）使用验证器应用扫描二维码。</p>'+
    ...
    function openChangeLangModal(){
      var client = refreshClient();
      if (!client){
        showToast('Supabase 尚未就绪。请几秒后再试。', 'error');
        return;
      }
      ...
      client.auth.getUser().then(function(res){
        ...
        openModal({
          title: '更改语言',
          bodyHtml:
            '<p class="tva-modal__subtitle">选择你希望接收 Travirae 邮件的语言。</p>' +
            '<div class="tva-field">' +
              '<label class="tva-label" for="tvaLangSelect">语言</label>' +
              '<select class="tva-input" id="tvaLangSelect">' + optionsHtml + '</select>' +
              '<div class="tva-error" id="tvaLangError" style="display:none;"></div>' +
            '</div>',
          buttons: [
            { label: '取消', className: 'btn btn-outline', onClick: function(){ closeModal(); } },
            { label: '保存', className: 'btn btn-primary', onClick: async function(){
              var selEl = document.getElementById('tvaLangSelect');
              var errEl = document.getElementById('tvaLangError');
              var next = selEl ? selEl.value : '';
              if (errEl){ errEl.style.display = 'none'; errEl.textContent = ''; }
              if (!next){
                if (errEl){ errEl.textContent = '请选择一种语言以继续。'; errEl.style.display = 'block'; }
                return;
              }
              if (supported.indexOf(next) === -1){
                if (errEl){ errEl.textContent = '语言无效。'; errEl.style.display = 'block'; }
                return;
              }
              ...
              try{
                ...
                closeModal();
                showToast('语言更新成功。', 'success');
              }catch(e){
                console.warn(e);
                showToast((e && e.message) ? e.message : '更改语言时出错。', 'error');
              }
            } }
          ]
        });
      ...
      if (loginForm){
        loginForm.addEventListener('submit', async function(e){
          ...
          if (!client || !client.auth){
            setMessage('认证服务不可用。请稍后再试。', 'error');
            return;
          }
          ...
          try{
            ...
            if (result.error){
              setMessage(result.error.message || '账号或密码不正确。', 'error');
              return;
            }
            ...
            if (!state.user){
              setMessage('登录成功，但无法获取用户数据。', 'error');
              return;
            }
          ...
          try{
            ...
            if (result.error){
              ...
              if(msg.includes('already') && msg.includes('registered')){
                setMessage('该邮箱已注册。请尝试其他邮箱。', 'error');
              }else{
                setMessage(result.error.message || '注册出错。', 'error');
              }
              return;
            }
            ...
            setMessage('注册完成！请先查看邮箱并确认地址，然后再登录。', 'success');
            showView('login');
          }catch(err){
            console.error(err);
            setMessage('注册过程中发生意外错误。', 'error');
          }
        });
      }
      ...
      if (forgotForm){
        forgotForm.addEventListener('submit', async function(e){
          ...
          try{
            ...
            if (result.error){
              setMessage(result.error.message || '重置密码时出错。', 'error');
              return;
            }

            setMessage('如果该邮箱存在于系统中，你将收到一封包含重置链接的邮件。', 'success');
            showView('login');
          }catch(err){
            console.error(err);
            setMessage('重置密码时发生意外错误。', 'error');
          }
        });
      }
...
try {
  var mfaOk = await ensureMfaAal2IfNeeded(client, { force: true });
  if (!mfaOk){
    setMessage('更新密码需要 2FA 验证（验证器代码）。', 'error');
    return;
  }
} catch (e) {
...
if (result.error){
  var em = result.error.message || '更新密码时出错。';
  if (String(em).includes('AAL2') || String(em).toLowerCase().includes('aal2 session')){
    em = '启用 2FA 后，修改密码时还需要输入验证器（2FA）验证码。请重试并完成 2FA 验证。';
  }
  setMessage(em, 'error');
  return;
}

            setMessage('密码更新成功！你现在可以使用新凭据登录。', 'success');
            showView('login');
          }catch(err){
            console.error(err);
            setMessage('更新密码时发生意外错误。', 'error');
          }
        });
      }
        ...
        elCopyBtn.addEventListener('click', function(){
          ...
          if (navigator.clipboard && navigator.clipboard.writeText){
            navigator.clipboard.writeText(elLinkInput.value)
              .then(function(){ setMessage('链接已复制到剪贴板。', 'success'); })
              .catch(function(){ setMessage('复制失败；请手动选择并复制。', 'error'); });
          }else{
            try{
              var ok = document.execCommand('copy');
              setMessage(ok ? '链接已复制到剪贴板。' : '复制失败；请手动选择并复制。', ok ? 'success' : 'error');
            }catch(e){
              setMessage('请选择链接并手动复制。', 'error');
            }
          }
        });
      }
    ...
    function setTierUI(tier){
      ...
      if (elTierNext){
        if (tier.nextAt == null){
          elTierNext.textContent = '你已达到最高档位（60%）。';
        } else {
          elTierNext.textContent = '下一档位：' + (tier.nextPercent * 100).toFixed(0) + '% a ' + tier.nextAt + ' 笔销售（还差 ' + tier.missing + '）。';
        }
      }
    }
    ...
    function renderPayoutRequestsTable(rows){
      ...
      rows.forEach(function(r){
        ...
        var date = '';
        try{ date = new Date(r.requested_at).toLocaleString('zh-CN'); }catch(e){ date = String(r.requested_at || ''); }

        var status = String(r.status || 'pending');
        var statusLabel = status;
        if (status === 'pending') statusLabel = '待处理';
        if (status === 'approved') statusLabel = '已批准';
        if (status === 'paid') statusLabel = '已支付';
        if (status === 'rejected') statusLabel = '已拒绝';
      ...
      if (btnOpenPayoutForm){
        btnOpenPayoutForm.addEventListener('click', async function(){
          ...
          if ((Number(state.availableBalance || 0) || 0) < 50){
            setMessage('余额不足：可提现余额至少需要 50 USD 才能申请提现。', 'error');
            return;
          }
	          ...
	          var okAuth = await promptPasswordThen2FA(refreshClient(), {
	            title: '确认提现申请',
	            message: '继续操作前请输入账户密码。如果已启用 2FA，还会要求输入验证码。',
	            passwordTitle: '确认密码'
	          });
	          if (!okAuth){
	            return;
      ...
      if (payoutForm){
        payoutForm.addEventListener('submit', async function(e){
          ...
          if (!client || !client.functions){
            setMessage('提现服务不可用（Functions）。', 'error');
            return;
          }
          ...
          if (amount > availableNow){
            setMessage('余额不足：申请金额超过可提现余额。', 'error');
            return;
          }

          if (amount < 50){
            setMessage('错误：最低提现门槛为 50 USD。', 'error');
            return;
          }

          if (confirmEl && !confirmEl.checked){
            setMessage('你需要确认信息无误。', 'error');
            return;
          }
          ...
          try{
            ...
            setMessage('✅ 申请已发送！你可以在历史记录中查看，管理员也会收到邮件通知。', 'success');
            if (payoutFormWrap) payoutFormWrap.hidden = true;
    ...
    async function loadDashboardStats(){
      ...
      try{
        ...
        if (btnOpenPayoutForm){
          ...
          btnOpenPayoutForm.title = (available < 50) ? '余额不足：申请提现至少需要 50 USD。' : '';
        }
    ...
    async function initDashboard(){
      ...
      if (!aff){
        setMessage('无法获取联盟资料。请稍后再试。', 'error');
        showView('login');
        return;
      }
