...
(function(){
  ...
  // === Local background images for category bars (no external links) ===
  function slug(s){
  ...
  function applyTo(el){
    ...
    const src = 'assets/img/subcats/' + slug(label) + '.jpg';
    const test = new Image();
    test.onload = () => { el.style.backgroundImage = 'url(\"' + src + '\")'; };
    test.onerror = () => { el.style.backgroundImage = 'url(assets/img/common/hero.jpg)'; };
    test.src = src;
  }
  function init(){
