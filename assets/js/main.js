...
/*! Travirae affiliate tracking v1 */
(function(){
  ...
  function getAffiliateFromStorage(){
    var val = '';
    try{
      if(typeof localStorage !== 'undefined'){
        val = localStorage.getItem('tva_aff') || '';
      }
    }catch(e){}
    if(!val){
      try{
    ...
    return normalizeAffiliateId(val);
  }
  function setAffiliateId(id){
    var normalized = normalizeAffiliateId(id);
    if(!normalized) return;
    try{
      var expires = new Date();
      expires.setTime(expires.getTime() + 30*24*60*60*1000);
      document.cookie = 'tva_aff='+encodeURIComponent(normalized)+'; expires='+expires.toUTCString()+'; path=/';
    }catch(e){}
    try{
      if(typeof localStorage !== 'undefined'){
        localStorage.setItem('tva_aff', normalized);
      }
    }catch(e2){}
  }
  ...
  try{
    var fromQuery = getQueryParam('ref');
    if(fromQuery){
      currentAffiliate = normalizeAffiliateId(fromQuery);
      if(currentAffiliate){
        setAffiliateId(currentAffiliate);
      }
    } else {
      currentAffiliate = getAffiliateFromStorage();
    }
  }catch(e){
    currentAffiliate = getAffiliateFromStorage();
  }
  ...
  window.traviraeAffiliate = {
    getId: function(){
      return getAffiliateFromStorage();
    }
  };

  try{
    var affIdForClick = currentAffiliate || (window.traviraeAffiliate && typeof window.traviraeAffiliate.getId === 'function'
      ? window.traviraeAffiliate.getId()
      : '');
    if(affIdForClick && window.supabaseClient && typeof window.supabaseClient.from === 'function'){
      window.supabaseClient
        .from('affiliate_clicks')
        .insert({
          affiliate_slug: affIdForClick,
          page: (window.location.pathname || '') + (window.location.search || '')
        })
        .then(function(result){
          if(result && result.error){
            console.warn('Errore salvataggio click affiliato', result.error);
          }
        })
        .catch(function(err){
          console.warn('Errore rete Supabase affiliate_clicks', err);
        });
    }
  }catch(eClick){
    console.warn('Impossibile tracciare il click affiliato', eClick);
  }
})();
