...
(function(){
  ...
  function getSiteLang(){
    ...
    if (storedSite) return normalizeLang(storedSite);

    const storedLegacy = safeGetLS(LANG_STORAGE_KEY);
    if (storedLegacy) {
      const normalizedLegacy = normalizeLang(storedLegacy);
      safeSetLS(SITE_LANG_STORAGE_KEY, normalizedLegacy);
      return normalizedLegacy;
    }

    const detected = normalizeLang(detectSiteLangFromBrowser());
    safeSetLS(SITE_LANG_STORAGE_KEY, detected);
    // Backward compatibility: keep the legacy/email key aligned on first detection.
...
(function(){
  ...
  window.traviraeAffiliate.trackBookingClick = function(input){
    try{
      var options = (input && typeof input === 'object' && !Array.isArray(input)) ? input : { partner: input };
      var partner = String(options.partner || input || 'unknown').trim() || 'unknown';

      // Affiliazione generale del sito (legacy) + override puntuale per widget influencer.
      var legacyId = '';
      try { legacyId = (window.traviraeAffiliate.getId && window.traviraeAffiliate.getId()) ? String(window.traviraeAffiliate.getId()).trim() : ''; } catch(eId) { legacyId = ''; }
      var affiliateOverride = String(options.affiliateSlugOverride || options.affiliate_slug || '').trim();
      var affiliateSlug = affiliateOverride || legacyId || 'direct';

      var page = (window.location && window.location.pathname) ? window.location.pathname : '/';
      var widgetTrackingCode = String(options.widgetTrackingCode || options.widget_tracking_code || '').trim();
      var widgetId = String(options.widgetId || options.widget_id || '').trim();
      var postId = options.postId || options.post_id || null;
      var postSlug = String(options.postSlug || options.post_slug || '').trim() || null;
      var source = String(options.source || '').trim() || 'site_general';

      // Anti-doppio conteggio: deduplica più precisa per partner + pagina + widget.
      try {
        if (window.sessionStorage) {
          var guardKey = 'tva_booking_guard_' + safeKeyPart(affiliateSlug) + '_' + safeKeyPart(partner) + '_' + safeKeyPart(page) + '_' + safeKeyPart(widgetTrackingCode || widgetId || postSlug || 'site');
          var now = Date.now();
          var last = parseInt(sessionStorage.getItem(guardKey) || '0', 10);
          if (last && (now - last) < 1200) {
            if (window.console && console.log) console.log('Travirae: booking click duplicato ignorato', { partner: partner, page: page, widgetTrackingCode: widgetTrackingCode || null });
            return;
          }
      ...
      var extraMeta = (options.metadata && typeof options.metadata === 'object') ? options.metadata : {};
      var metadata = Object.assign({
        ...
        source: source,
        creator_slug: affiliateOverride || extraMeta.creator_slug || null,
        post_id: postId || extraMeta.post_id || null,
        post_slug: postSlug || extraMeta.post_slug || null,
        widget_id: widgetId || extraMeta.widget_id || null,
        widget_tracking_code: widgetTrackingCode || extraMeta.widget_tracking_code || null,
        tracking_mode: affiliateOverride ? 'widget_override' : 'site_ref',
        site_affiliate_slug: legacyId || null
      }, extraMeta || {});
