...
(function(){
  ...
  if(flightsForm){
    flightsForm.addEventListener('submit', (e)=>{
      ...
      const isDE = lang === 'DEU';
      const parts = [];
      if(from && to) parts.push(isDE ? `Flug von ${from} nach ${to}` : `volo da ${from} a ${to}`);
      if(d1) parts.push(isDE ? `Abflug ${d1}` : `partenza ${d1}`);
      if(d2) parts.push(isDE ? `Rückflug ${d2}` : `ritorno ${d2}`);
      parts.push(isDE ? `${adults} Erwachsene ${cabin}` : `${adults} adulti ${cabin}`);
      const url = 'https://www.google.com/travel/flights?q=' + encodeURIComponent(parts.join(' ')) + (isDE ? '&hl=de' : '&hl=it');
...
(function(){
  ...
  function showToast(message, type = 'success') {
    try {
      ...
      if (type === 'error') title.textContent = isDE ? 'Fehler' : 'Errore';
      else if (type === 'info') title.textContent = 'Info';
      else title.textContent = isDE ? 'Erfolgreich' : 'Operazione completata';
      ...
      closeBtn.setAttribute('aria-label', isDE ? 'Schließen' : 'Chiudi');
  ...
  // --- Language router (Approach A: *-de.html pages) ---
    ...
    try{
      ...
      if (code === 'DEU') {
        if (!isDePage && /\.html$/i.test(file)) {
          const targetFile = file.replace(/\.html$/i, '-de.html');
  ...
  function initNewsletterSignup(){
    ...
    form.addEventListener('submit', async (e) => {
      ...
      if (!email || !/^\S+@\S+\.\S+$/.test(email)){
        showToast((getSiteLang() === 'DEU') ? 'Bitte gib eine gültige E‑Mail-Adresse ein.' : 'Inserisci un\'email valida.', 'error');
        return;
      }
      ...
      try{
        ...
        showToast((lang === 'DEU') ? 'Abo bestätigt! Danke.' : 'Iscrizione completata! Grazie.', 'success');
      } catch (err){
        showToast((lang === 'DEU') ? 'Fehler bei der Anmeldung. Bitte versuche es später erneut.' : 'Errore durante iscrizione. Riprova tra poco.', 'error');
...
(function(){
  ...
  document.addEventListener('click', (ev)=>{
    ...
    const fallbackCity = isDE ? 'Das Reiseziel' : 'La destinazione';
