...
(function(){
  function getQueryRef(){
    try{
      ...
      if(!ref) return '';
      ref = String(ref).trim();
      // consenti solo caratteri sicuri per slug
  ...
  function readAffiliate(){
    ...
    try{
      if('localStorage' in window){
        v = window.localStorage.getItem('tva_aff') || '';
      }
    }catch(e){
      v = '';
    }
  ...
  if(ref){
    storeAffiliate(ref);
    // traccia click in Supabase se disponibile
    try{
      if(window.supabaseClient && typeof window.supabaseClient.from === 'function'){
        ...
        window.supabaseClient
          ...
          .insert(payload)
          .then(function(res){ /* opzionale: logging */ })
          .catch(function(err){
            if(window.console && console.warn){
              console.warn('Errore tracking affiliato', err);
            }
          });
      }
    }catch(e){
      if(window.console && console.warn){
        console.warn('Errore tracking affiliato', e);
      }
    }
  }
