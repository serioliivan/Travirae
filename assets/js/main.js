// Travirae front-end V6 — robust proxy negotiation + tools
...
window.__TRAVIRAE__ = { setBase: (b,style='api')=>{ CFG.API_BASE_EFFECTIVE=b; CFG.API_STYLE=style; notify('Base impostata manualmente: '+b+' ('+style+')'); } };
...
async function tryPing(base, style, endpoint){
  ...
  try{
    ...
    const text = await r.text(); let ok = (r.status>=200 && r.status<300);
    try{ const js = JSON.parse(text); if(js && (js.ok===true || js.ok==='true' || js.status==='ok')) ok = true; }catch{}
    return {ok,url,status:r.status,style,body:text};
  }catch(e){ return {ok:false,url,style,error:String(e)}; }
}
async function resolveApi(base){
  const attempts=[];
  for(const b of variants(base)){
    for(const style of ['api','direct']){
      const a1 = await tryPing(b, style, style==='api'?'/api/ping':'/ping'); attempts.push(a1);
      if(a1.ok) return {effective:b,style,attempts};
      const a2 = await tryPing(b, style, style==='api'?'/ping':'/api/ping'); attempts.push(a2);
      if(a2.ok) return {effective:b,style,attempts};
    }
  }
  console.warn('Proxy negotiation failed', attempts);
  window.__TRAVIRAE_ATTEMPTS__ = attempts;
  return null;
}
...
function attachFlightForm(formSel, resultsSel){
  ...
  form.addEventListener('submit', async e=>{
    ...
    try{
      const r=await fetch(url,{cache:'no-store'}); const text=await r.text(); let js={}; try{ js=JSON.parse(text); }catch{ throw new Error('Bad JSON ('+text.slice(0,80)+'...)'); }
...
async function initStays(){
  try{
    ...
    const aid=partners.STAY22_AID||'travirae'; const campaign=partners.STAY22_CAMPAIGN_DEFAULT||'homepage';
    const iframe=$('#stay22-widget'), form=$('#stays-form');
    form.addEventListener('submit', async e=>{
      e.preventDefault();
      const city=$('#stay-city').value.trim(); const ci=$('#stay-in').value; const co=$('#stay-out').value;
      let lat='41.9028', lng='12.4964';
      if(city){ try{ const u=new URL('https://nominatim.openstreetmap.org/search'); u.searchParams.set('q',city); u.searchParams.set('format','json'); u.searchParams.set('limit','1'); const r=await fetch(u); const js=await r.json(); if(js&&js[0]){ lat=js[0].lat; lng=js[0].lon; } }catch{} }
...
async function initTransport(){
  ...
  const partners=await (await fetch('config/partners.json')).json().catch(()=>({}));
  const marker=partners.TP_SHMARKER || '669407'; const promo=partners.TP_PROMO_ID_12GO || 'XXXXX'; const clickDomain=partners.TRAVELPAYOUTS_CLICK_DOMAIN || 'c1.travelpayouts.com';
...
async function initDeals(){
  try{
    const js=await (await fetch('data/offers.json',{cache:'no-store'})).json();
    const list=$('#deals-list'); const btn=$('#more-deals'); let i=0; const step=6;
    const render=()=>{ const slice=js.slice(i,i+step); i+=slice.length; slice.forEach(x=>{ const el=document.createElement('article'); el.className='card-deal'; el.innerHTML=`<img loading="lazy" src="${escapeAttr(x.img)}" alt="${escapeAttr(x.title)}"><div class="route">${escapeHtml(x.title)}</div><div class="meta">${escapeHtml(x.subtitle||'')}</div><div><span class="badge">${escapeHtml(x.price||'')}</span></div><a class="btn btn-primary" href="${escapeAttr(x.link)}" target="_blank" rel="noopener">Apri offerta</a>`; list.appendChild(el); }); if(i>=js.length) btn.style.display='none'; };
