// Travirae front-end V5 — Flights on-site + stays + transport helpers (vanilla JS)
...
function variants(base){
  ...
  const out=[b,b.replace(/\/api\/proxy$/,''),b.replace(/\/\.netlify\/functions\/proxy$/,''),b.replace(/\/api$/,'')];
  return out.filter((v,i,a)=>v && a.indexOf(v)===i);
}
function join(base, path, style='api'){
  base=(base||'').replace(/\/+$/,'');
  if(style==='direct'){ if(/\/api$/.test(base)) return base + path.replace(/^\/api/,''); return base + path.replace(/^\/api/,''); }
  if(/\/api$/.test(base)) return base + path.replace(/^\/api/,''); return base + path;
}
async function resolveApi(base){
  const tried=new Set();
  for(const b of variants(base)){
    for(const style of ['api','direct']){
      const u1=join(b,'/api/ping',style);
      if(!tried.has(u1)){ tried.add(u1);
        try{ const r=await fetch(u1); if(r.ok){ const js=await r.json().catch(()=>({})); if(js && js.ok) return {effective:b,style}; } }catch{}
      }
      const u2=join(b,'/ping',style);
      if(!tried.has(u2)){ tried.add(u2);
        try{ const r=await fetch(u2); if(r.ok){ const js=await r.json().catch(()=>({})); if(js && js.ok) return {effective:b,style}; } }catch{}
      }
    }
  }
  return null;
...
// ----------- Flights (form + autocomplete + results) -----------
...
// ----------- Stays (Stay22 simple) -----------
async function initStays(){
  try{
    const partners=await (await fetch('config/partners.json')).json().catch(()=>({}));
    const aid=partners.STAY22_AID||'travirae';
    const campaign=partners.STAY22_CAMPAIGN_DEFAULT||'homepage';
    const iframe=$('#stay22-widget');
    const form=$('#stays-form');
    form.addEventListener('submit', async e=>{
      e.preventDefault();
      const city=$('#stay-city').value.trim();
      const checkin=$('#stay-in').value; const checkout=$('#stay-out').value;
      let lat='41.9028', lng='12.4964'; // default Rome
      if(city){
        try{
          const u=new URL('https://nominatim.openstreetmap.org/search');
          u.searchParams.set('q', city); u.searchParams.set('format','json'); u.searchParams.set('limit','1');
          const r=await fetch(u.toString(),{headers:{'Accept':'application/json'}}); const js=await r.json(); if(js && js[0]){ lat=js[0].lat; lng=js[0].lon; }
        }catch{}
      }
      iframe.src=`https://www.stay22.com/embed/gm?aid=${encodeURIComponent(aid)}&lat=${encodeURIComponent(lat)}&lng=${encodeURIComponent(lng)}&checkin=${encodeURIComponent(checkin)}&checkout=${encodeURIComponent(checkout)}&campaign=${encodeURIComponent(campaign)}`;
    });
  }catch{}
}

// ----------- Transport (12Go deeplink via TP click) -----------
async function initTransport(){
  const form=$('#transport-form'); if(!form) return;
  const partners=await (await fetch('config/partners.json')).json().catch(()=>({}));
  const marker=partners.TP_SHMARKER || '669407';
  const promo=partners.TP_PROMO_ID_12GO || 'XXXXX';
  const clickDomain=partners.TRAVELPAYOUTS_CLICK_DOMAIN || 'c1.travelpayouts.com';
  form.addEventListener('submit', e=>{
    e.preventDefault();
    const o=$('#t-origin').value.trim(); const d=$('#t-dest').value.trim(); const date=$('#t-date').value;
    const trip=`https://12go.asia/en/travel/${encodeURIComponent(o)}/${encodeURIComponent(d)}?date=${encodeURIComponent(date)}`;
    const deep=`https://${clickDomain}/click?shmarker=${encodeURIComponent(marker)}&promo_id=${encodeURIComponent(promo)}&type=click&source_type=customlink&custom_url=${encodeURIComponent(trip)}`;
    window.open(deep, '_blank','noopener');
  });
}

// ----------- Deals (load from /data/offers.json) -----------
async function initDeals(){
  try{
    const js=await (await fetch('data/offers.json')).json();
    const list=$('#deals-list'); const btn=$('#more-deals');
    let i=0; const step=6;
    const render=()=>{
      const slice=js.slice(i,i+step); i+=slice.length;
      slice.forEach(x=>{
        const el=document.createElement('article'); el.className='card-deal';
        el.innerHTML=`<img loading="lazy" src="${escapeAttr(x.img)}" alt="${escapeAttr(x.title)}"><div class="route">${escapeHtml(x.title)}</div><div class="meta">${escapeHtml(x.subtitle||'')}</div><div><span class="badge">${escapeHtml(x.price||'')}</span></div><a class="btn btn-primary" href="${escapeAttr(x.link)}" target="_blank" rel="noopener">Apri offerta</a>`;
        list.appendChild(el);
      });
      if(i>=js.length) btn.style.display='none';
    };
