...
(function(){
  ...
  // --- Third-party widgets language helpers (Stay22, Aviasales, ...)
  // Widgets typically accept a 2-letter locale (it, en, fr, ...).
  // We derive it from the LANGS table to keep everything consistent.
  function mapSiteLangToWidgetLocale(code){
    const normalized = normalizeLang(code);
    const found = LANGS.find(l => l.code === normalized);
    return (found?.htmlLang || 'en').toLowerCase();
  }

  function setUrlParamSafe(url, key, value){
    try {
  ...
  function updateStay22EmbedsLanguage(code){
    const locale = mapSiteLangToWidgetLocale(code);
    const iframes = document.querySelectorAll('iframe[src*="stay22.com/embed"]');
    if (!iframes || iframes.length === 0) return;

    iframes.forEach((iframe) => {
      const src = iframe.getAttribute('src') || '';
      if (!src) return;
      // Try a few common param names used by embeds.
      let next = src;
      next = setUrlParamSafe(next, 'lang', locale);
      next = setUrlParamSafe(next, 'language', locale);
      next = setUrlParamSafe(next, 'locale', locale);
      if (next !== src) iframe.setAttribute('src', next);
    });
  }

  function broadcastLangChanged(code){
    const normalized = normalizeLang(code);
    const locale = mapSiteLangToWidgetLocale(normalized);
    try {
      window.dispatchEvent(new CustomEvent('travirae:langChanged', { detail: { code: normalized, locale } }));
    } catch {
      // Ignore
    }
  }
  ...
  function setSiteLang(code, { persist = true, syncSelect = true, syncUser = false } = {}){
    ...
    // Update third‑party widgets that depend on locale
    updateStay22IframesLang(normalized);
    // Let page-specific scripts (e.g. Aviasales) refresh themselves
    broadcastLangChanged(normalized);
  }
