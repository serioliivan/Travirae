...
(function(){
  ...
  document.querySelectorAll('.accordion-toggle.category').forEach(btn=>{
    ...
    if(explicit){ tryURL(explicit, ()=>tryURL(local, ()=>tryURL(unsplash, ()=>setBG('linear-gradient(120deg, #dbe4ff, #e9ddff)')))); }
    else { tryURL(local, ()=>tryURL(unsplash, ()=>setBG('linear-gradient(120deg, #dbe4ff, #e9ddff)'))); }
  });
}
