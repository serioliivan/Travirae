...
function sanitizePopupTitles(root=document){
  const selectors = [
    '.modal .modal-title','.modal [class*="title"]',
    '[role="dialog"] h1','[role="dialog"] h2','[role="dialog"] h3',
    '.popup h1','.popup h2','.popup h3','.dialog-title','.popup-title','.popover-title'
  ];
  selectors.forEach(sel => {
    document.querySelectorAll(sel).forEach(el => { el.textContent = stripEmojis(el.textContent||''); });
  });
}
document.addEventListener('DOMContentLoaded', ()=>{
  sanitizePopupTitles();
  const mo = new MutationObserver((muts)=>{
    muts.forEach(m=> m.addedNodes && m.addedNodes.forEach(n=>{ if(n.nodeType===1) sanitizePopupTitles(n); }));
  });
...
// === Inject 10 extra destinations for Mare & Montagna ===
document.addEventListener('DOMContentLoaded', ()=>{
  function findCarouselByCategory(labelText){
    const items = Array.from(document.querySelectorAll('.accordion-item'));
    for(const it of items){
      const label = it.querySelector('.label')?.textContent?.trim().toLowerCase();
      if(label === labelText.toLowerCase()){
        return it.querySelector('.accordion-panel .carousel.within');
      }
    }
    return null;
  }
  function createCard(name, img){
    ...
    art.className = 'city-card small';
    art.innerHTML = `
      <img alt="${name}" loading="eager" src="${img}"/>
      <div class="city-info">
        <h3>${name}</h3>
        <button class="btn secondary more" data-city="${name}">Scopri di più</button>
      </div>`;
    return art;
  }
  const seaExtra = [
    "Santorini","Mykonos","Costa Smeralda","Costiera Amalfitana",
    "Ibiza","Tenerife","Creta (Chania)","Ko Samui","Fiji","Bahamas (Exuma)"
  ];
  const mountainExtra = [
    "Cortina d'Ampezzo","Val Gardena (Dolomiti)","Madonna di Campiglio",
    "Courmayeur","St. Moritz","Kitzbühel","Chamonix (Monte Bianco)",
    "Innsbruck","Zillertal (Mayrhofen)","Sölden"
  ];
  const hero = 'assets/img/common/hero.jpg';
  const seaCarousel = findCarouselByCategory('mare');
  if(seaCarousel){
    seaExtra.forEach(n=> seaCarousel.appendChild(createCard(n, hero)));
  }
  const montCarousel = findCarouselByCategory('montagna');
  if(montCarousel){
    mountainExtra.forEach(n=> montCarousel.appendChild(createCard(n, hero)));
  }
});
