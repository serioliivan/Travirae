...
(function(){
  ...
  const map = {
    ...
    'luna-di-miele':'honeymoon,overwater','capodanno-caldo':'fireworks,beach','premium':'luxury,resort','luxury':'luxury,resort',
...
// --- Runtime guard: remove any 'Addio (al) celibato' accordion item in Hotel -> Occasioni ---
document.addEventListener('DOMContentLoaded', () => {
  try {
    const occ = document.querySelector('#occasioni');
    if (!occ) return;
    const items = occ.querySelectorAll('.accordion-item');
    items.forEach(it => {
      const labelEl = it.querySelector('.accordion-toggle .label');
      const txt = (labelEl?.textContent || '').toLowerCase().replace(/\s+/g,' ').trim();
      if (txt.includes('addio') && txt.includes('celibato')) {
        it.remove();
      }
    });
  } catch(e){ /* no-op */ }
