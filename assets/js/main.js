...
(function(){
  ...
  if(flightsForm){
    flightsForm.addEventListener('submit', (e)=>{
      ...
      const isEN = lang === 'ENG';
      const parts = [];
      if(from && to) parts.push(isDE ? `Flug von ${from} nach ${to}` : (isEN ? `Flight from ${from} to ${to}` : `volo da ${from} a ${to}`));
      if(d1) parts.push(isDE ? `Abflug ${d1}` : (isEN ? `Depart ${d1}` : `partenza ${d1}`));
      if(d2) parts.push(isDE ? `Rückflug ${d2}` : (isEN ? `Return ${d2}` : `ritorno ${d2}`));
      parts.push(isDE ? `${adults} Erwachsene ${cabin}` : (isEN ? `${adults} adults ${cabin}` : `${adults} adulti ${cabin}`));
      const url = 'https://www.google.com/travel/flights?q=' + encodeURIComponent(parts.join(' ')) + (isDE ? '&hl=de' : (isEN ? '&hl=en' : '&hl=it'));
...
(function(){
  ...
  function showToast(message, type = 'success') {
    try {
      ...
      if (type === 'error') title.textContent = isDE ? 'Fehler' : (isEN ? 'Error' : 'Errore');
      else if (type === 'info') title.textContent = 'Info';
      else title.textContent = isDE ? 'Erfolgreich' : (isEN ? 'Success' : 'Operazione completata');
      ...
      closeBtn.setAttribute('aria-label', isDE ? 'Schließen' : (isEN ? 'Close' : 'Chiudi'));
  ...
  function injectLangSelector(){
    ...
    // a11y label: translate only for DEU/ENG (others keep Italian)
  ...
  function initNewsletterSignup(){
    ...
    form.addEventListener('submit', async (e) => {
      ...
      try{
        ...
        const baseReqBody = { email, language: lang, source: 'site_footer' };

        async function sendNewsletterSubscribe(reqBody){
        ...
        try{
          await sendNewsletterSubscribe(baseReqBody);
        }catch(firstErr){
          // Some backends may not support every language code yet.
          // For any language without backend support, retry once with ITA to avoid breaking the signup flow.
          if (lang !== 'DEU' && lang !== 'ITA') {
            await sendNewsletterSubscribe({ email, language: 'ITA', source: 'site_footer' });
          } else {
            throw firstErr;
          }
        }
        ...
        showToast((lang === 'DEU') ? 'Abo bestätigt! Danke.' : ((lang === 'ENG') ? 'Subscription confirmed. Thank you.' : 'Iscrizione completata! Grazie.'), 'success');
      } catch (err){
        showToast((lang === 'DEU') ? 'Fehler bei der Anmeldung. Bitte versuche es später erneut.' : ((lang === 'ENG') ? 'Subscription failed. Please try again later.' : 'Errore durante iscrizione. Riprova tra poco.'), 'error');
...
(function(){
  ...
  document.addEventListener('click', (ev)=>{
    ...
    const fallbackCity = isDE ? 'Das Reiseziel' : (isEN ? 'The destination' : 'La destinazione');
