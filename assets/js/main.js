...
(function(){
  ...
  function showToast(message, type = 'success') {
    try {
      ...
      if (type === 'error') title.textContent = isDE ? 'Fehler' : (isEN ? 'Error' : (isFR ? 'Erreur' : (isES ? 'Error' : (isAR ? 'خطأ' : (isRU ? 'Ошибка' : 'Errore')))));
      else if (type === 'info') title.textContent = isRU ? 'Информация' : (isAR ? 'معلومات' : 'Info');
      else title.textContent = isDE ? 'Erfolgreich' : (isEN ? 'Success' : (isFR ? 'Succès' : (isES ? 'Éxito' : (isAR ? 'تم بنجاح' : (isRU ? 'Успешно' : 'Operazione completata')))));
      ...
      closeBtn.setAttribute('aria-label', isDE ? 'Schließen' : (isEN ? 'Close' : (isFR ? 'Fermer' : (isES ? 'Cerrar' : (isAR ? 'إغلاق' : (isRU ? 'Закрыть' : 'Chiudi'))))));
  ...
  function routeByLang(forceCode){
    ...
    try{
      if (code === 'DEU') {
        if (!isDe) {
          go(file.replace(/\.html$/i, '-de.html').replace(/-(en|fr|es|ru|ar)-de\.html$/i, '-de.html'));
        }
      } else if (code === 'ENG') {
        if (!isEn) {
          go(file.replace(/\.html$/i, '-en.html').replace(/-(de|fr|es|ru|ar)-en\.html$/i, '-en.html'));
        }
      } else if (code === 'FRA') {
        if (!isFr) {
          go(file.replace(/\.html$/i, '-fr.html').replace(/-(de|en|es|ru|ar)-fr\.html$/i, '-fr.html'));
        }
      } else if (code === 'SPA') {
        if (!isEs) {
          go(file.replace(/\.html$/i, '-es.html').replace(/-(de|en|fr|ru|ar)-es\.html$/i, '-es.html'));
        }
      } else if (code === 'RUS') {
        if (!isRu) {
          go(file.replace(/\.html$/i, '-ru.html').replace(/-(de|en|fr|es|ar)-ru\.html$/i, '-ru.html'));
        }
      } else if (code === 'ARA') {
        if (!isAr) {
          go(file.replace(/\.html$/i, '-ar.html').replace(/-(de|en|fr|es|ru)-ar\.html$/i, '-ar.html'));
        }
      } else {
        // Default Italian pages: drop any suffix
        if (isDe || isEn || isFr || isEs || isRu || isAr) {
          go(file.replace(/-(de|en|fr|es|ru|ar)\.html$/i, '.html'));
        }
      }
    }catch(e){
      // non-blocking
