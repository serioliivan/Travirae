...
(function(){
  ...
  document.addEventListener('click', (ev)=>{
    ...
    if (isDE) {
      ...
      // Chinese pages: use dedicated ZH datasets when available (data-*-zh).
      // We intentionally do NOT render the ITA datasets (data-desc / data-highlights / data-bonus).
      const desc = trg.dataset.descZh || '';
      const hiRaw = trg.dataset.highlightsZh || '';
      const bonus = trg.dataset.bonusZh || '';

      descEl.textContent = desc ? desc : '简介即将上线。';
      hiEl.innerHTML = '';
      const hiList = (hiRaw ? hiRaw.split('|') : ['拍照打卡点', '本地美食', '轻松氛围', '日落散步']);
      hiList.forEach(x=>{
        const li = document.createElement('li'); 
        li.textContent = String(x).trim(); hiEl.appendChild(li);
      });
      bonusEl.textContent = bonus ? bonus : '🎁 小惊喜：详情即将上线。';
    } else {
