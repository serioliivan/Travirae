...
(function(){
  ...
  if(flightsForm){
    flightsForm.addEventListener('submit', (e)=>{
      ...
      const isFR = lang === 'FRA';
      const parts = [];
      if(from && to) parts.push(isDE ? `Flug von ${from} nach ${to}` : (isEN ? `Flight from ${from} to ${to}` : (isFR ? `Vol de ${from} à ${to}` : `volo da ${from} a ${to}`)));
      if(d1) parts.push(isDE ? `Abflug ${d1}` : (isEN ? `Departure ${d1}` : (isFR ? `Départ ${d1}` : `partenza ${d1}`)));
      if(d2) parts.push(isDE ? `Rückflug ${d2}` : (isEN ? `Return ${d2}` : (isFR ? `Retour ${d2}` : `ritorno ${d2}`)));
      parts.push(isDE ? `${adults} Erwachsene ${cabin}` : (isEN ? `${adults} adults ${cabin}` : (isFR ? `${adults} adultes ${cabin}` : `${adults} adulti ${cabin}`)));
      const url = 'https://www.google.com/travel/flights?q=' + encodeURIComponent(parts.join(' ')) + (isDE ? '&hl=de' : (isEN ? '&hl=en' : (isFR ? '&hl=fr' : '&hl=it')));
...
(function(){
  ...
  function showToast(message, type = 'success') {
    try {
      ...
      if (type === 'error') title.textContent = isDE ? 'Fehler' : (isEN ? 'Error' : (isFR ? 'Erreur' : 'Errore'));
      else if (type === 'info') title.textContent = 'Info';
      else title.textContent = isDE ? 'Erfolgreich' : (isEN ? 'Success' : (isFR ? 'Succès' : 'Operazione completata'));
      ...
      closeBtn.setAttribute('aria-label', isDE ? 'Schließen' : (isEN ? 'Close' : (isFR ? 'Fermer' : 'Chiudi')));
...
(function(){
  ...
  document.addEventListener('click', (ev)=>{
    ...
    const fallbackCity = isDE ? 'Das Reiseziel' : (isEN ? 'The destination' : (isFR ? 'La destination' : 'La destinazione'));
