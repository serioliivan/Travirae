...
// === Title sanitation utilities for popups ===
...
function nameOnly(str){
  if(!str) return '';
  // Keep only part before EM DASH (used by our taglines)
  let s = stripEmojis(str);
  const i = s.indexOf('—');
  if(i > 0) s = s.slice(0, i);
  return s.trim();
}
// Remove emojis and taglines from popup headings
function sanitizePopupTitles(root=document){
  ...
  selectors.forEach(sel => {
    root.querySelectorAll(sel).forEach(el => {
      el.textContent = nameOnly(el.textContent || '');
    });
  });
}
document.addEventListener('DOMContentLoaded', ()=>{
  ...
  const mo = new MutationObserver((muts)=>{
    for(const m of muts){
      m.addedNodes && m.addedNodes.forEach(n=>{ if(n.nodeType===1) sanitizePopupTitles(n); });
    }
  });
...
// ===== Country labels & card decoration =====
(function(){
  // Mapping: destination -> country (IT)
  const COUNTRY_MAP = {
    // Mare
    'Maldive':'Maldive','Seychelles':'Seychelles','Bora Bora':'Polinesia Francese','Mauritius':'Mauritius','Zanzibar':'Tanzania',
    'Bali (Uluwatu)':'Indonesia','Bali':'Indonesia','Phuket':'Thailandia','Cancun – Riviera Maya':'Messico','Maui (Hawaii)':'Stati Uniti','Maui':'Stati Uniti','Algarve':'Portogallo',
    'Santorini (Grecia)':'Grecia','Santorini':'Grecia','Mykonos (Grecia)':'Grecia','Mykonos':'Grecia','Formentera (Spagna)':'Spagna','Formentera':'Spagna',
    'Costa Smeralda (Sardegna)':'Italia','Costa Smeralda':'Italia','Salento (Puglia)':'Italia','Salento':'Italia','Amalfi (Costiera Amalfitana)':'Italia','Amalfi':'Italia',
    'Ibiza (Spagna)':'Spagna','Ibiza':'Spagna','Koh Samui (Thailandia)':'Thailandia','Koh Samui':'Thailandia','Boracay (Filippine)':'Filippine','Boracay':'Filippine',
    'Nosy Be (Madagascar)':'Madagascar','Nosy Be':'Madagascar',
    // Montagna
    'Zermatt (Svizzera)':'Svizzera','Zermatt':'Svizzera',
    'Chamonix (Francia)':'Francia','Chamonix':'Francia',
    'Dolomiti (Cortina)':'Italia','Dolomiti':'Italia','Cortina':'Italia',
    'Canadian Rockies (Banff)':'Canada','Banff':'Canada','Canadian Rockies':'Canada',
    'Patagonia – El Chalten':'Argentina','El Chalten':'Argentina','Patagonia':'Argentina',
    'Annapurna (Nepal)':'Nepal','Annapurna':'Nepal',
    'Japanese Alps (Hakuba)':'Giappone','Hakuba':'Giappone','Japanese Alps':'Giappone',
    'Queenstown (New Zealand)':'Nuova Zelanda','Queenstown':'Nuova Zelanda',
    'Aspen (Colorado)':'Stati Uniti','Aspen':'Stati Uniti',
    'Kitzbuhel (Tirolo)':'Austria','Kitzbuhel':'Austria',
    'Courmayeur (Italia)':'Italia','Courmayeur':'Italia',
    'Cervinia (Italia)':'Italia','Cervinia':'Italia',
    'St. Moritz (Svizzera)':'Svizzera','St. Moritz':'Svizzera',
    'Verbier (Svizzera)':'Svizzera','Verbier':'Svizzera',
    'Grindelwald (Svizzera)':'Svizzera','Grindelwald':'Svizzera',
    'Innsbruck (Austria)':'Austria','Innsbruck':'Austria',
    'Zillertal (Austria)':'Austria','Zillertal':'Austria',
    'Val d’Isère (Francia)':'Francia','Val d’Isère':'Francia',
    'Whistler (Canada)':'Canada','Whistler':'Canada',
    'Tignes (Francia)':'Francia','Tignes':'Francia'
  };

  function baseName(raw){
    if(!raw) return '';
    let s = (raw||'').trim();
    s = s.replace(/\s*\(.*?\)\s*$/,''); // drop parenthetical tail
    return s.trim();
  }
  ...
  function applyCardCountryLabels(){
    document.querySelectorAll('.city-card .city-info h3').forEach(h=>{
      ...
      const country = COUNTRY_MAP[raw] || COUNTRY_MAP[place] || '';
  ...
  window.__TR_COUNTRY_MAP__ = COUNTRY_MAP;
  window.__TR_baseName__ = baseName;
})();

// ===== Lazy load Stay22 main map =====
document.addEventListener('DOMContentLoaded', ()=>{
  const map = document.getElementById('stay22-widget');
  if(!map) return;
  const src = map.getAttribute('data-src') || map.getAttribute('src');
  if(!src) return;
  // Remove immediate src to prevent eager load if present
  if(map.getAttribute('src')) map.removeAttribute('src');
  const load = ()=>{ if(!map.getAttribute('src')) map.setAttribute('src', src); };
...
// Helper for popup title (only the place, no country)
...
(function(){
  ...
  document.addEventListener('click', (ev)=>{
    ...
    const mapEl = modal.querySelector('.map-embed');
    if(mapEl && !mapEl.getAttribute('src')){ const ds = mapEl.getAttribute('data-src'); if(ds) mapEl.setAttribute('src', ds); }
    hiEl.innerHTML = '';
