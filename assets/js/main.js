...
(function(){
  ...
  function detectSiteLangFromBrowser(){
    // First visit on an unsuffixed page follows the browser language.
    // Unsupported browser languages still fall back to English.
    try {
      const candidates = [];
  ...
  function detectSiteLangFromPath(){
    try {
      const file = (location.pathname.split('/').pop() || '').toLowerCase();
      const m = file.match(/-(de|en|fr|es|ru|ar|nl|zh)\.html$/i);
      if (!m) return '';
      const found = LANGS.find(l => String(l.htmlLang || '').toLowerCase() === String(m[1] || '').toLowerCase());
  ...
  function getSiteLang(){
    ...
    // If the user opens a specific language URL directly (for example index-en.html),
    // respect that URL instead of redirecting back to the browser language.
    const pathLang = detectSiteLangFromPath();
    if (pathLang) return normalizeLang(pathLang);

    // Otherwise, fresh unsuffixed visits follow the browser language.
...
(function(){
  ...
  function storeAffiliate(id, source){
    id = sanitizeRef(id);
    if (!id) return;
    try{
      if (window.sessionStorage) {
        sessionStorage.setItem('tva_session_ref', id);
        sessionStorage.setItem('tva_session_ref_source', sanitizeRef(source || 'ref') || 'ref');
      }
    }catch(e){}
    purgeLegacyAffiliateStorage();
  }

  function getStoredAffiliateSource(){
    try{
      return window.sessionStorage ? String(sessionStorage.getItem('tva_session_ref_source') || '') : '';
    }catch(e){
      return '';
    }
  }
    ...
    if (!queryRef && window.sessionStorage){
      ...
      if (existingRef){
        ...
        if (!sameOrigin){
          ...
          sessionStorage.removeItem('tva_session_ref_source');
          sessionStorage.removeItem('tva_session_ref_context');
          sessionStorage.removeItem('tva_session_ref_post_slug');
        }
      }
    }
  }catch(e){}
  ...
  try{
    ...
    if (queryRef && !sessionRef){
      ...
      sessionStorage.setItem('tva_session_ref_source', 'ref');
    }
  }catch(e){
    sessionRef = queryRef || '';
  }
  ...
  /**
   * Imposta l'affiliato di sessione quando l'utente arriva/apre un post creator.
   * Priorità mantenuta:
   * 1) se l'URL contiene ?ref=..., quel ref NON viene sovrascritto;
   * 2) i widget dentro i post usano comunque il tracking code del post;
   * 3) se non c'è un ref esplicito, il creator dell'ultimo post aperto diventa
   *    l'affiliato della sessione per i widget generali del sito.
   */
  window.traviraeAffiliate.setFromCreatorPost = function(rawSlug, options){
    options = options || {};
    var slug = sanitizeRef(rawSlug);
    if (!slug) return false;

    var explicitRef = readQueryRef();
    if (explicitRef && options.overrideExplicitRef !== true) return false;

    var currentSource = getStoredAffiliateSource();
    var currentSlug = getStoredAffiliate();

    // Non sovrascrivere una sessione nata da un vero link affiliato ?ref=.
    if (currentSlug && currentSource === 'ref' && options.overrideRefSource !== true) return false;

    storeAffiliate(slug, 'creator_post');
    affiliateId = slug;
    sessionRef = slug;
    ...
    try{
      if (window.sessionStorage) {
        ...
        if (options.postSlug) sessionStorage.setItem('tva_session_ref_post_slug', sanitizeRef(options.postSlug));
      }
    }catch(e){}

    // Se l'utente è arrivato direttamente con un link al post, conteggia anche la landing
    // affiliata una sola volta per sessione/post. Non viene usato per i click interni.
    if (options.logLanding === true && !isLangRedirectInProgress){
      var landingPath = (window.location.pathname || '/') + '?post=' + safeKeyPart(options.postSlug || '');
      if (!hasSessionLandingLogged(slug, landingPath)){
        markSessionLandingLogged(slug, landingPath);
        insertAffiliateClick(slug);
      }
    }
  ...
  window.traviraeAffiliate.getAttributionDebug = function(){
    return {
      affiliate_id: affiliateId || getStoredAffiliate() || '',
      session_ref: sessionRef || getStoredAffiliate() || '',
      session_ref_source: getStoredAffiliateSource() || '',
      explicit_ref: readQueryRef() || ''
    };
  };
...
function getSupabaseRestConfig(){
  try{
    ...
    var anonKey = String(cfg.SUPABASE_ANON_KEY || '').trim();
    if (!url || !anonKey) return null;
    return {
      url: url + '/rest/v1/influencer_post_events',
      anonKey: anonKey
    };
  }catch(_e){
    return null;
...
async function insertInfluencerEventViaClient(payload){
  try{
...
async function insertInfluencerEventViaRest(payload, keepalive){
  try{
...
async function insertInfluencerEventBestEffort(payload, options){
  options = options || {};
  var preferRest = options.preferRest !== false;
  var keepalive = options.keepalive !== false;

  if (preferRest){
    var okRestFirst = await insertInfluencerEventViaRest(payload, keepalive);
    if (okRestFirst) return true;
    return insertInfluencerEventViaClient(payload);
  }

  var okClientFirst = await insertInfluencerEventViaClient(payload);
  if (okClientFirst) return true;
  return insertInfluencerEventViaRest(payload, keepalive);
}
  ...
  window.traviraeAffiliate.trackWidgetOutbound = async function(options){
    ...
    var refSlug = sanitizeRef(options.affiliateSlug || sessionRef || getStoredAffiliate() || '');
    var payload = {
      event_type: 'widget_click',
      affiliate_slug: refSlug || null,
      partner: partner,
      ...
      metadata: {
        ...
        ref_affiliate_slug: refSlug || null,
        ref_source: getStoredAffiliateSource() || null
      }
    };

    var ok = await insertInfluencerEventBestEffort(payload, {
      preferRest: true,
      keepalive: true
    });

    // affiliate_slug has a DB foreign key. If an old/invalid external ref is present,
    // keep the outbound event without breaking tracking for valid refs.
    if (!ok && refSlug){
      var fallbackPayload = Object.assign({}, payload);
      delete fallbackPayload.affiliate_slug;
      return insertInfluencerEventBestEffort(fallbackPayload, {
        preferRest: true,
        keepalive: true
      });
    }
