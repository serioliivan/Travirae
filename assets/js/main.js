...
(function(){
  ...
  if(flightsForm){
    flightsForm.addEventListener('submit', (e)=>{
      ...
      const isES = lang === 'SPA';
      const parts = [];
      if(from && to) parts.push(isDE ? `Flug von ${from} nach ${to}` : (isEN ? `Flight from ${from} to ${to}` : (isFR ? `Vol de ${from} à ${to}` : (isES ? `Vuelo de ${from} a ${to}` : `volo da ${from} a ${to}`))));
      if(d1) parts.push(isDE ? `Abflug ${d1}` : (isEN ? `Departure ${d1}` : (isFR ? `Départ ${d1}` : (isES ? `Salida ${d1}` : `partenza ${d1}`))));
      if(d2) parts.push(isDE ? `Rückflug ${d2}` : (isEN ? `Return ${d2}` : (isFR ? `Retour ${d2}` : (isES ? `Regreso ${d2}` : `ritorno ${d2}`))));
      parts.push(isDE ? `${adults} Erwachsene ${cabin}` : (isEN ? `${adults} adults ${cabin}` : (isFR ? `${adults} adultes ${cabin}` : (isES ? `${adults} adultos ${cabin}` : `${adults} adulti ${cabin}`))));
      const url = 'https://www.google.com/travel/flights?q=' + encodeURIComponent(parts.join(' ')) + (isDE ? '&hl=de' : (isEN ? '&hl=en' : (isFR ? '&hl=fr' : (isES ? '&hl=es' : '&hl=it'))));
...
(function(){
  ...
  function showToast(message, type = 'success') {
    try {
      ...
      if (type === 'error') title.textContent = isDE ? 'Fehler' : (isEN ? 'Error' : (isFR ? 'Erreur' : (isES ? 'Error' : 'Errore')));
      else if (type === 'info') title.textContent = 'Info';
      else title.textContent = isDE ? 'Erfolgreich' : (isEN ? 'Success' : (isFR ? 'Succès' : (isES ? 'Éxito' : 'Operazione completata')));
      ...
      closeBtn.setAttribute('aria-label', isDE ? 'Schließen' : (isEN ? 'Close' : (isFR ? 'Fermer' : (isES ? 'Cerrar' : 'Chiudi'))));
  ...
  function routeByLang(forceCode){
    try{
      ...
      if (code === 'DEU') {
        ...
        if (isDePage) {
          const targetFile = file.replace(/-de\.html$/i, '.html');
          u.pathname = dir + targetFile;
          window.location.replace(u.toString());
        } else if (isEnPage) {
          const targetFile = file.replace(/-en\.html$/i, '.html');
          u.pathname = dir + targetFile;
          window.location.replace(u.toString());
        } else if (isFrPage) {
          const targetFile = file.replace(/-fr\.html$/i, '.html');
          u.pathname = dir + targetFile;
          window.location.replace(u.toString());
        } else if (isEsPage) {
          const targetFile = file.replace(/-es\.html$/i, '.html');
          u.pathname = dir + targetFile;
          window.location.replace(u.toString());
        }
      }
  ...
  function initNewsletterSignup(){
    ...
    form.addEventListener('submit', async (e) => {
      ...
      try{
        ...
        showToast((lang === 'DEU') ? 'Abo bestätigt! Danke.' : ((lang === 'ENG') ? 'Subscription confirmed. Thank you.' : ((lang === 'FRA') ? 'Inscription confirmée. Merci.' : ((lang === 'SPA') ? 'Suscripción confirmada. ¡Gracias!' : 'Iscrizione completata! Grazie.'))), 'success');
      } catch (err){
        showToast((lang === 'DEU') ? 'Fehler bei der Anmeldung. Bitte versuche es später erneut.' : ((lang === 'ENG') ? 'Subscription failed. Please try again later.' : ((lang === 'FRA') ? 'Erreur lors de l’inscription. Réessaie plus tard.' : ((lang === 'SPA') ? 'Error al suscribirse. Inténtalo de nuevo más tarde.' : 'Errore durante iscrizione. Riprova tra poco.'))), 'error');
...
(function(){
  ...
  document.addEventListener('click', (ev)=>{
    ...
    const fallbackCity = isDE ? 'Das Reiseziel' : (isEN ? 'The destination' : (isFR ? 'La destination' : (isES ? 'El destino' : 'La destinazione')));
