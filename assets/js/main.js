...
// === Travirae affiliate tracking (global ?ref handler + booking helper) ===
(function(){
  ...
  function getCookie(name){
    try {
      var re = new RegExp('(?:^|;\\s*)' + name.replace(/[-[\]{}()*+?.,\\^$|#\\s]/g,'\\$&') + '=([^;]*)');
  ...
  function storeAffiliate(id){
    ...
    try {
      if ('localStorage' in window){
        window.localStorage.setItem('tva_aff', id);
      }
    } catch (e) {}
  ...
  function readAffiliate(){
    ...
    try {
      if ('localStorage' in window){
        val = window.localStorage.getItem('tva_aff') || '';
      }
    } catch (e) {
      val = '';
    }
    if (val) return val;
    return getCookie('tva_aff') || '';
  }
  ...
  function isInternalLink(a){
    ...
    // If absolute URL, keep only same-origin
    if (/^https?:\/\//i.test(href)) {
      try {
        var urlAbs = new URL(href, window.location.href);
        if (urlAbs.origin !== window.location.origin) return false;
      } catch (e) {
        return false;
      }
    }
  ...
  function decorateInternalLinks(affId){
    if (!affId) return;
    try {
      var anchors = document.querySelectorAll('a[href]');
      for (var i = 0; i < anchors.length; i++){
        var a = anchors[i];
        if (!isInternalLink(a)) continue;
        var href = a.getAttribute('href') || '';
        ...
        try {
          ...
          if (href.indexOf('?') === -1) {
            a.setAttribute('href', href + '?ref=' + encodeURIComponent(affId));
          } else {
            a.setAttribute('href', href + '&ref=' + encodeURIComponent(affId));
          }
        }
      ...
      if (window.console && console.warn) {
        console.warn('Travirae: errore decorateInternalLinks', e);
      }
    }
  }
  ...
  var refFromUrl = getQueryRef();
  var affiliateId = '';

  if (refFromUrl){
    affiliateId = refFromUrl;
    storeAffiliate(refFromUrl);

    // Track click in affiliate_clicks
    try {
      if (window.supabaseClient && typeof window.supabaseClient.from === 'function') {
        window.supabaseClient
          ...
          .catch(function(err){
            if (window.console && console.warn) console.warn('Travirae: errore tracking affiliato', err);
          });
      }
    } catch (e) {
      if (window.console && console.warn) console.warn('Travirae: errore tracking affiliato (blocco esterno)', e);
    }
  } else {
    affiliateId = readAffiliate() || '';
  }

  // expose helper
  window.traviraeAffiliate = window.traviraeAffiliate || {};
  window.traviraeAffiliate.getId = function(){
    return affiliateId || readAffiliate() || '';
  };

  // Booking click helper: registra un "click verso partner" nella tabella bookings
  window.traviraeAffiliate.trackBookingClick = function(partner){
    try {
      var affiliate = window.traviraeAffiliate.getId();
      if (!affiliate) {
        if (window.console && console.warn) console.warn('Travirae: trackBookingClick senza affiliate ID');
        return;
      }
      if (!window.supabaseClient || typeof window.supabaseClient.from !== 'function') {
        if (window.console && console.warn) console.warn('Travirae: trackBookingClick senza supabaseClient');
        return;
      }
      var payload = {
      ...
      if (window.console && console.warn) console.warn('Travirae: errore generale trackBookingClick', e);
    }
  };

  // Apply link decoration when DOM is ready
  function onReady(fn){
  ...
  onReady(function(){
    var id = affiliateId || readAffiliate();
    if (id) {
      decorateInternalLinks(id);
    }
  });
})();
