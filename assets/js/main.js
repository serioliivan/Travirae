...
if(document.readyState==='loading') document.addEventListener('DOMContentLoaded', applyBG);
  ...
  // --- Affiliate tracking & click logging (affiliate ID + Supabase) ---
  (function(){
    ...
    function setAffiliate(id){
      ...
      document.cookie = 'tva_aff=' + encodeURIComponent(clean) +
                        '; path=/; max-age=' + maxAge;
      try {
        window.localStorage.setItem('tva_aff', clean);
      } catch(e){}
    }

    function getAffiliate(){
      const m = document.cookie.match(/(?:^|;\s*)tva_aff=([^;]+)/);
    ...
    if (refFromUrl){
      setAffiliate(refFromUrl);

      // Log del click in Supabase (tabella affiliate_clicks)
      if (window.supabaseClient && window.supabaseClient.from){
        try{
          window.supabaseClient
            .from('affiliate_clicks')
            .insert({
              affiliate_slug: refFromUrl,
    ...
    // Espone una funzione globale per leggere l'Affiliate ID
