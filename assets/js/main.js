...
(function(){
  ...
  function initNewsletterSignup(){
    ...
    const ensureStatus = (form) => {
      let el = form.querySelector('.newsletter-status');
      if (!el) {
    ...
    const setStatus = (form, msg, type='info') => {
      const el = ensureStatus(form);
      el.textContent = msg;
    ...
    forms.forEach(form => {
      ...
      form.addEventListener('submit', async (e) => {
        ...
        const emailInput = form.querySelector('input[type="email"], input[name="email"], input#newsletter-email');
        const submitBtn = form.querySelector('button[type="submit"], input[type="submit"]');
        const email = (emailInput?.value || '').trim();

        if (!email || !/^[^\s@]+@[^\s@]+\.[^\s@]+$/.test(email)) {
          setStatus(form, 'Inserisci un\'email valida.', 'error');
          return;
        }

        if (submitBtn) submitBtn.disabled = true;
        setStatus(form, 'Iscrizione in corso…', 'info');

        try {
          const sb = window.supabaseClient;
          if (!sb?.functions?.invoke) throw new Error('Supabase non inizializzato');
          ...
          // data.already_subscribed might exist
          setStatus(
            form,
          ...
          setStatus(form, 'Errore durante l\'iscrizione. Riprova tra poco.', 'error');
        } finally {
