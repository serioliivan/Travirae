...
// ===== Background photos for category bars (REMOTE via Unsplash)
(function(){
  function slug(s){return (s||'').toLowerCase().normalize('NFD').replace(/[\u0300-\u036f]/g,'').replace(/[^a-z0-9]+/g,'-').replace(/(^-|-$)/g,'');}
  const queryMap = {
    'addio-celibato':'party,nightlife,beach club',
    'anniversario':'anniversary,romantic,city night',
    ...
    'citta':'city skyline,old town,europe',
    'compleanno':'birthday trip,celebration',
    'coppia':'romantic sunset,coast',
    'cultura':'old town,museum,architecture',
    'da-solo':'solo traveler,scenic road',
    'destinazioni-popolari':'famous landmarks,world travel',
    'esotici':'tropical,paradise,lagoon',
    'estate':'summer,beach',
  ...
  function applyBG(){
    document.querySelectorAll('.accordion-toggle.category').forEach(btn=>{
      ...
      test.onerror = ()=>{ btn.style.backgroundImage = 'url(assets/img/common/hero-plane.jpg)'; }; // fallback clean image
      test.src = chosen;
    });
  }
