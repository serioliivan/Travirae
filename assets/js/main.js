...
(function(){
  ...
  if(flightsForm){
    flightsForm.addEventListener('submit', (e)=>{
      ...
      const isRU = lang === 'RUS';
      const parts = [];
      if(from && to) parts.push(isDE ? `Flug von ${from} nach ${to}` : (isEN ? `Flight from ${from} to ${to}` : (isFR ? `Vol de ${from} à ${to}` : (isES ? `Vuelo de ${from} a ${to}` : (isRU ? `Рейс из ${from} в ${to}` : `volo da ${from} a ${to}`)))));
      if(d1) parts.push(isDE ? `Abflug ${d1}` : (isEN ? `Departure ${d1}` : (isFR ? `Départ ${d1}` : (isES ? `Salida ${d1}` : (isRU ? `Вылет ${d1}` : `partenza ${d1}`)))));
      if(d2) parts.push(isDE ? `Rückflug ${d2}` : (isEN ? `Return ${d2}` : (isFR ? `Retour ${d2}` : (isES ? `Regreso ${d2}` : (isRU ? `Обратный рейс ${d2}` : `ritorno ${d2}`)))));
      parts.push(isDE ? `${adults} Erwachsene ${cabin}` : (isEN ? `${adults} adults ${cabin}` : (isFR ? `${adults} adultes ${cabin}` : (isES ? `${adults} adultos ${cabin}` : (isRU ? `${adults} взрослых ${cabin}` : `${adults} adulti ${cabin}`)))));
      const url = 'https://www.google.com/travel/flights?q=' + encodeURIComponent(parts.join(' ')) + (isDE ? '&hl=de' : (isEN ? '&hl=en' : (isFR ? '&hl=fr' : (isES ? '&hl=es' : (isRU ? '&hl=ru' : '&hl=it')))));
...
(function(){
  ...
  function showToast(message, type = 'success') {
    try {
      ...
      if (type === 'error') title.textContent = isDE ? 'Fehler' : (isEN ? 'Error' : (isFR ? 'Erreur' : (isES ? 'Error' : (isRU ? 'Ошибка' : 'Errore'))));
      else if (type === 'info') title.textContent = isRU ? 'Информация' : 'Info';
      else title.textContent = isDE ? 'Erfolgreich' : (isEN ? 'Success' : (isFR ? 'Succès' : (isES ? 'Éxito' : (isRU ? 'Успешно' : 'Operazione completata'))));
      ...
      closeBtn.setAttribute('aria-label', isDE ? 'Schließen' : (isEN ? 'Close' : (isFR ? 'Fermer' : (isES ? 'Cerrar' : (isRU ? 'Закрыть' : 'Chiudi')))));
  ...
  function routeByLang(forceCode){
    try{
      const code = normalizeLang(forceCode || getSiteLang());
      const u = new URL(window.location.href);
      let path = u.pathname || '';
      // If the site is opened at a directory ("/"), assume index.html
      if (path.endsWith('/')) path = path + 'index.html';
      const file = path.split('/').pop() || 'index.html';
      const dir = path.slice(0, path.length - file.length);
      const isDePage = /-de\.html$/i.test(file);
      const isEnPage = /-en\.html$/i.test(file);
      const isFrPage = /-fr\.html$/i.test(file);
      const isEsPage = /-es\.html$/i.test(file);
      const isRuPage = /-ru\.html$/i.test(file);

      function go(targetFile){
        u.pathname = dir + targetFile;
        window.location.replace(u.toString());
      }

      if (code === 'DEU') {
        if (!isDePage && /\.html$/i.test(file)) {
          const targetFile = isEnPage ? file.replace(/-en\.html$/i, '-de.html')
                                      : (isFrPage ? file.replace(/-fr\.html$/i, '-de.html')
                                                  : (isEsPage ? file.replace(/-es\.html$/i, '-de.html')
                                                              : (isRuPage ? file.replace(/-ru\.html$/i, '-de.html')
                                                                          : file.replace(/\.html$/i, '-de.html'))));
          go(targetFile);
        }
      } else if (code === 'ENG') {
        if (!isEnPage && /\.html$/i.test(file)) {
          const targetFile = isDePage ? file.replace(/-de\.html$/i, '-en.html')
                                      : (isFrPage ? file.replace(/-fr\.html$/i, '-en.html')
                                                  : (isEsPage ? file.replace(/-es\.html$/i, '-en.html')
                                                              : (isRuPage ? file.replace(/-ru\.html$/i, '-en.html')
                                                                          : file.replace(/\.html$/i, '-en.html'))));
          go(targetFile);
        }
      } else if (code === 'FRA') {
        if (!isFrPage && /\.html$/i.test(file)) {
          const targetFile = isDePage ? file.replace(/-de\.html$/i, '-fr.html')
                                      : (isEnPage ? file.replace(/-en\.html$/i, '-fr.html')
                                                  : (isEsPage ? file.replace(/-es\.html$/i, '-fr.html')
                                                              : (isRuPage ? file.replace(/-ru\.html$/i, '-fr.html')
                                                                          : file.replace(/\.html$/i, '-fr.html'))));
          go(targetFile);
        }
      } else if (code === 'SPA') {
        if (!isEsPage && /\.html$/i.test(file)) {
          const targetFile = isDePage ? file.replace(/-de\.html$/i, '-es.html')
                                      : (isEnPage ? file.replace(/-en\.html$/i, '-es.html')
                                                  : (isFrPage ? file.replace(/-fr\.html$/i, '-es.html')
                                                              : (isRuPage ? file.replace(/-ru\.html$/i, '-es.html')
                                                                          : file.replace(/\.html$/i, '-es.html'))));
          go(targetFile);
        }
      } else if (code === 'RUS') {
        if (!isRuPage && /\.html$/i.test(file)) {
          const targetFile = isDePage ? file.replace(/-de\.html$/i, '-ru.html')
                                      : (isEnPage ? file.replace(/-en\.html$/i, '-ru.html')
                                                  : (isFrPage ? file.replace(/-fr\.html$/i, '-ru.html')
                                                              : (isEsPage ? file.replace(/-es\.html$/i, '-ru.html')
                                                                          : file.replace(/\.html$/i, '-ru.html'))));
          go(targetFile);
        }
      } else {
        if (isDePage) {
          go(file.replace(/-de\.html$/i, '.html'));
        } else if (isEnPage) {
          go(file.replace(/-en\.html$/i, '.html'));
        } else if (isFrPage) {
          go(file.replace(/-fr\.html$/i, '.html'));
        } else if (isEsPage) {
          go(file.replace(/-es\.html$/i, '.html'));
        } else if (isRuPage) {
          go(file.replace(/-ru\.html$/i, '.html'));
        }
      }
    }catch(e){
      // non-blocking
...
(function(){
  ...
  document.addEventListener('click', (ev)=>{
    ...
    const fallbackCity = isDE ? 'Das Reiseziel' : (isEN ? 'The destination' : (isFR ? 'La destination' : (isES ? 'El destino' : (isRU ? 'Направление' : 'La destinazione'))));
