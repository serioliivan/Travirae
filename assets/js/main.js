...
(function(){
  ...
  function getSiteLang(){
    const isExplicit = safeGetLS(SITE_LANG_EXPLICIT_KEY) === '1';
    const storedSite = safeGetLS(SITE_LANG_STORAGE_KEY);
    if (isExplicit && storedSite) return normalizeLang(storedSite);

    const detected = normalizeLang(detectSiteLangFromBrowser());
    if (storedSite !== detected) safeSetLS(SITE_LANG_STORAGE_KEY, detected);
    return detected;
  }
