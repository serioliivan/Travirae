...
(function(){
  ...
  // Registra UN solo click di ingresso per affiliateId per browser
  function logLandingClick(id){
    if (!id) return;
    if (hasTracked(id)) return;
  ...
  if (queryRef && affiliateId){
    // Solo alla prima pagina aperta con ?ref=ID conteggiamo il click
