...
function cityForTitle(raw){
  ...
  // remove trailing "(...)" including surrounding spaces
...
// === Emoji sanitation for popup titles ===
...
function sanitizePopupTitles(root=document){
  const selectors = [
    ...
    '.dialog-title','.popup-title','.popover-title'
  ];
  selectors.forEach(sel => {
    document.querySelectorAll(sel).forEach(el => {
      el.textContent = stripEmojis(el.textContent || '').replace(/\s*[—–-]\s*.*$/, '').trim();
    });
  });
}
...
(function(){
  ...
  document.addEventListener('click', (ev)=>{
    ...
    titleEl.textContent = stripEmojis ? stripEmojis(cityForTitle(city)) : cityForTitle(city);
    descEl.textContent = longDescription(city, category);
    const mapEl = modal.querySelector('.map-embed');
    if(mapEl){ const ds = mapEl.getAttribute('data-src'); if(ds && !mapEl.getAttribute('src')) mapEl.setAttribute('src', ds); }
