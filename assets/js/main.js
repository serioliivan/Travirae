...
// ==== Insert flag emojis on destination cards (image-based, cross‑platform) ====
(function(){
  ...
  // Key words -> ISO alpha-2 (lowercase)
  const KEY2ISO = {
    // Europe
    'italia':'it','francia':'fr','svizzera':'ch','austria':'at','germania':'de','spagna':'es','portogallo':'pt','grecia':'gr',
    'regno unito':'gb','inghilterra':'gb','scozia':'gb','irlanda':'ie','islanda':'is','norvegia':'no','svezia':'se','finlandia':'fi',
    'paesi bassi':'nl','olanda':'nl','belgio':'be','lussemburgo':'lu','andorra':'ad','monaco':'mc','san marino':'sm',
    'polonia':'pl','repubblica ceca':'cz','cechia':'cz','slovacchia':'sk','slovenia':'si','croazia':'hr','serbia':'rs','bosnia':'ba',
    'montenegro':'me','albania':'al','romania':'ro','bulgaria':'bg','macedonia':'mk','cipro':'cy','malta':'mt',
    'turchia':'tr','georgia':'ge','armenia':'am','azerbaigian':'az',
    // Americas
    'stati uniti':'us','usa':'us','hawaii':'us','california':'us','colorado':'us','new york':'us','utah':'us','arizona':'us','alaska':'us',
    'canada':'ca','messico':'mx','mexico':'mx','cuba':'cu','repubblica dominicana':'do','giamaica':'jm','bahamas':'bs','porto rico':'pr',
    'costa rica':'cr','colombia':'co','peru':'pe','cile':'cl','argentina':'ar','brasile':'br','uruguay':'uy','bolivia':'bo','ecuador':'ec',
    // Africa & Middle East
    'marocco':'ma','egitto':'eg','tunisia':'tn','algeria':'dz','tanzania':'tz','kenya':'ke','sudafrica':'za','namibia':'na',
    'botswana':'bw','madagascar':'mg','seychelles':'sc','mauritius':'mu','maldive':'mv','zanzibar':'tz',
    'emirati arabi uniti':'ae','dubai':'ae','abu dhabi':'ae','qatar':'qa','oman':'om','giordania':'jo','israele':'il','arabia saudita':'sa',
    // Asia & Oceania
    'india':'in','sri lanka':'lk','nepal':'np','kazakistan':'kz','uzbekistan':'uz','kirghizistan':'kg',
    'cina':'cn','giappone':'jp','corea del sud':'kr','corea':'kr','taiwan':'tw','hong kong':'hk','indonesia':'id','bali':'id',
    'malesia':'my','filippine':'ph','thailandia':'th','vietnam':'vn','cambogia':'kh','laos':'la','myanmar':'mm','singapore':'sg',
    'australia':'au','nuova zelanda':'nz',
    // Territories / islands / keywords
    'polinesia francese':'pf','bora bora':'pf','oahu':'us','maui':'us','kauai':'us','algarve':'pt','ubud':'id','phuket':'th','koh samui':'th','samui':'th',
    'kamala':'th','surin':'th','sentosa':'sg','banff':'ca','jasper':'ca'
  };
  function isoFor(name){
    const raw = name||'';
    const inPar = /\(([^)]+)\)/.exec(raw); // es: (Francia), (Ubud), (Hawaii)
    const candidates = [];
    if(inPar) candidates.push(inPar[1]);
    candidates.push(raw);
    raw.split(/[–—\/,\-]/).forEach(x=>candidates.push(x)); // dash, slash, comma
    for(const c of candidates){
      const n = normalize(c);
      if(KEY2ISO[n]) return KEY2ISO[n];
      for(const key in KEY2ISO){ if(n.includes(key)) return KEY2ISO[key]; }
    }
    return null;
  }
  function injectFlags(){
    document.querySelectorAll('.city-card').forEach(card=>{
      if(card.querySelector('.flag-emoji, img.flag-emoji')) return;
      const info = card.querySelector('.city-info');
      const title = card.querySelector('h3')?.textContent?.trim() || '';
      if(!info) return;
      const iso = isoFor(title);
      if(iso){
        const img = document.createElement('img');
        img.className = 'flag-emoji';
        img.alt = iso.toUpperCase();
        img.width = 22; img.height = 16;
        img.src = 'https://flagcdn.com/24x18/' + iso.toLowerCase() + '.png';
