...
(function(){
  ...
  function setEmailLang(lang){
    ...
    // Keep only the affiliate email-language selects in sync.
    $all('select[data-lang-select]').forEach((sel) => {
  ...
  function syncLangFromUser(user){
    try{
      ...
      var stored = null;
      try{ stored = localStorage.getItem(EMAIL_LANG_STORAGE_KEY); }catch(_lsErr){ stored = null; }
      // Respect the language explicitly chosen in the site header for this browser.
  ...
  function init(){
    ...
    var profilePortalTabs = $all('.if-auth-only-tab');
    var profilePortalGroups = $all('.if-auth-gated-tabs');

    function setProfilePortalTabVisibility(isVisible){
      profilePortalTabs.forEach(function(link){
        if (isVisible) link.removeAttribute('hidden');
        else link.setAttribute('hidden', 'hidden');
      });
      profilePortalGroups.forEach(function(group){
        if (isVisible) group.removeAttribute('hidden');
    ...
    var payoutCancelarBtn = $('#affiliate-cancel-payout-form-btn');
    ...
    function ensureToast(){
      ...
      if (toastEl){
        ...
        toastCerrarEl = toastEl.querySelector('.tva-toast__close');
        if (toastCerrarEl && !toastCerrarEl.__bound){
          toastCerrarEl.__bound = true;
          toastCerrarEl.addEventListener('click', hideToast);
        }
      } else {
        ...
        toastEl.innerHTML = ''+
          '<div class="tva-toast__inner" role="alert" aria-live="polite">'+
            ...
            '<button type="button" class="tva-toast__close" aria-label="Cerrar">×</button>'+
          '</div>';
        document.body.appendChild(toastEl);

        toastMsgEl = toastEl.querySelector('.tva-toast__message');
        toastCerrarEl = toastEl.querySelector('.tva-toast__close');
        if (toastCerrarEl) toastCerrarEl.addEventListener('click', hideToast);
      }
    ...
    function ensureModal(){
      ...
      modalEl.innerHTML = ''+
        '<div class="tva-modal__card" role="dialog" aria-modal="true">'+
          '<div class="tva-modal__header">'+
            ...
            '<button type="button" class="tva-modal__close" aria-label="Cerrar">×</button>'+
          '</div>'+
      ...
      document.addEventListener('keydown', function(ev){
        ...
        if (ev.key === 'Escape') requestCerrarModal();
      });
...
function openModal(opts){
    ...
    async function ensureMfaAal2IfNeeded(client, opts = {}){
      try{
        ...
        showToast('2FA verificada.', 'success');
        return true;
      }catch(e){
        console.warn(e);
        return true;
...
// Richiede una re‑autenticazione (password) e, se presente, anche un codice 2FA.
	    ...
	    async function promptPasswordThen2FA(client, opts){
	      ...
	      try{
	          ...
	          return new Promise(function(resolve){
	            var modalId = openModal({
	              ...
	              bodyHtml: [
	                ...
	                '</div>',
	                '<input id="tvaReauthPassword" type="password" class="tva-modal__input" placeholder="Contraseña" autocomplete="current-password" />',
	              ...
	              buttons: [
	                ...
	                },
	                {
	                  ...
	                  onClick: async function(id){
	                    ...
	                    pw = pw.trim();
	                    if (!pw){
	                      if (errEl){ errEl.textContent = 'Introduce la contraseña.'; errEl.style.display = 'block'; }
	                      return;
	                    }
	                    // small loading lock
	                    ...
	                    try{
	                      ...
	                      console.warn(e);
	                      if (errEl){ errEl.textContent = 'Error de verificación. Inténtalo de nuevo.'; errEl.style.display = 'block'; }
	                      if (modal){
	              ...
	              onCerrar: function(){ resolve(null); }
	            });
	        ...
	        console.warn(e);
	        showToast('Error de verificación. Inténtalo de nuevo.', 'error');
	        return false;
	      }
	    }
    ...
    function promptTotpVerification(factorId, title, subtitle){
      return new Promise(function(resolve){
        var body = ''+
          ...
          '<label class="tva-field">Código 2FA</label>'+
          ...
          buttons: [
            ...
            },
            {
              ...
              onClick: async function(){
                ...
                if (!/^\d{6}$/.test(code)){
                  if (errEl){ errEl.textContent = 'Introduce un código válido de 6 dígitos.'; errEl.style.display = 'block'; }
                  return;
                }
                ...
                try{
                  ...
                  if (errEl){ errEl.textContent = (err && err.message) ? err.message : 'Código no válido. Inténtalo de nuevo.'; errEl.style.display = 'block'; }
                }
              }
            }
          ]
    ...
    async function promptTotpCodeModal(promptTitle){
      ...
      return new Promise(function(resolve){
        ...
        openModal({
          title: promptTitle,
          bodyHtml: ''+
            '<p class="tva-modal__subtitle">Introduce el código de 6 dígitos generado por tu app Authenticator.</p>'+
            '<div class="tva-field">'+
              '<label class="tva-label">Código 2FA</label>'+
            ...
            '</div>',
          buttons: [
            { label: 'Cancelar', className: 'btn btn-outline', onClick: function(){ closeModal(); finish(null); } },
          ...
          onCerrar: function(){ closeModal(); finish(null); }
        });
    ...
    async function open2faSettings(){
      ...
      if (verifiedFactor && verifiedFactor.id){
        openModal({
          title: 'Seguridad (2FA)',
          bodyHtml: '<p class="tva-modal__subtitle">2FA activada. Para desactivarla, confirma con un código de tu app.</p>',
          buttons: [
      ...
      // Clean stale unverified factors (e.g. user pressed "Cancelarar" during setup)
      await cleanupStaleUnverifiedTotpFactors(client, TOTP_FRIENDLY_NAME);

      openModal({
        title: 'Seguridad (2FA)',
        bodyHtml: '<p class="tva-modal__subtitle">Añade un segundo nivel de seguridad a tu cuenta (Authenticator).</p>',
        buttons: [
          { label: 'Cerrar', className: 'btn btn-outline', onClick: closeModal },
              ...
              openModal({
                ...
                bodyHtml:
                  '<div class="tva-modal__stack">'+
                    '<div style="margin:10px 0 14px; padding:12px; border:1px solid #fde68a; background:#fffbeb; border-radius:12px; font-size:13px; line-height:1.35; color:#111;"><strong>Importante:</strong> si pierdes acceso a tu app Authenticator, es posible que ya no puedas <strong>iniciar sesión</strong> en tu cuenta y que no puedas <strong>restablecer tu contraseña</strong> (“¿Olvidaste tu contraseña?”) hasta que recuperes el acceso a la 2FA. Asegúrate de mantener acceso al dispositivo/app y considera configurar la 2FA también en un segundo dispositivo.</div>'+
                    '<p class="tva-modal__subtitle">1) Escanea el código QR con tu app Authenticator.</p>'+
                  ...
                  '</div>',
                buttons: [
                  { label: 'Cancelar', className: 'btn btn-outline', onClick: cleanupEnroll },
                  { label: 'Verify & enable', className: 'btn btn-primary', onClick: async function(){
                      ...
                      closeModal();
                      showToast('¡2FA activada! Recuerda: no pierdas el acceso a tu app Authenticator; de lo contrario, podrías no poder recuperar tu cuenta.', 'success');
                    } }
                ],
    ...
    function openChangeLangModal(){
      var client = refreshClient();
      if (!client){
        showToast('Supabase no está listo. Inténtalo de nuevo en unos segundos.', 'error');
        return;
      }
      ...
      client.auth.getUser().then(function(res){
        ...
        openModal({
          ...
          bodyHtml:
            '<p class="tva-modal__subtitle">Selecciona el idioma en el que quieres recibir los correos de Travirae.</p>' +
            '<div class="tva-field">' +
              '<label class="tva-label" for="tvaLangSelect">Idioma</label>' +
              '<select class="tva-input" id="tvaLangSelect">' + optionsHtml + '</select>' +
              '<div class="tva-error" id="tvaLangError" style="display:none;"></div>' +
            '</div>',
          buttons: [
            { label: 'Cancelar', className: 'btn btn-outline', onClick: function(){ closeModal(); } },
            { label: 'Guardar', className: 'btn btn-primary', onClick: async function(){
              var selEl = document.getElementById('tvaLangSelect');
              var errEl = document.getElementById('tvaLangError');
              var next = selEl ? selEl.value : '';
              if (errEl){ errEl.style.display = 'none'; errEl.textContent = ''; }
              if (!next){
                if (errEl){ errEl.textContent = 'Selecciona un idioma para continuar.'; errEl.style.display = 'block'; }
                return;
              }
              if (supported.indexOf(next) === -1){
                if (errEl){ errEl.textContent = 'Idioma no válido.'; errEl.style.display = 'block'; }
                return;
              }
              ...
              try{
                ...
                // Keep affiliate email language separate from the visible site language.
                // The site language should keep following the browser on first visit,
                // unless the user explicitly changes it from the public header selector.
                try{
                  setEmailLang(next);
                  try{ localStorage.removeItem('travirae_site_lang_explicit'); }catch(_langErr){}
                  try{ localStorage.removeItem('travirae_site_lang_manual'); }catch(_langErr2){}
                ...
                console.warn(e);
                showToast((e && e.message) ? e.message : 'Error al cambiar el idioma.', 'error');
              }
            } }
          ]
        });
    ...
    function bindAuthForms(){
      ...
      if (loginForm){
        loginForm.addEventListener('submit', async function(e){
          ...
          if (!client || !client.auth){
            setMessage('Servicio de autenticación no disponible. Inténtalo de nuevo en breve.', 'error');
            return;
          }
          ...
          try{
            ...
            if (result.error){
              setMessage(result.error.message || 'Credenciales no válidas.', 'error');
              return;
            }
            ...
            if (!state.user){
              setMessage('Inicio de sesión correcto, pero no se pudieron obtener los datos del usuario.', 'error');
              return;
            }
            ...
            // (la lingua email viene decisa in fase di registrazione e può essere cambiata dal pulsante "Cambiar idioma")
            syncLangFromUser(state.user);

            await initDashboard();
          }catch(err){
            console.error(err);
            setMessage('Error de inicio de sesión. Inténtalo de nuevo en breve.', 'error');
          }
        });
      }
      ...
      if (registerForm){
        registerForm.addEventListener('submit', async function(e){
          ...
          if (!email || !password){
            setMessage('Introduce el email y la contraseña.', 'error');
            return;
          }
          if (password !== confirm){
          ...
          try{
            ...
            if (result.error){
              ...
              if(msg.includes('already') && msg.includes('registered')){
                setMessage('Este email ya está registrado. Prueba con otro email.', 'error');
              }else{
            ...
            setMessage('¡Registro completado! Revisa tu email para confirmar la dirección antes de iniciar sesión.', 'success');
            showView('login');
          }catch(err){
            console.error(err);
            setMessage('Error inesperado durante el registro.', 'error');
          }
        });
      }
      ...
      if (forgotForm){
        forgotForm.addEventListener('submit', async function(e){
          ...
          if (!email){
            setMessage('Introduce tu email.', 'error');
            return;
          }
          ...
          try{
            ...
            if (result.error){
              setMessage(result.error.message || 'Error al restablecer la contraseña.', 'error');
              return;
            }

            setMessage('Si la dirección existe en nuestro sistema, recibirás un correo con un enlace para restablecer tu contraseña.', 'success');
            showView('login');
          }catch(err){
            console.error(err);
            setMessage('Error inesperado al restablecer la contraseña.', 'error');
          }
        });
      }
      ...
      if (recoveryForm){
        recoveryForm.addEventListener('submit', async function(e){
          ...
          if (!password){
            setMessage('Introduce una nueva contraseña.', 'error');
            return;
          }
          if (password !== confirm){
            setMessage('Las contraseñas no coinciden.', 'error');
...
try {
  var mfaOk = await ensureMfaAal2IfNeeded(client, { force: true });
  if (!mfaOk){
    setMessage('Para actualizar tu contraseña se requiere la verificación 2FA (código del autenticador).', 'error');
    return;
  }
} catch (e) {
...
if (result.error){
  var em = result.error.message || 'Error al actualizar tu contraseña.';
  if (String(em).includes('AAL2') || String(em).toLowerCase().includes('aal2 session')){
    em = 'Para cambiar tu contraseña con la 2FA activada, también debes introducir el código del autenticador (2FA). Inténtalo de nuevo y completa la verificación 2FA.';
  }
  setMessage(em, 'error');
  return;
}

            setMessage('¡Contraseña actualizada correctamente! Ahora puedes iniciar sesión con tus nuevas credenciales.', 'success');
            showView('login');
          }catch(err){
            console.error(err);
            setMessage('Error inesperado al actualizar tu contraseña.', 'error');
          }
        });
      }
      ...
      if (logoutBtn){
        logoutBtn.addEventListener('click', async function(){
          ...
          state.slug = null;
          setProfilePortalTabVisibility(false);

          if (state.chart && typeof state.chart.destroy === 'function'){
            state.chart.destroy();
          }

          setMessage('Has cerrado sesión en el área de afiliados.', 'success');
          showView('login');
        });
      }
        ...
        elCopyBtn.addEventListener('click', function(){
          ...
          if (navigator.clipboard && navigator.clipboard.writeText){
            navigator.clipboard.writeText(elLinkInput.value)
              .then(function(){ setMessage('Enlace copiado al portapapeles.', 'success'); })
              .catch(function(){ setMessage('No se pudo copiar; selecciónalo y copia manualmente.', 'error'); });
          }else{
            try{
              var ok = document.execCommand('copy');
              setMessage(ok ? 'Enlace copiado al portapapeles.' : 'No se pudo copiar; selecciónalo y copia manualmente.', ok ? 'success' : 'error');
            }catch(e){
              setMessage('Selecciona el enlace y cópialo manualmente.', 'error');
            }
          }
        });
      }
    ...
    function renderPayoutRequestsTable(rows){
      ...
      rows.forEach(function(r){
        ...
        if (status === 'paid') statusLabel = 'Pagado';
    ...
    function bindPayoutUIOnce(){
      ...
      if (btnOpenPayoutForm){
        btnOpenPayoutForm.addEventListener('click', async function(){
          ...
          if ((Number(state.availableBalance || 0) || 0) < 50){
            setMessage('Saldo insuficiente: necesitas al menos 50 USD disponibles para solicitar un pago.', 'error');
            return;
          }
      ...
      if (payoutCancelarBtn){
        payoutCancelarBtn.addEventListener('click', function(){
      ...
      if (payoutForm){
        payoutForm.addEventListener('submit', async function(e){
          ...
          if (!client || !client.functions){
            setMessage('Servicio de pagos no disponible (Functions).', 'error');
            return;
          }
          ...
          if (amount > availableNow){
            setMessage('Saldo insuficiente: el importe solicitado supera tu saldo disponible.', 'error');
            return;
          }

          if (amount < 50){
            setMessage('Error: el mínimo para solicitar un pago es de 50 USD.', 'error');
            return;
          }

          if (confirmEl && !confirmEl.checked){
            setMessage('Debes confirmar que los datos son correctos.', 'error');
            return;
          }
          ...
          if (!fiscal.legal_name || !fiscal.tax_residence_country || !fiscal.address_country || !fiscal.address_city){
            setMessage('Completa los datos fiscales requeridos.', 'error');
            return;
          }

          if (method === 'paypal'){
            if (!payout.paypal_email){
              setMessage('Introduce tu email de PayPal.', 'error');
              return;
            }
          } else {
            if (!payout.beneficiary_name || !payout.bank_country){
              setMessage('Introduce el titular de la cuenta y el país del banco.', 'error');
              return;
            }
            // IBAN è richiesto per SEPA; per SWIFT puro potrebbe essere diverso, ma per semplicità lo chiediamo.
            if (!payout.iban){
              setMessage('Introduce un IBAN (para transferencias SEPA).', 'error');
              return;
            }
          }
          ...
          try{
            ...
            setMessage('✅ Solicitud enviada. La verás en tu historial y el administrador también recibirá un correo.', 'success');
            if (payoutFormWrap) payoutFormWrap.hidden = true;
            ...
            setMessage('❌ Error al solicitar el pago: ' + msg, 'error');
          }finally{
    ...
    async function loadDashboardStats(){
      ...
      try{
        ...
        if (btnOpenPayoutForm){
          // Non disabilitiamo il bottone: se l'utente ci clicca, mostriamo un popup "Saldo insuficiente".
          ...
          btnOpenPayoutForm.title = (available < 50) ? 'Saldo insuficiente: se requieren al menos 50 USD para solicitar un pago.' : '';
        }
    ...
    async function initDashboard(){
      ...
      if (!aff){
        setMessage('No se pudo obtener el perfil de afiliado. Inténtalo de nuevo más tarde.', 'error');
        showView('login');
        return;
      }
      ...
      setProfilePortalTabVisibility(true);
      showView('dashboard');
    ...
    if (mode === 'recovery'){
      showView('recovery');
      setMessage('Establece una nueva contraseña para tu cuenta.', null);
      return;
    }
