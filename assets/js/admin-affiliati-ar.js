  ...
  function init(){
    ...
    function ensureModal(){
      ...
      modalEl.innerHTML = [
        ...
        '    <button type="button" class="tva-modal__close" aria-label="إغلاق" id="tvaModalClose">×</button>',
        '  </div>',
...
function openModal(opts){
    ...
    async function promptAndVerifyTotpCode(client, factorId, promptTitle){
      var code = await promptTotpCodeModal({ title: (promptTitle || 'Verify 2FA') });
      if (!code) return false;
      var ok = await verifyTotpCode(client, factorId, code);
      if (!ok) showToast('رمز 2FA غير صالح.', 'error');
      return ok;
    }
    ...
    function promptTotpCodeModal(opts){
      ...
      return new Promise(function(resolve){
        openModal({
          ...
          actions: [
            { label: 'إلغاء', className: 'btn secondary', onClick: function(){ closeModal(); resolve(null); } },
            { label: 'تحقق', className: 'btn', onClick: function(){
    ...
    async function ensureMfaAal2IfNeeded(client){
      try{
        ...
        if (cur === 'aal1' && next === 'aal2'){
          ...
          while (true){
            var code = await promptTotpCodeModal({ title: 'التحقق من 2FA' });
            if (code === null){
            ...
            if (!/^\d{6}$/.test(code)){
              ...
              if (errEl) errEl.textContent = 'أدخل رمزًا صالحًا مكونًا من 6 أرقام.';
              continue;
            }
            ...
            if (client.auth.mfa.challengeAndVerify){
              ...
              if (!ok){
                ...
                if (e1) e1.textContent = (v1 && v1.error && v1.error.message) ? v1.error.message : 'Invalid code.';
              }
            }else{
              ...
              if (!ok){
                ...
                if (e3) e3.textContent = (v2 && v2.error && v2.error.message) ? v2.error.message : 'Invalid code.';
              }
            }
    ...
    async function open2faSettings(){
      ...
      if (!user){
        showToast('يجب تسجيل الدخول كمسؤول لإدارة 2FA.', 'error');
        return;
      }
      ...
      if (verifiedFactor && verifiedFactor.id){
        openModal({
          title: 'الأمان (2FA)',
          bodyHtml: '<p class="tva-modal__subtitle">تم تفعيل 2FA. لتعطيلها، أكّد باستخدام رمز من تطبيق المصادقة.</p>',
          actions: [
            { label: 'إغلاق', className: 'btn btn-outline', onClick: closeModal },
            { label: 'تعطيل 2FA', className: 'btn btn-danger', onClick: async function(){
                ...
                if (un.error){
                  showToast(un.error.message || 'تعذر تعطيل 2FA', 'error');
                  return;
                }
                closeModal();
      ...
      openModal({
        ...
        actions: [
          { label: 'إغلاق', className: 'btn btn-outline', onClick: closeModal },
          { label: 'تفعيل 2FA', className: 'btn btn-primary', onClick: async function(){

              // Ensure no verified factor already
              var existing = await getVerifiedTotpFactor(client);
              if (existing && existing.id){
                showToast('2FA مفعّلة بالفعل.', 'success');
                open2faSettings();
                return;
              }
              ...
              if (en.error){
                showToast(en.error.message || 'تعذر تفعيل 2FA', 'error');
                return;
              }
              ...
              openModal({
                ...
                bodyHtml:
                  '<div class="tva-modal__stack">'+
                    '<p class="tva-modal__subtitle">1) امسح رمز QR باستخدام Google Authenticator / Authy / Microsoft Authenticator.</p>'+
                    ...
                    '<p class="tva-modal__subtitle" style="margin-top:12px;">2) أدخل الرمز الذي تم إنشاؤه:</p>'+
                    '<div class="tva-modal__field">'+
                      '<label class="tva-modal__label">2FA code</label>'+
                  ...
                  '</div>',
                actions: [
                  { label: 'إلغاء', className: 'btn btn-outline', onClick: cleanupEnroll },
                  { label: 'تحقق وتفعيل', className: 'btn btn-primary', onClick: async function(){
                      ...
                      if (!code){
                        if (errEl){ errEl.style.display = 'block'; errEl.textContent = 'أدخل رمز 2FA.'; }
                        return;
                      }

                      var ok = await verifyTotpCode(client, factorId, code);
                      if (!ok){
                        if (errEl){ errEl.style.display = 'block'; errEl.textContent = 'رمز غير صالح. حاول مرة أخرى.'; }
                        return;
                      }
                ...
                try{
                  ...
                  if (qrWrap){
                    ...
                    if (qrUrl){
                      ...
                      p.textContent = 'رمز QR غير متوفر. استخدم المفتاح أدناه.';
