  ...
  function init(){
    ...
    function ensureModal(){
      ...
      modalEl.innerHTML = [
        ...
        '    <button type="button" class="tva-modal__close" aria-label="Fermer" id="tvaModalClose">×</button>',
        '  </div>',
...
function openModal(opts){
    ...
    async function promptAndVerifyTotpCode(client, factorId, promptTitle){
      var code = await promptTotpCodeModal({ title: (promptTitle || 'Vérifier la 2FA') });
      if (!code) return false;
      var ok = await verifyTotpCode(client, factorId, code);
      if (!ok) showToast('Code 2FA invalide.', 'error');
      return ok;
    }
    ...
    function promptTotpCodeModal(opts){
      ...
      return new Promise(function(resolve){
        openModal({
          title: opts.title || 'Vérifier la 2FA',
          bodyHtml: [
          ...
          actions: [
            ...
            { label: 'Vérifier', className: 'btn', onClick: function(){
    ...
    async function ensureMfaAal2IfNeeded(client){
      try{
        ...
        if (cur === 'aal1' && next === 'aal2'){
          ...
          if (!factor || !factor.id){
            setMessage('La 2FA est activée, mais aucun facteur TOTP n’a été trouvé.', 'error');
            return false;
          }

          while (true){
            var code = await promptTotpCodeModal({ title: 'Vérifier la 2FA' });
            if (code === null){
            ...
            if (!/^\d{6}$/.test(code)){
              ...
              if (errEl) errEl.textContent = 'Enter a valid 6-digit code.';
              continue;
            }
            ...
            if (client.auth.mfa.challengeAndVerify){
              ...
              if (!ok){
                ...
                if (e1) e1.textContent = (v1 && v1.error && v1.error.message) ? v1.error.message : 'Code invalide.';
              }
            }else{
              var ch = await client.auth.mfa.challenge({ factorId: factor.id });
              if (ch && ch.error){
                var e2 = document.getElementById('tvaMfaErr');
                if (e2) e2.textContent = ch.error.message || 'Could not create challenge.';
                continue;
              }
              ...
              if (!ok){
                ...
                if (e3) e3.textContent = (v2 && v2.error && v2.error.message) ? v2.error.message : 'Code invalide.';
              }
            }
    ...
    async function open2faSettings(){
      ...
      if (!user){
        showToast('Tu dois être connecté en tant qu’admin pour gérer la 2FA.', 'error');
        return;
      }
      ...
      if (verifiedFactor && verifiedFactor.id){
        openModal({
          ...
          actions: [
            { label: 'Fermer', className: 'btn btn-outline', onClick: closeModal },
            { label: 'Désactiver la 2FA', className: 'btn btn-danger', onClick: async function(){
                var ok = await promptAndVerifyTotpCode(client, verifiedFactor.id, 'Désactiver la 2FA');
                if (!ok) return;

                var un = await client.auth.mfa.unenroll({ factorId: verifiedFactor.id });
                if (un.error){
                  showToast(un.error.message || 'Erreur lors de la désactivation de la 2FA', 'error');
                  return;
                }
                closeModal();
        ...
        actions: [
          ...
          { label: 'Enable 2FA', className: 'btn btn-primary', onClick: async function(){
              ...
              if (existing && existing.id){
                showToast('La 2FA est déjà activée.', 'success');
                open2faSettings();
                return;
              }
              ...
              if (en.error){
                showToast(en.error.message || 'Erreur lors de l’activation de la 2FA', 'error');
                return;
              }
              ...
              openModal({
                ...
                actions: [
                  ...
                  { label: 'Vérifier et activer', className: 'btn btn-primary', onClick: async function(){
                      ...
                      var ok = await verifyTotpCode(client, factorId, code);
                      if (!ok){
                        if (errEl){ errEl.style.display = 'block'; errEl.textContent = 'Code invalide. Réessaie.'; }
                        return;
                      }
  ...
  try{
    ...
    try{
      setNewsletterStatus('Erreur lors du chargement de la newsletter : ' + (err?.message || String(err)), 'error');
    }catch(_){}
  }
}
...
Total : ${total} · Envoyées : ${sent} · Erreurs : ${failed}${extra}`, failed ? 'warn' : 'success');
...
function clearTbody(tbody){
    ...
    async function updatePayoutRequestStatus(id, newStatus){
      ...
      setMessage('Mise à jour de la demande...', null);
      try{
        var resp = await client.from('payout_requests').update(patch).eq('id', id);
        if (resp.error){
          console.error(resp.error);
          setMessage('Erreur lors de la mise à jour de la demande : ' + (resp.error.message || 'unknown'), 'error');
          return;
        }
        setMessage('Demande mise à jour.', 'success');
        await loadAll();
      }catch(e){
        console.error(e);
        setMessage('Erreur inattendue lors de la mise à jour de la demande.', 'error');
      }
    }
    ...
    async function runImportTravelpayouts(){
      ...
      setMessage('Import en cours (Aviasales)...', null);
      try{
        // days = 45 default
        var resp = await client.functions.invoke('import-travelpayouts', { body: { days: 45, campaign_id: 100 } });
        if (resp.error){
          console.error(resp.error);
          setMessage('Erreur d’import : ' + (resp.error.message || 'unknown'), 'error');
          return;
        }
        setMessage('Import terminé. Mise à jour du tableau de bord...', 'success');
        await loadAll();
      }catch(e){
        console.error(e);
        setMessage('Erreur inattendue pendant l’import.', 'error');
      }
    }
    ...
    async function runRefreshPayouts(){
      ...
      setMessage('Actualisation des versements...', null);
      try{
        var resp = await client.rpc('refresh_monthly_affiliate_payouts');
        if (resp && resp.error){
          setMessage('Erreur d’actualisation : ' + (resp.error.message || 'unknown'), 'error');
          return;
        }
        setMessage('Versements actualisés.', 'success');
        await loadAll();
      }catch(e){
        console.error(e);
        setMessage('Erreur inattendue lors de l’actualisation.', 'error');
      }
    }
    ...
    async function importStay22CSV(file){
      ...
      setMessage('Lecture du CSV...', null);

      var text = await new Promise(function(resolve, reject){
        var r = new FileReader();
        r.onload = function(){ resolve(String(r.result || '')); };
        r.onerror = function(){ reject(new Error('erreur de lecture')); };
        r.readAsText(file);
      });

      var parsed = parseCSV(text);
      if (!parsed.rows.length){
        setMessage('CSV vide ou invalide.', 'error');
        return;
      }
      ...
      if (!rows.length){
        setMessage('CSV non pris en charge : aucune ligne valide trouvée. Assure-toi que le CSV contient au moins Campaign ID (ou affiliate_slug) et Transaction ID (ou partner_reference).', 'error');
        return;
      }
      setMessage('Import du CSV Stay22 : upsert en cours...', null);
      ...
      for (var i=0;i<rows.length;i+=chunkSize){
        ...
        if (up.error){
          ...
          setMessage('Erreur d’import du CSV : ' + (up.error.message || 'unknown'), 'error');
          return;
        }
      }
      ...
      setMessage('CSV importé. Tableau de bord mis à jour.', 'success');
      await loadAll();
    }
    ...
    if (loginForm){
      loginForm.addEventListener('submit', async function(e){
        ...
        if (!client || !client.auth){
          setMessage('Supabase Auth indisponible.', 'error');
          return;
        }
        ...
        try{
          ...
          if (res.error){
            setMessage(res.error.message || 'Erreur de connexion.', 'error');
            return;
          }
          ...
          if (!u || !isAdminEmail(u.email)){
            ...
            setMessage('Ce compte n’est pas un compte admin.', 'error');
            return;
          }
          state.user = u;
          ...
          showDashboard(true);
          setMessage('Connexion admin réussie.', 'success');
          await loadAll();
          startRealtime();
          startAutoRefresh();
        }catch(err){
          console.error(err);
          setMessage('Erreur inattendue lors de la connexion.', 'error');
        }
      });
    }
