...
(function(){
  ...
  function init(){
    ...
    // Traffico sito (mese corrente)
    var elTrafficVisits = $('#admin-traffic-visits');
    var elTrafficUniqueVisitors = $('#admin-traffic-unique-visitors');
    var elTrafficVisitsAffiliate = $('#admin-traffic-visits-affiliate');
    var elTrafficVisitsDirect = $('#admin-traffic-visits-direct');

    // KPI qualità vendite (mese corrente)
    var elQualityAffiliateSales = $('#admin-quality-affiliate-sales');
    var elQualityConvAffiliate = $('#admin-quality-conv-affiliate');
    var elQualityConvTotal = $('#admin-quality-conv-total');
    var elQualityAvgCommission = $('#admin-quality-avg-commission');
    ...
    function ensureModal(){
      ...
      modalEl.innerHTML = [
        ...
        '    <button type="button" class="tva-modal__close" aria-label="Cerrar" id="tvaModalCerrar">×</button>',
        '  </div>',
      ...
      modalEl.addEventListener('click', function(ev){
        if (ev.target === modalEl) requestCerrarModal();
      });
      var closeBtn = document.getElementById('tvaModalCerrar');
    ...
    function requestCerrarModal(){
      if (!modalEl) return;
      var cb = modalEl._onCerrar;
...
function openModal(opts){
      ...
      modalEl._onCerrar = (opts && typeof opts.onCerrar === 'function') ? opts.onCerrar : null;
      var titleEl = document.getElementById('tvaModalTitle');
      var body = document.getElementById('tvaModalBody');
      var actions = document.getElementById('tvaModalActions');
    ...
    async function promptAndVerifyTotpCode(client, factorId, promptTitle){
      var code = await promptTotpCodeModal({ title: (promptTitle || 'Verificar 2FA') });
      if (!code) return false;
      var ok = await verifyTotpCode(client, factorId, code);
      if (!ok) showToast('Código 2FA no válido.', 'error');
      return ok;
    }
    ...
    function promptTotpCodeModal(opts){
      ...
      return new Promise(function(resolve){
        openModal({
          title: opts.title || 'Verificar 2FA',
          bodyHtml: [
            '<p class="tva-modal__hint">Introduce el código de 6 dígitos de tu app de autenticación.</p>',
            '<div class="tva-modal__field">',
            '  <label class="tva-modal__label">Código 2FA</label>',
          ...
          actions: [
            { label: 'Cancel', className: 'btn secondary', onClick: function(){ closeModal(); resolve(null); } },
            { label: 'Verify', className: 'btn', onClick: function(){
              var code = document.getElementById('tvaMfaCode');
              resolve(code ? String(code.value || '').trim() : '');
            } }
          ],
          onCerrar: function(){ closeModal(); resolve(null); }
        });
    ...
    async function ensureMfaAal2IfNeeded(client){
      try{
        ...
        if (cur === 'aal1' && next === 'aal2'){
          ...
          if (!factor || !factor.id){
            setMessage('La 2FA está activada pero no se encontró ningún factor TOTP.', 'error');
            return false;
          }

          while (true){
            var code = await promptTotpCodeModal({ title: 'Verificar 2FA' });
            if (code === null){
            ...
            if (!/^\d{6}$/.test(code)){
              ...
              if (errEl) errEl.textContent = 'Introduce un código válido de 6 dígitos.';
              continue;
            }
            ...
            if (client.auth.mfa.challengeAndVerify){
              ...
              if (!ok){
                ...
                if (e1) e1.textContent = (v1 && v1.error && v1.error.message) ? v1.error.message : 'Código no válido.';
              }
            }else{
              var ch = await client.auth.mfa.challenge({ factorId: factor.id });
              if (ch && ch.error){
                var e2 = document.getElementById('tvaMfaErr');
                if (e2) e2.textContent = ch.error.message || 'No se pudo crear el desafío.';
                continue;
              }
              ...
              if (!ok){
                ...
                if (e3) e3.textContent = (v2 && v2.error && v2.error.message) ? v2.error.message : 'Código no válido.';
              }
            }
    ...
    async function open2faSettings(){
      ...
      if (!user){
        showToast('Debes iniciar sesión como admin para gestionar la 2FA.', 'error');
        return;
      }
      ...
      if (verifiedFactor && verifiedFactor.id){
        openModal({
          title: 'Security (2FA)',
          bodyHtml: '<p class="tva-modal__subtitle">La 2FA está activada. Para desactivarla, confirma con un código de tu app.</p>',
          actions: [
            { label: 'Cerrar', className: 'btn btn-outline', onClick: closeModal },
            { label: 'Desactivar 2FA', className: 'btn btn-danger', onClick: async function(){
                var ok = await promptAndVerifyTotpCode(client, verifiedFactor.id, 'Desactivar 2FA');
                if (!ok) return;

                var un = await client.auth.mfa.unenroll({ factorId: verifiedFactor.id });
                if (un.error){
                  showToast(un.error.message || 'Error al desactivar 2FA', 'error');
                  return;
                }
                closeModal();
      ...
      openModal({
        title: 'Security (2FA)',
        bodyHtml: '<p class="tva-modal__subtitle">Protege el acceso de admin con un segundo factor (Authenticator).</p>',
        actions: [
          { label: 'Cerrar', className: 'btn btn-outline', onClick: closeModal },
          { label: 'Activar 2FA', className: 'btn btn-primary', onClick: async function(){

              // Ensure no verified factor already
              var existing = await getVerifiedTotpFactor(client);
              if (existing && existing.id){
                showToast('La 2FA ya está activada.', 'success');
                open2faSettings();
                return;
              }
              ...
              if (en.error){
                showToast(en.error.message || 'Error al activar 2FA', 'error');
                return;
              }
              ...
              openModal({
                ...
                bodyHtml:
                  '<div class="tva-modal__stack">'+
                    '<p class="tva-modal__subtitle">1) Escanea el código QR con Google Authenticator / Authy / Microsoft Authenticator.</p>'+
                    '<div class="mfa-qr" id="tvaMfaQrWrap"></div>'+
                    '<p class="tva-muted" id="tvaMfaSecretWrap" style="margin-top:10px; display:none;">Si no puedes escanear: <code style="user-select:all; word-break:break-all;" id="tvaMfaSecret"></code></p>'+
                    '<p class="tva-modal__subtitle" style="margin-top:12px;">2) Introduce el código generado:</p>'+
                    '<div class="tva-modal__field">'+
                      '<label class="tva-modal__label">Código 2FA</label>'+
                ...
                actions: [
                  ...
                  { label: 'Verificar y activar', className: 'btn btn-primary', onClick: async function(){
                      ...
                      if (!code){
                        if (errEl){ errEl.style.display = 'block'; errEl.textContent = 'Introduce el código 2FA.'; }
                        return;
                      }

                      var ok = await verifyTotpCode(client, factorId, code);
                      if (!ok){
                        if (errEl){ errEl.style.display = 'block'; errEl.textContent = 'Código no válido. Inténtalo de nuevo.'; }
                        return;
                      }

                      closeModal();
                      showToast('¡2FA activada!', 'success');
                    } }
                ],
                onCerrar: cleanupEnroll
              });
                ...
                try{
                  ...
                  if (qrWrap){
                    ...
                    if (qrUrl){
                      ...
                      p.textContent = 'QR no disponible. Usa la clave de abajo.';
    ...
    function setAdminTabsVisibility(show){
      Array.prototype.slice.call(document.querySelectorAll('.if-admin-auth-tabs')).forEach(function(el){
    ...
    function showDashboard(show){
      ...
      document.body.classList.toggle('if-admin-authenticated', !!show);
      setAdminTabsVisibility(!!show);
    }
    ...
    function renderChart(monthly){
      ...
      var affDatasetLabel = 'Affiliate earnings';
      try{
        var affCardLabel = elTotalAff && elTotalAff.previousElementSibling ? String(elTotalAff.previousElementSibling.textContent || '').trim() : '';
        if (affCardLabel) affDatasetLabel = affCardLabel.replace(/\s*\([^)]*\)\s*$/,'');
      }catch(e){}
      ...
      state.chart = new Chart(ctx, {
        ...
        data: {
          labels: labels,
          datasets: [
            { label: 'Pagos de afiliados', data: affValues, tension: 0.25, fill: false },
            { label: 'Margen Travirae', data: travValues, tension: 0.25, fill: false },
          ]
        },
        options: {
          responsive: true,
  ...
  try{
    ...
    try{
      setNewsletterStatus('Error al cargar la newsletter: ' + (err?.message || String(err)), 'error');
    }catch(_){}
  }
}
  ...
  form.addEventListener('submit', async (e)=>{
    ...
    if(missing.length){
      const msg = 'Rellena todos los campos para continuar.';
...
`+
    ...
    try{
      ...
      setNewsletterStatus('Enviando...', 'info');
...
Totale: ${total} · Inviate: ${sent} · Errori: ${failed}${extra}`, failed ? 'warn' : 'success');
      ...
      setNewsletterStatus('Error al enviar la newsletter: ' + (err?.message || String(err)), 'error');
    }finally{
      if(sendBtn){ sendBtn.disabled = false; sendBtn.textContent = 'Enviar newsletter'; }
    }
  });
    ...
    function buildPreviewEmailHtml(language, subject, bodyRaw){
      ...
      const footer = `
        ...
        <p style="font-size:12px;color:#6b7280;margin:0;">
          If you no longer wish to receive these emails, you can <a href="${safeUnsub}" target="_blank" rel="noopener">unsubscribe</a>.
        </p>
      `;
...
<html>
  ...
  <body style="margin:0;padding:16px;font-family:system-ui,-apple-system,Segoe UI,Roboto,Ubuntu,Cantarell,Noto Sans,Arial;">
    ...
    function updatePreview(){
      ...
      subjectPreviewEl.textContent = subject || '(sin asunto)';
    ...
    const clearBtn = q('#admin-newsletter-clear-btn');
      if(clearBtn){
        clearBtn.addEventListener('click', () => {
          ...
          setNewsletterStatus('Borradores eliminados.', 'success');
          updatePreview();
        });
      }
...
function clearTbody(tbody){
    ...
    function renderMonthlyTable(rows){
      ...
      if (!rows || !rows.length){
        ...
        td0.textContent = 'No hay datos disponibles.';
        tr0.appendChild(td0);
    ...
    function renderAffiliatesSummaryTable(rows){
      ...
      rows.forEach(function(r){
        ...
        if (slugLabel === 'direct') slugLabel = 'Tráfico directo (sin afiliado)';
    ...
    function renderOperationalTable(rows){
      ...
      rows.forEach(function(r){
        ...
        tr.appendChild(td(formatUsd(r.net_commissions_month || 0)));
    ...
    function renderPayoutRequestsTable(rows){
      ...
      if (!rows || !rows.length){
        ...
        td0.colSpan = 7;
        td0.className = 'muted small';
        td0.textContent = 'No hay solicitudes de pago.';
        tr0.appendChild(td0);
      ...
      rows.forEach(function(r){
        ...
        if (s === 'pending'){
          actionsCell.appendChild(makeAction('Approve', 'approved'));
          actionsCell.appendChild(makeAction('Reject', 'rejected'));
          actionsCell.appendChild(makeAction('Marcar como pagado', 'paid'));
        } else if (s === 'approved'){
          actionsCell.appendChild(makeAction('Marcar como pagado', 'paid'));
          actionsCell.appendChild(makeAction('Reject', 'rejected'));
        } else {
    ...
    async function updatePayoutRequestStatus(id, newStatus){
      ...
      var note = window.prompt('Admin note (optional):', '') || '';
      ...
      setMessage('Actualizando solicitud...', null);
      try{
        var resp = await client.from('payout_requests').update(patch).eq('id', id);
        if (resp.error){
          console.error(resp.error);
          setMessage('Error al actualizar la solicitud: ' + (resp.error.message || 'unknown'), 'error');
          return;
        }
        setMessage('Solicitud actualizada.', 'success');
        await loadAll();
      }catch(e){
        console.error(e);
        setMessage('Error inesperado al actualizar la solicitud.', 'error');
      }
    }
    async function loadSiteTraffic(){
      ...
      try{
        ...
        safeSetText(elTrafficUniqueVisitors, String(d.unique_visitors_month ?? '0'));
        ...
        state.dashboardMetrics.traffic = {
          visitsMonth: Number(d.visits_month || 0),
          uniqueVisitorsMonth: Number(d.unique_visitors_month || 0),
          visitsAffiliateMonth: Number(d.visits_affiliate_month || 0),
          visitsDirectMonth: Number(d.visits_direct_month || 0)
        };

        return state.dashboardMetrics.traffic;
      }catch(e){
    ...
    async function loadAggregates(){
      ...
      // Monthly aggregate (from monthly_affiliate_stats live view)
      var resp = await client
      ...
      if (resp.error){
        console.warn('Error loading monthly_affiliate_payouts', resp.error);
      ...
      state.dashboardMetrics.aggregates = {
    ...
    function updateQualityKpis(){
      var ag = state.dashboardMetrics && state.dashboardMetrics.aggregates ? state.dashboardMetrics.aggregates : null;
      var tr = state.dashboardMetrics && state.dashboardMetrics.traffic ? state.dashboardMetrics.traffic : null;
      if (!ag || !tr) return;

      var totalSales = Number(ag.monthSales || 0);
      var totalNet = Number(ag.monthNet || 0);
      var directSales = Number(ag.directSales || 0);
      var affiliateSales = Math.max(0, totalSales - directSales);
      var affiliateVisits = Number(tr.visitsAffiliateMonth || 0);
      var totalVisits = Number(tr.visitsMonth || 0);
      var affiliateConv = affiliateVisits > 0 ? (affiliateSales / affiliateVisits) : 0;
      var totalConv = totalVisits > 0 ? (totalSales / totalVisits) : 0;
      var avgCommission = totalSales > 0 ? (totalNet / totalSales) : 0;

      safeSetText(elQualityAffiliateSales, String(affiliateSales));
      safeSetText(elQualityConvAffiliate, formatPercentRatio(affiliateConv));
      safeSetText(elQualityConvTotal, formatPercentRatio(totalConv));
      safeSetText(elQualityAvgCommission, formatUsd(avgCommission));
    }
    ...
    async function loadOperationalOverview(){
      ...
      if (resp.error){
        console.warn('Error loading admin_affiliate_overview', resp.error);
        return;
      }
    ...
    async function loadPayoutRequests(){
      ...
      if (resp.error){
        console.warn('Error loading payout_requests', resp.error);
        return;
      }
    ...
    async function runImportTravelpayouts(){
      ...
      setMessage('Importación en curso (Aviasales)...', null);
      try{
        // days = 45 default
        var resp = await client.functions.invoke('import-travelpayouts', { body: { days: 45, campaign_id: 100 } });
        if (resp.error){
          console.error(resp.error);
          setMessage('Error de importación: ' + (resp.error.message || 'unknown'), 'error');
          return;
        }
        setMessage('Importación completada. Actualizando el panel...', 'success');
        await loadAll();
      }catch(e){
        console.error(e);
        setMessage('Error inesperado durante la importación.', 'error');
      }
    }
    ...
    async function runRefreshPayouts(){
      ...
      setMessage('Actualizando pagos...', null);
      try{
        var resp = await client.rpc('refresh_monthly_affiliate_payouts');
        if (resp && resp.error){
          setMessage('Error al actualizar: ' + (resp.error.message || 'unknown'), 'error');
          return;
        }
        setMessage('Pagos actualizados.', 'success');
        await loadAll();
      }catch(e){
        console.error(e);
        setMessage('Error inesperado durante la actualización.', 'error');
      }
    }
    ...
    async function importStay22CSV(file){
      ...
      if (!parsed.rows.length){
        setMessage('CSV vacío o no válido.', 'error');
        return;
      }
      ...
      async function resolveStay22Campaign(rawCampaign){
        var code = String(rawCampaign || '').trim();
        if (!code) {
          return { affiliate_slug: 'direct', match_type: 'direct', creator_slug: null, metadata: { raw_partner_campaign: null } };
        }
        try {
          var legacy = await client.from('affiliates').select('slug').eq('slug', code).maybeSingle();
          if (legacy && legacy.data && legacy.data.slug) {
            return { affiliate_slug: legacy.data.slug, match_type: 'legacy_affiliate', creator_slug: legacy.data.slug, metadata: { raw_partner_campaign: code } };
          }
        } catch(_legacyErr) {}
        try {
          var rpc = await client.rpc('resolve_influencer_tracking_code', { p_tracking_code: code });
          var row = Array.isArray(rpc && rpc.data) ? rpc.data[0] : (rpc && rpc.data ? rpc.data : null);
          if (row && row.affiliate_slug) {
            return {
              affiliate_slug: row.affiliate_slug,
              match_type: row.match_type || 'influencer_widget',
              creator_slug: row.creator_slug || row.affiliate_slug,
              post_id: row.post_id || null,
              post_slug: row.post_slug || null,
              widget_id: row.widget_id || null,
              widget_short_id: row.widget_short_id || null,
              widget_tracking_code: row.tracking_code || code,
              partner: row.partner || 'stay22',
              metadata: row.metadata || {}
            };
          }
        } catch(_rpcErr) {}
        return { affiliate_slug: code, match_type: 'fallback_raw', creator_slug: code, metadata: { raw_partner_campaign: code } };
      }
      var rows = [];
      for (var rowIndex = 0; rowIndex < parsed.rows.length; rowIndex++){
        var r = parsed.rows[rowIndex];
        var rawCampaign = String(pick(r, ['affiliate_slug','campaign id','campaign','campaign_id'])).trim();
        var resolvedCampaign = await resolveStay22Campaign(rawCampaign);
        var affiliateSlug = String(resolvedCampaign.affiliate_slug || '').trim();
        ...
        rows.push({
          ...
          metadata: Object.assign({}, r, {
            source: resolvedCampaign.match_type === 'influencer_widget' ? 'influencer_post' : (r.source || null),
            creator_slug: resolvedCampaign.creator_slug || null,
            post_id: resolvedCampaign.post_id || null,
            post_slug: resolvedCampaign.post_slug || null,
            widget_id: resolvedCampaign.widget_id || null,
            widget_short_id: resolvedCampaign.widget_short_id || null,
            widget_tracking_code: resolvedCampaign.widget_tracking_code || (rawCampaign || null),
            raw_partner_campaign: rawCampaign || null,
            tracking_match_type: resolvedCampaign.match_type || null,
            resolved_tracking_metadata: resolvedCampaign.metadata || {}
          })
        });
      }
      ...
      if (!rows.length){
        setMessage('CSV no compatible: no se encontraron filas válidas. Asegúrate de que el CSV incluya al menos Campaign ID (o affiliate_slug) y Transaction ID (o partner_reference).', 'error');
        return;
      }
      setMessage('Importando CSV de Stay22: upsert en curso...', null);
      ...
      for (var i=0;i<rows.length;i+=chunkSize){
        ...
        if (up.error){
          ...
          setMessage('Error de importación CSV: ' + (up.error.message || 'unknown'), 'error');
          return;
        }
      }
      ...
      setMessage('CSV importado. Panel actualizado.', 'success');
      await loadAll();
    }
    ...
    function startRealtime(){
      ...
      try{
        ...
        console.warn('Realtime not available or not configured', e);
      }
    }
    ...
    async function initSession(){
      ...
      try{
        ...
        if (resp && resp.data && resp.data.user && isAdminEmail(resp.data.user.email)){
          ...
          // 2FA (TOTP): se attiva per l'admin, chiedi il codice prima di mostrare la dashboard
    ...
    if (loginForm){
      loginForm.addEventListener('submit', async function(e){
        ...
        if (!client || !client.auth){
          setMessage('Supabase Auth no disponible.', 'error');
          return;
        }
        ...
        try{
          ...
          if (res.error){
            setMessage(res.error.message || 'Error de inicio de sesión.', 'error');
            return;
          }
          ...
          if (!u || !isAdminEmail(u.email)){
            ...
            setMessage('Esta cuenta no es una cuenta de administrador.', 'error');
            return;
          }
          state.user = u;
          ...
          showDashboard(true);
          setMessage('Inicio de sesión de administrador correcto.', 'success');
          await loadAll();
          startRealtime();
          startAutoRefresh();
        }catch(err){
          console.error(err);
          setMessage('Error inesperado durante el inicio de sesión.', 'error');
        }
      });
    }
