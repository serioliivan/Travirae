  ...
  function setMapFor(btn){
    ...
    loadMapping().then(function(map){
      ...
      if(src){
        ...
        // Inject Stay22 campaign from affiliate ID if disponibile
        try {
          ...
          if (affId && src) {
            try {
              var urlStay = new URL(src);
              urlStay.searchParams.set('campaign', affId);
              src = urlStay.toString();
            } catch (eUrlStay) {
              if (src.indexOf('?') === -1) src += '?campaign=' + encodeURIComponent(affId);
              else src += '&campaign=' + encodeURIComponent(affId);
            }
            // Traccia il click verso Stay22 per questa mappa dinamica
            try {
              ...
              if (window.console && console.warn) console.warn('Travirae: errore tracking booking Stay22 (mappe dinamiche)', eTrackStay);
            }
          }
        } catch (eAffStay) {
          if (window.console && console.warn) console.warn('Travirae: errore impostazione campaign Stay22', eAffStay);
        }
