...
(function(){
  // Widget locale (2-letter). We keep it aligned with the site's language selector.
  function getWidgetLocale(){
    const code = String(window.TRAVIRAE_LANG || localStorage.getItem('travirae_lang') || 'ENG').toUpperCase();
    const map = {
      ITA: 'it',
      ENG: 'en',
      FRA: 'fr',
  ...
  function setMapFor(btn){
    ...
    loadMapping().then(function(map){
      ...
      if(src){
        ...
        // --- Locale + tracking params
        // 1) Forziamo la lingua del widget in base alla lingua del sito.
        var locale = getWidgetLocale();
        src = addParams(src, { lang: locale, language: locale, locale: locale });

        // 2) Aggiungi campaign=AFFILIATE_SLUG alla src (se presente un affiliate id)
        var aff = '';
        try {
        ...
        if (aff) src = addParams(src, { campaign: aff });
