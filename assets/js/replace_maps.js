  ...
  function setMapFor(btn){
    ...
    loadMapping().then(function(map){
      ...
      if(src){
        ...
        if(city) frame.setAttribute('title', (getCurrentLangCode() === 'DEU' ? 'Hotelkarte in ' : 'Mappa hotel a ') + city);
        ...
        try {
          ...
          ov.setAttribute('aria-label', (getCurrentLangCode() === 'DEU') ? 'Karte aktivieren' : 'Attiva la mappa');
        ...
        p.textContent = (getCurrentLangCode() === 'DEU') ? 'Karte ist momentan nicht verfügbar.' : 'Mappa non disponibile al momento.';
        mount.appendChild(p);
      }
    });
    lastBtn = btn;
