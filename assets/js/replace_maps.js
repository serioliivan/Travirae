...
(function(){
  ...
  document.addEventListener('click', function(e){
    ...
    if(btn){
      ...
      // conteggiamo un booking click verso Stay22 al momento dell'apertura.
      try {
        if (window.traviraeAffiliate && typeof window.traviraeAffiliate.trackBookingClick === 'function') {
          if (window.console && console.log) console.log('Travirae: apertura mappa Stay22 (destinazione) - booking click');
          window.traviraeAffiliate.trackBookingClick('stay22');
        }
      } catch(eTrack) {
        if (window.console && console.warn) console.warn('Travirae: errore tracking booking Stay22 (open destination map)', eTrack);
      }
      setMapFor(btn);
    }
  }, true);
