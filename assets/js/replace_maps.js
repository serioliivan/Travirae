    ...
    loadMapping().then(function(map){
      ...
      if(src){
        ...
        try {
          ...
          var hideTimer = 0;
          var consume = function(ev){
            if (!ev) return;
            try { if (typeof ev.preventDefault === 'function') ev.preventDefault(); } catch(ePrev){}
            try { if (typeof ev.stopPropagation === 'function') ev.stopPropagation(); } catch(eStop){}
            try { if (typeof ev.stopImmediatePropagation === 'function') ev.stopImmediatePropagation(); } catch(eStopImm){}
          };
          var hide = function(){
            try { if (hideTimer) { clearTimeout(hideTimer); hideTimer = 0; } } catch(eTimer){}
          ...
          var scheduleHide = function(delay){
            try { if (hideTimer) clearTimeout(hideTimer); } catch(eTimerClear){}
          ...
          var fire = function(ev){
            ...
            try {
              ...
              // Failsafe soltanto: non sbloccare durante lo stesso gesto.
              scheduleHide(1600);
            }
          };
          ...
          var release = function(ev){
            ...
            try {
              ...
              var delay = (type === 'click' || type === 'touchcancel' || type === 'pointercancel') ? 0 : 450;
              scheduleHide(delay);
            }
          };
          ...
          ov.addEventListener('touchstart', fire, { capture: true, passive: false });
          ov.addEventListener('pointerup', release, true);
          ov.addEventListener('mouseup', release, true);
          ov.addEventListener('pointercancel', release, true);
          ov.addEventListener('click', release, true);
          ov.addEventListener('touchend', release, { capture: true, passive: false });
          ov.addEventListener('touchcancel', release, { capture: true, passive: false });
        } catch(eOverlay) {
