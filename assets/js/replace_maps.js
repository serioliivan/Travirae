  ...
  function setMapFor(btn){
    ...
    loadMapping().then(function(map){
      ...
      if(src){
        ...
        // Lo attiviamo SOLO se esiste un affiliate id (aff).
        try {
          if (aff) {
            ...
            ov.style.cssText = 'position:absolute; inset:0; z-index:5; display:flex; align-items:center; justify-content:center; cursor:pointer; background:rgba(255,255,255,0.01);';
            ov.innerHTML = '<div style="background:#fff; border:1px solid rgba(0,0,0,.08); padding:12px 14px; border-radius:12px; box-shadow:0 10px 24px rgba(0,0,0,.08); text-align:center; max-width:280px;"><strong>Attiva la mappa</strong><div class="muted" style="font-size:13px; margin-top:4px;">Clicca una volta per tracciare il booking.</div></div>';
            mount.appendChild(ov);

            var hide = function(){
              try { ov.style.pointerEvents = 'none'; ov.style.opacity = '0'; } catch(eCss){}
              setTimeout(function(){ try { ov.remove(); } catch(eRm){ ov.style.display = 'none'; } }, 250);
            };
