...
function openModal(opts){
    ...
    async function ensureMfaAal2IfNeeded(client, opts = {}){
      try{
        ...
        showToast('تم التحقق من 2FA.', 'success');
        return true;
      }catch(e){
        console.warn(e);
        return true;
...
// Richiede una re‑autenticazione (password) e, se presente, anche un codice 2FA.
	              ...
	              buttons: [
	                ...
	                },
	                {
	                  ...
	                  onClick: async function(id){
	                    ...
	                    pw = pw.trim();
	                    if (!pw){
	                      if (errEl){ errEl.textContent = 'أدخل كلمة المرور.'; errEl.style.display = 'block'; }
	                      return;
	                    }
	                    // small loading lock
	                    ...
	                    try{
	                      ...
	                      if (si && si.error){
	                        if (errEl){ errEl.textContent = 'كلمة المرور غير صحيحة.'; errEl.style.display = 'block'; }
	                        if (modal){
            ...
            },
            {
              ...
              onClick: async function(){
                ...
                try{
                  ...
                  if (errEl){ errEl.textContent = (err && err.message) ? err.message : 'رمز غير صالح. حاول مرة أخرى.'; errEl.style.display = 'block'; }
                }
              }
            }
          ]
    ...
    async function promptTotpCodeModal(promptTitle){
      promptTitle = promptTitle || 'التحقق من 2FA';
      return new Promise(function(resolve){
        var done = false;
        function finish(val){
          if (done) return;
        ...
        openModal({
          title: promptTitle,
          bodyHtml: ''+
            '<p class="tva-modal__subtitle">أدخل الرمز المكون من 6 أرقام الذي ينشئه تطبيق المصادقة لديك.</p>'+
            '<div class="tva-field">'+
              '<label class="tva-label">رمز 2FA</label>'+
            ...
            '</div>',
          buttons: [
            { label: 'إلغاء', className: 'btn btn-outline', onClick: function(){ closeModal(); finish(null); } },
            { label: 'تحقق', className: 'btn btn-primary', onClick: function(){
    ...
    async function open2faSettings(){
      ...
      if (!user){
        showToast('يجب تسجيل الدخول لإدارة 2FA.', 'error');
        return;
      }
      ...
      openModal({
        title: 'الأمان (2FA)',
        bodyHtml: '<p class="tva-modal__subtitle">أضف مستوى أمان ثانٍ إلى حسابك (Authenticator).</p>',
        buttons: [
          { label: 'إغلاق', className: 'btn btn-outline', onClick: closeModal },
              ...
              openModal({
                ...
                bodyHtml:
                  '<div class="tva-modal__stack">'+
                    '<div style="margin:10px 0 14px; padding:12px; border:1px solid #fde68a; background:#fffbeb; border-radius:12px; font-size:13px; line-height:1.35; color:#111;"><strong>مهم:</strong> إذا فقدت الوصول إلى تطبيق المصادقة، فقد لا تتمكن من <strong>تسجيل الدخول</strong> إلى حسابك وقد لا تتمكن من <strong>إعادة تعيين كلمة المرور</strong> (“نسيت كلمة المرور”) حتى تستعيد الوصول إلى 2FA. احرص على الاحتفاظ بالوصول إلى الجهاز/التطبيق وفكّر في إعداد 2FA على جهاز ثانٍ أيضًا.</div>'+
                    '<p class="tva-modal__subtitle">1) امسح رمز QR باستخدام تطبيق المصادقة.</p>'+
                    '<div class="mfa-qr" id="tvaMfaQrWrap"></div>'+
                    '<p class="tva-muted" id="tvaMfaSecretWrap" style="margin-top:10px; display:none;">إذا لم تتمكن من المسح: <code style="user-select:all; word-break:break-all;" id="tvaMfaSecret"></code></p>'+
                ...
                buttons: [
                  ...
                  { label: 'تحقق وتفعيل', className: 'btn btn-primary', onClick: async function(){
                      ...
                      closeModal();
                      showToast('تم تفعيل 2FA! تذكّر: لا تفقد الوصول إلى تطبيق المصادقة، وإلا فقد لا تتمكن من استعادة حسابك.', 'success');
                    } }
                ],
    ...
    function openChangeLangModal(){
      var client = refreshClient();
      if (!client){
        showToast('Supabase غير جاهز. حاول مرة أخرى بعد بضع ثوانٍ.', 'error');
        return;
      }
      ...
      client.auth.getUser().then(function(res){
        ...
        if (!user){
          showToast('يجب تسجيل الدخول لتغيير اللغة.', 'error');
          return;
        }
        ...
        openModal({
          title: 'تغيير اللغة',
          bodyHtml:
            '<p class="tva-modal__subtitle">اختر اللغة التي ترغب في استلام رسائل Travirae بها.</p>' +
            '<div class="tva-field">' +
              '<label class="tva-label" for="tvaLangSelect">اللغة</label>' +
              '<select class="tva-input" id="tvaLangSelect">' + optionsHtml + '</select>' +
              '<div class="tva-error" id="tvaLangError" style="display:none;"></div>' +
            '</div>',
          buttons: [
            { label: 'إلغاء', className: 'btn btn-outline', onClick: function(){ closeModal(); } },
            { label: 'حفظ', className: 'btn btn-primary', onClick: async function(){
              var selEl = document.getElementById('tvaLangSelect');
              var errEl = document.getElementById('tvaLangError');
              var next = selEl ? selEl.value : '';
              if (errEl){ errEl.style.display = 'none'; errEl.textContent = ''; }
              if (!next){
                if (errEl){ errEl.textContent = 'اختر لغة للمتابعة.'; errEl.style.display = 'block'; }
                return;
              }
              if (supported.indexOf(next) === -1){
                if (errEl){ errEl.textContent = 'لغة غير صالحة.'; errEl.style.display = 'block'; }
                return;
              }
              ...
              try{
                ...
                closeModal();
                showToast('تم تحديث اللغة بنجاح.', 'success');
              }catch(e){
                console.warn(e);
                showToast((e && e.message) ? e.message : 'تعذر تغيير اللغة.', 'error');
              }
            } }
          ]
        });
        ...
        console.warn(e);
        showToast('تعذر جلب ملف المستخدم.', 'error');
      });
    }
