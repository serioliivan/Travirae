...
(function(){
  ...
  var WIDGET_AUTOCOMPLETE = { loaded: false, loading: null, stay22: [], airports: [], airportLookup: Object.create(null), remoteCache: Object.create(null) };
  ...
  function searchStay22DestinationsLocal(term, limit){
    ...
    if (!query) return [];
    var pool = POPULAR_STAY22_DESTINATIONS.concat(WIDGET_AUTOCOMPLETE.stay22 || []);
    return uniqueSuggestionItems(pool.map(function(label, index){
      var score = scoreAutocompleteMatch(query, label);
      if (score < 0) return null;
      return { label: String(label || '').trim(), value: String(label || '').trim(), score: score + Math.max(0, 120 - index) };
  ...
  function searchStay22Destinations(term, locale, limit){
    return ensureWidgetAutocompleteCatalog().then(function(){
      return fetchDestinationSuggestions(term, locale, limit).then(function(items){
        if (items && items.length) return items.slice(0, typeof limit === 'number' ? limit : 3);
        return searchStay22DestinationsLocal(term, limit);
      }, function(){
        return searchStay22DestinationsLocal(term, limit);
      });
    });
  ...
  function ensureWidgetAutocompleteCatalog(){
    ...
    WIDGET_AUTOCOMPLETE.loading = fetch('assets/data/widget-autocomplete.json', { cache: 'force-cache' })
  ...
  function renderWidgetCard(block, context){
    ...
    if (block.partner === 'stay22' && block.destination){
      embedHtml = '' +
        '<div class="if-widget-card__embed if-widget-card__embed--map">' +
          '<iframe loading="lazy" referrerpolicy="strict-origin-when-cross-origin" src="' + escapeAttr(buildStay22EmbedUrl({ trackingCode: trackingCode, destination: block.destination, checkin: block.checkin, checkout: block.checkout, locale: block.widgetLocale })) + '" title="' + escapeAttr(block.title || 'Hotel Map') + '"></iframe>' +
        '</div>';
      ...
      embedHtml = '' +
        '<div class="if-widget-card__embed if-widget-card__embed--aviasales">' +
          '<div class="if-aviasales-widget-host" data-if-aviasales-widget="1" data-widget-src="' + escapeAttr(aviaWidgetSrc) + '">' +
            '<div class="if-aviasales-widget-fallback" data-if-aviasales-fallback="1">' +
              ...
              '</div>' +
            '</div>' +
            '<div class="if-aviasales-widget-mount" data-if-aviasales-mount="1"></div>' +
          '</div>' +
        '</div>';
    }
    return '' +
