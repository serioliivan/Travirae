...
(function(){
  ...
  function initPublicPage(){
    ...
    function renderPagination(totalCount){
      ...
      var html = '' +
        '<button class="btn secondary if-pagination-arrow" type="button" aria-label="' + escapeHtml(t('previous')) + '" data-page="' + (state.currentPage - 1) + '"' + (state.currentPage <= 1 ? ' disabled' : '') + '>←</button>' +
        '<span class="muted small">' + escapeHtml(t('page')) + ' ' + escapeHtml(String(state.currentPage)) + ' / ' + escapeHtml(String(totalPages)) + '</span>' +
        '<button class="btn secondary if-pagination-arrow" type="button" aria-label="' + escapeHtml(t('next')) + '" data-page="' + (state.currentPage + 1) + '"' + (state.currentPage >= totalPages ? ' disabled' : '') + '>→</button>';
