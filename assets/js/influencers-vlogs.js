...
(function(){
  ...
  function bindWidgetTouchGuard(root){
    ...
    if (!modalBody.getAttribute('data-widget-guard-root')){
      modalBody.setAttribute('data-widget-guard-root', '1');
      ['touchstart','pointerdown','mousedown'].forEach(function(evt){
        modalBody.addEventListener(evt, function(e){
          var target = e.target;
          if (target && target.closest && target.closest('[data-widget-interactive="1"], .if-widget-card__embed, .if-aviasales-widget-host')) return;
          releaseLock(0);
        }, { passive: true });
      });
      modalBody.addEventListener('scroll', function(){ releaseLock(0); }, { passive: true });
    }
    var selector = '[data-widget-interactive="1"], .if-widget-card__embed, .if-aviasales-widget-host, .if-widget-card__embed iframe';
    $all(selector, scope).forEach(function(el){
      ...
      ['touchend','touchcancel','pointerup','pointercancel','mouseleave','mouseup','blur'].forEach(function(evt){
        el.addEventListener(evt, function(){ releaseLock(220); }, { passive: true });
      });
    });
  }
