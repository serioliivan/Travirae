  ...
  function initPublicPage(){
    ...
    function renderPagination(totalCount){
      ...
      var html = '' +
        ...
        '<span class="if-pagination-status muted small">' + escapeHtml(String(state.currentPage)) + ' / ' + escapeHtml(String(totalPages)) + '</span>' +
    ...
    function ensureSearchUi(){
      ...
      if (existing) {
        ...
        var label = existing.querySelector('label');
        if (label) label.textContent = t('searchPosts');
        if (input){
      ...
      row.innerHTML = '<div class="form-group if-search-field"><label for="if-post-search">' + escapeHtml(t('searchPosts')) + '</label><input id="if-post-search" type="search" placeholder="' + escapeAttr(t('searchPlaceholder')) + '" autocomplete="off"/></div>';
