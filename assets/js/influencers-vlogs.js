...
(function(){
  ...
  var DEFAULTS = {
    ...
    blockStay22: 'Widget Stay22',
    blockAviasales: 'Widget Aviasales',
    newPost: 'Nuovo post',
    ...
    widgetStay22Action: 'Apri ricerca Stay22',
    widgetAviasalesAction: 'Apri ricerca Aviasales',
  ...
  function normalizeContentBlocks(blocks, context){
    ...
    (Array.isArray(blocks) ? blocks : []).forEach(function(raw){
      ...
      if (block.type === 'heading'){
        ...
        block.title = String(block.title || (block.partner === 'stay22' ? 'Stay22' : 'Aviasales')).trim();
    ...
    return '' +
      '<section class="if-widget-card if-widget-card--' + escapeAttr(block.partner) + '" data-widget-wrap="1">' +
        '<div class="if-widget-card__head">' +
          '<span class="chip">' + escapeHtml(block.partner === 'stay22' ? 'Stay22' : 'Aviasales / Travelpayouts') + '</span>' +
          '<span class="muted small">' + escapeHtml(t('shareablePopup')) + '</span>' +
        '</div>' +
        '<h3>' + escapeHtml(block.title || (block.partner === 'stay22' ? 'Stay22' : 'Aviasales')) + '</h3>' +
  ...
  function defaultBlock(type){
    ...
    if (type === 'stay22') return { id: randomId('blk_'), type: 'stay22', widgetShortId: shortId('w'), title: 'Stay22', description: '', destination: '', checkin: '', checkout: '', ctaLabel: t('widgetStay22Action') };
    if (type === 'aviasales') return { id: randomId('blk_'), type: 'aviasales', widgetShortId: shortId('w'), title: 'Aviasales / Travelpayouts', description: '', origin: '', destination: '', checkin: '', returnDate: '', ctaLabel: t('widgetAviasalesAction') };
