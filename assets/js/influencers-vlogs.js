...
(function(){
  ...
  // Aviasales auto-determines the origin from the visitor IP when the origin parameter is omitted.
  // This invisible sentinel keeps the official Aviasales widget mounted while making the origin field start blank.
  var AVIASALES_EMPTY_ORIGIN_SENTINEL = '\u200B';
  var DEFAULTS = {
  ...
  function buildAviasalesWidgetScriptUrl(config){
    ...
    if (originCode){
      ...
      // Do not omit origin: the official Aviasales search form otherwise fills it from the visitor IP.
      // A zero-width value is truthy for the widget config but renders visually blank.
      url.searchParams.set('origin', AVIASALES_EMPTY_ORIGIN_SENTINEL);
      url.searchParams.set('origin_iata', AVIASALES_EMPTY_ORIGIN_SENTINEL);
    }
    if (destinationCode){
  ...
  function replaceLegacyEmptyOriginAviasalesWidgets(root){
    // Kept for backward compatibility with already rendered DOM fragments.
    // Empty-origin flight blocks must remain the official Aviasales widget, not a local replica.
  ...
  function clearAviasalesSentinelValue(value){
    return String(value || '').replace(/[\u200B\u200C\u200D\u2060]/g, '').trim();
  }
  function looksLikeAutoFilledOriginLabel(text, autoOriginText){
    var value = clearAviasalesSentinelValue(text);
    var autoValue = clearAviasalesSentinelValue(autoOriginText);
    if (!value) return true;
    if (autoValue && value.toLowerCase() === autoValue.toLowerCase()) return true;
    if (/^[A-Z]{3}$/.test(value)) return true;
    return false;
  }
  function bindAviasalesEmptyOriginUserInput(host, input){
    if (!host || !input || input.__ifEmptyOriginUserInputBound) return;
    input.__ifEmptyOriginUserInputBound = true;
    function markUserInput(){ host.__ifAviasalesOriginUserEdited = true; }
    input.addEventListener('keydown', markUserInput, true);
    input.addEventListener('paste', markUserInput, true);
    input.addEventListener('drop', markUserInput, true);
    input.addEventListener('input', function(e){
      if (!host.__ifClearingEmptyOrigin && e && e.isTrusted) markUserInput();
    }, true);
  }
  function clearAviasalesGeneratedOrigin(host){
    ...
    if (!mount) return;
    var controls = $all('input, textarea', mount).filter(isVisibleFormControl);
    var originInput = controls.length ? controls[0] : null;
    if (originInput) bindAviasalesEmptyOriginUserInput(host, originInput);
    if (host.__ifAviasalesOriginUserEdited) return;
    if (originInput){
      var autoOriginText = '';
      try{
        var current = clearAviasalesSentinelValue(originInput.value);
        autoOriginText = current;
        if (current){
          host.__ifClearingEmptyOrigin = true;
          originInput.value = '';
          ...
          window.setTimeout(function(){ host.__ifClearingEmptyOrigin = false; }, 0);
        }
        originInput.setAttribute('placeholder', originInput.getAttribute('placeholder') || '');
      }catch(_){ host.__ifClearingEmptyOrigin = false; }
      try{
        var container = originInput.parentElement;
        for (var depth = 0; container && depth < 5; depth += 1){
          var visibleFields = $all('input, textarea', container).filter(isVisibleFormControl);
          if (visibleFields.length > 1 && depth > 0) break;
          $all('span,small,strong,b,em,button,div', container).forEach(function(node){
            if (!node || node === container || node.contains(originInput)) return;
            var text = String(node.textContent || '').trim();
            if (looksLikeAutoFilledOriginLabel(text, autoOriginText)) node.textContent = '';
          });
          container = container.parentElement;
        }
      }catch(_){ }
    }
    // If the provider keeps the field inside accessible same-origin markup, remove only the invisible guard text.
    try{
      $all('input, textarea', mount).forEach(function(field){
        if (/[\u200B\u200C\u200D\u2060]/.test(String(field.value || ''))){
          field.value = clearAviasalesSentinelValue(field.value);
  ...
  function scheduleAviasalesOriginCleanup(host){
    if (!host || host.getAttribute('data-empty-origin') !== '1') return;
    [0, 80, 180, 360, 700, 1200, 2000, 3500, 5500].forEach(function(delay){
      window.setTimeout(function(){ clearAviasalesGeneratedOrigin(host); }, delay);
    });
    if (!host.__ifAviasalesOriginObserver){
      try{
        var mount = $('[data-if-aviasales-mount]', host) || host;
        host.__ifAviasalesOriginObserver = new MutationObserver(function(){ clearAviasalesGeneratedOrigin(host); });
        host.__ifAviasalesOriginObserver.observe(mount, { childList: true, subtree: true, characterData: true, attributes: true, attributeFilter: ['value'] });
      }catch(_){ host.__ifAviasalesOriginObserver = true; }
    }
  }
  ...
  function renderWidgetCard(block, context){
    ...
    if (block.partner === 'stay22' && block.destination){
      ...
      var emptyOriginAttr = hasFlightOrigin ? '' : ' data-empty-origin="1"';
      embedHtml = '' +
        '<div class="if-widget-card__embed if-widget-card__embed--aviasales" data-widget-interactive="1">' +
          '<div class="if-aviasales-widget-host" data-if-aviasales-widget="1" data-widget-src="' + escapeAttr(aviaWidgetSrc) + '"' + emptyOriginAttr + ' data-widget-interactive="1">' +
