    ...
    if (block.partner === 'stay22' && block.destination){
      embedHtml = '' +
        '<div class="if-widget-card__embed if-widget-card__embed--map">' +
          '<iframe loading="lazy" referrerpolicy="strict-origin-when-cross-origin" src="' + escapeAttr(buildStay22EmbedUrl({ trackingCode: trackingCode, destination: block.destination, checkin: block.checkin, checkout: block.checkout })) + '" title="' + escapeAttr(block.title || 'Hotel Map') + '"></iframe>' +
        '</div>';
    } else if (block.partner === 'aviasales'){
      embedHtml = '' +
        '<div class="if-flight-widget-preview">' +
          '<div class="if-flight-widget-preview__route">' +
            '<div class="if-flight-widget-preview__field"><span class="muted small">' + escapeHtml(String(block.origin || '—').toUpperCase()) + '</span></div>' +
            '<div class="if-flight-widget-preview__arrow">→</div>' +
            '<div class="if-flight-widget-preview__field"><span class="muted small">' + escapeHtml(String(block.destination || '—').toUpperCase()) + '</span></div>' +
          '</div>' +
