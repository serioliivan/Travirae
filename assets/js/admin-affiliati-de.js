  ...
  function init(){
    ...
    function ensureModal(){
      ...
      modalEl.innerHTML = [
        ...
        '    <button type="button" class="tva-modal__close" aria-label="Schließen" id="tvaModalClose">×</button>',
        '  </div>',
...
function openModal(opts){
    ...
    async function promptAndVerifyTotpCode(client, factorId, promptTitle){
      var code = await promptTotpCodeModal({ title: (promptTitle || '2FA verifizieren') });
      if (!code) return false;
      var ok = await verifyTotpCode(client, factorId, code);
      if (!ok) showToast('2FA-Code non valido.', 'error');
      return ok;
    }
    ...
    function promptTotpCodeModal(opts){
      ...
      return new Promise(function(resolve){
        openModal({
          title: opts.title || '2FA verifizieren',
          bodyHtml: [
            '<p class="tva-modal__hint">Gib den 6-stelligen Code aus deiner Authenticator-App ein.</p>',
          ...
          actions: [
            { label: 'Abbrechen', className: 'btn secondary', onClick: function(){ closeModal(); resolve(null); } },
            { label: 'Verifizieren', className: 'btn', onClick: function(){
    ...
    async function ensureMfaAal2IfNeeded(client){
      try{
        ...
        if (cur === 'aal1' && next === 'aal2'){
          ...
          if (!factor || !factor.id){
            setMessage('2FA ist aktiv, aber es wurde kein TOTP-Faktor gefunden.', 'error');
            return false;
          }

          while (true){
            var code = await promptTotpCodeModal({ title: '2FA verifizieren' });
            if (code === null){
            ...
            if (!/^\d{6}$/.test(code)){
              ...
              if (errEl) errEl.textContent = 'Bitte gib einen gültigen Code ein (6 Ziffern).';
              continue;
            }
            ...
            if (client.auth.mfa.challengeAndVerify){
              ...
              if (!ok){
                ...
                if (e1) e1.textContent = (v1 && v1.error && v1.error.message) ? v1.error.message : 'Ungültiger Code.';
              }
            }else{
              var ch = await client.auth.mfa.challenge({ factorId: factor.id });
              if (ch && ch.error){
                var e2 = document.getElementById('tvaMfaErr');
                if (e2) e2.textContent = ch.error.message || 'Challenge konnte nicht erstellt werden.';
                continue;
              }
              ...
              if (!ok){
                ...
                if (e3) e3.textContent = (v2 && v2.error && v2.error.message) ? v2.error.message : 'Ungültiger Code.';
              }
            }
    ...
    async function open2faSettings(){
      ...
      if (!user){
        showToast('Du musst als Admin angemeldet sein, um 2FA zu verwalten.', 'error');
        return;
      }
      ...
      if (verifiedFactor && verifiedFactor.id){
        openModal({
          ...
          bodyHtml: '<p class="tva-modal__subtitle">2FA ist aktiv. Um sie zu deaktivieren, bestätige mit einem Code aus deiner Authenticator-App.</p>',
          actions: [
      ...
      openModal({
        title: 'Sicherheit (2FA)',
        bodyHtml: '<p class="tva-modal__subtitle">Schütze den Admin-Zugang mit einem zweiten Faktor (Authenticator).</p>',
        actions: [
          { label: 'Schließen', className: 'btn btn-outline', onClick: closeModal },
          { label: '2FA aktivieren', className: 'btn btn-primary', onClick: async function(){
              ...
              if (existing && existing.id){
                showToast('2FA ist bereits aktiviert.', 'success');
                open2faSettings();
                return;
              }
              ...
              openModal({
                ...
                bodyHtml:
                  '<div class="tva-modal__stack">'+
                    '<p class="tva-modal__subtitle">1) Scanne den QR-Code mit Google Authenticator / Authy / Microsoft Authenticator.</p>'+
    ...
    function renderChart(monthly){
      ...
      state.chart = new Chart(ctx, {
        ...
        data: {
          labels: labels,
          datasets: [
            { label: 'Affiliate-Auszahlungen', data: affValues, tension: 0.25, fill: false },
  ...
  try{
    ...
    try{
      setNewsletterStatus('Fehler beim Laden der Newsletter-Daten: ' + (err?.message || String(err)), 'error');
    }catch(_){}
  }
}
  ...
  form.addEventListener('submit', async (e)=>{
    ...
    if(missing.length){
      const msg = 'Bitte fülle alle Felder aus, um fortzufahren.';
      setNewsletterStatus(`${msg} Fehlen: ${missing.join(', ')}`, 'error');
      try{
...
`+
      `Fallback: ITA
`+
      `Ausgefüllte Sprachen: ${totalFilled}/${NL_LANGS.length}

`+
      `Betreff (ITA): ${ita.subject}`
    );
    if(!ok) return;

    try{
      if(sendBtn){ sendBtn.disabled = true; sendBtn.textContent = 'Senden...'; }
      setNewsletterStatus('Versand läuft...', 'info');
...
Details: ${parts.join(' · ')}`;
      ...
      setNewsletterStatus(`Newsletter gesendet.
Gesamt: ${total} · Gesendet: ${sent} · Fehler: ${failed}${extra}`, failed ? 'warn' : 'success');

      // refresh stats
      await loadNewsletterStats();
    }catch(err){
      setNewsletterStatus('Fehler beim Newsletter-Versand: ' + (err?.message || String(err)), 'error');
    }finally{
      if(sendBtn){ sendBtn.disabled = false; sendBtn.textContent = 'Newsletter senden'; }
    }
  });
    ...
    const clearBtn = q('#admin-newsletter-clear-btn');
      if(clearBtn){
        clearBtn.addEventListener('click', () => {
          ...
          setNewsletterStatus('Entwürfe gelöscht.', 'success');
          updatePreview();
        });
      }
...
function clearTbody(tbody){
    ...
    function renderMonthlyTable(rows){
      ...
      if (!rows || !rows.length){
        ...
        td0.textContent = 'Keine Daten verfügbar.';
        tr0.appendChild(td0);
    ...
    function renderAffiliatesSummaryTable(rows){
      ...
      rows.forEach(function(r){
        ...
        if (slugLabel === 'direct') slugLabel = 'Direkter Traffic (ohne Affiliate)';
    ...
    function renderPayoutRequestsTable(rows){
      ...
      if (!rows || !rows.length){
        ...
        td0.colSpan = 7;
        td0.className = 'muted small';
        td0.textContent = 'Keine Auszahlungsanfragen.';
        tr0.appendChild(td0);
      ...
      rows.forEach(function(r){
        ...
        if (s === 'pending'){
          actionsCell.appendChild(makeAction('Genehmigen', 'approved'));
          actionsCell.appendChild(makeAction('Ablehnen', 'rejected'));
          actionsCell.appendChild(makeAction('Als bezahlt markieren', 'paid'));
        } else if (s === 'approved'){
          actionsCell.appendChild(makeAction('Als bezahlt markieren', 'paid'));
          actionsCell.appendChild(makeAction('Ablehnen', 'rejected'));
        } else {
    ...
    async function updatePayoutRequestStatus(id, newStatus){
      ...
      var note = window.prompt('Admin-Notiz (optional):', '') || '';
      ...
      setMessage('Anfrage wird aktualisiert...', null);
      try{
        var resp = await client.from('payout_requests').update(patch).eq('id', id);
        if (resp.error){
          console.error(resp.error);
          setMessage('Fehler beim Aktualisieren der Anfrage: ' + (resp.error.message || 'unknown'), 'error');
          return;
        }
        setMessage('Anfrage aktualisiert.', 'success');
        await loadAll();
      }catch(e){
        console.error(e);
        setMessage('Unerwarteter Fehler beim Aktualisieren der Anfrage.', 'error');
      }
    }
    ...
    async function loadAggregates(){
      ...
      if (resp.error){
        console.warn('Errore caricamento monthly_affiliate_stats', resp.error);
    ...
    async function runImportTravelpayouts(){
      ...
      setMessage('Import läuft (Aviasales)...', null);
      try{
        // days = 45 default
        var resp = await client.functions.invoke('import-travelpayouts', { body: { days: 45, campaign_id: 100 } });
        if (resp.error){
          console.error(resp.error);
          setMessage('Importfehler: ' + (resp.error.message || 'unknown'), 'error');
          return;
        }
        setMessage('Import abgeschlossen. Dashboard wird aktualisiert...', 'success');
        await loadAll();
      }catch(e){
        console.error(e);
        setMessage('Unerwarteter Fehler während des Imports.', 'error');
      }
    }
    ...
    async function runRefreshPayouts(){
      ...
      setMessage('Auszahlungen werden aktualisiert...', null);
      try{
        var resp = await client.rpc('refresh_monthly_affiliate_payouts');
        if (resp && resp.error){
          setMessage('Fehler beim Aktualisieren: ' + (resp.error.message || 'unknown'), 'error');
          return;
        }
        setMessage('Auszahlungen aktualisiert.', 'success');
        await loadAll();
      }catch(e){
        console.error(e);
        setMessage('Unerwarteter Fehler beim Aktualisieren.', 'error');
      }
    }
    ...
    async function importStay22CSV(file){
      ...
      setMessage('CSV wird gelesen...', null);
      ...
      if (!parsed.rows.length){
        setMessage('CSV ist leer oder ungültig.', 'error');
        return;
      }
      ...
      if (!rows.length){
        setMessage('CSV nicht kompatibel: keine gültigen Zeilen gefunden. Stelle sicher, dass im CSV mindestens Campaign ID (oder affiliate_slug) und Transaction ID (oder partner_reference) enthalten sind.', 'error');
        return;
      }
      setMessage('Stay22-CSV-Import: Upsert läuft...', null);
      ...
      for (var i=0;i<rows.length;i+=chunkSize){
        ...
        if (up.error){
          ...
          setMessage('Fehler beim CSV-Import: ' + (up.error.message || 'unknown'), 'error');
          return;
        }
      }
      ...
      setMessage('CSV importiert. Dashboard aktualisiert.', 'success');
      await loadAll();
    }
    ...
    if (loginForm){
      loginForm.addEventListener('submit', async function(e){
        ...
        if (!client || !client.auth){
          setMessage('Supabase Auth ist nicht verfügbar.', 'error');
          return;
        }
        ...
        try{
          ...
          if (res.error){
            setMessage(res.error.message || 'Login-Fehler.', 'error');
            return;
          }
          ...
          if (!u || !isAdminEmail(u.email)){
            ...
            setMessage('Dieses Konto ist kein Admin-Konto.', 'error');
            return;
          }
          state.user = u;
          ...
          showDashboard(true);
          setMessage('Admin-Anmeldung erfolgreich.', 'success');
          await loadAll();
          startRealtime();
          startAutoRefresh();
        }catch(err){
          console.error(err);
          setMessage('Unerwarteter Fehler beim Login.', 'error');
        }
      });
    }
