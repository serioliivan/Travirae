  ...
  async function unsubscribe(){
    try {
      if (!window.supabaseClient?.functions?.invoke) {
        throw new Error(IS_DE ? 'Supabase nicht initialisiert' : (IS_EN ? 'Supabase not initialised' : (IS_FR ? 'Supabase n\'est pas initialisé' : (IS_ES ? 'Supabase no está inicializado' : 'Supabase non inizializzato'))));
      }
