(function(){
  ...
  if (!token) {
    setStatus(IS_DE ? 'Token fehlt. Der Link könnte unvollständig sein.' : (IS_EN ? 'Missing token. The link may be incomplete.' : (IS_FR ? 'Jeton manquant. Le lien est peut‑être incomplet.' : (IS_RU ? 'Отсутствует токен. Ссылка может быть неполной.' : 'Token mancante. Il link potrebbe essere incompleto.'))));
    return;
  }
  ...
  async function unsubscribe(){
    try {
      if (!window.supabaseClient?.functions?.invoke) {
        throw new Error(IS_DE ? 'Supabase nicht initialisiert' : (IS_EN ? 'Supabase not initialised' : (IS_FR ? 'Supabase n\'est pas initialisé' : (IS_ES ? 'Supabase no está inicializado' : (IS_RU ? 'Supabase не инициализирован' : 'Supabase non inizializzato')))));
      }
