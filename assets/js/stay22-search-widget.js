...
(function(){
  ...
  function initWidget(root){
    ...
    // The original page remains open. Re-enable search when the visitor returns.
    window.addEventListener('focus',function(){
      if (navigating) resetNavigation();
    });
    document.addEventListener('visibilitychange',function(){
      if (!document.hidden && navigating) resetNavigation();
    });
    if (submitButton) submitButton.title = copy.newTabHint;

    function openResultsTab(href){
      // Must run synchronously in the validated submit gesture, not after a
      // timeout or an API call. Only one new tab is requested per search.
      // Detach the opener BEFORE loading any partner page, while keeping the
      // normal Referer header (no noreferrer) and the affiliate URL unchanged.
      var resultTab = window.open('about:blank','_blank');
      if (!resultTab) return false;
      try{
        resultTab.opener = null;
        resultTab.location.replace(href);
        return true;
      }catch(_tabError){
        try{ resultTab.close(); }catch(_closeError){}
        return false;
      }
    }
    ...
    function setSearchMode(mode, focusField){
      ...
      if (submitLabel) submitLabel.textContent = currentSubmitText();
      if (submitButton) submitButton.setAttribute('aria-label',currentSubmitText() + '. ' + copy.newTabHint);
      clearError();
      hideSuggestions();
      setGuestPanel(false);
    ...
    form.addEventListener('submit',function(event){
      ...
      try{
        ...
        if (!openResultsTab(url.toString())){
          resetNavigation();
          setError(copy.popupBlocked,null);
          return;
        }
        ...
        // Debounce accidental double submissions without leaving the button
        // disabled on the still-open Travirae page. This timer never navigates.
        navigationTimer = window.setTimeout(resetNavigation,800);
      }catch(_urlError){
        navigating = false;
