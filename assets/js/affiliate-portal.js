...
(function(){
  ...
  if (!supa || !supa.auth) {
    console.error('Supabase client non inizializzato per area affiliati.');
  }
  ...
  function showView(name){
    ...
    if (tabsContainer){
      // Keep tabs only for login/register
  ...
  function bindForms(){
    ...
    if (registerForm && supa && supa.auth){
    ...
    if (forgotForm && supa && supa.auth){
      forgotForm.addEventListener('submit', async function(e){
        ...
        try {
          var { data, error } = await supa.auth.resetPasswordForEmail(email, {
    ...
    if (recoveryForm && supa && supa.auth){
      recoveryForm.addEventListener('submit', async function(e){
        ...
        try {
          var { data, error } = await supa.auth.updateUser({ password: password });
          if (error) {
  ...
  async function initDashboard(){
    ...
    // Get user if not already in state
    if (!state.user){
      try {
