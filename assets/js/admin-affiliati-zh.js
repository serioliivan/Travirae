...
function openModal(opts){
  ...
  try{
    ...
    try{
      setNewsletterStatus('加载 Newsletter 时出错：' + (err?.message || String(err)), 'error');
    }catch(_){}
  }
}
...
Totale: ${total} · Inviate: ${sent} · Errori: ${failed}${extra}`, failed ? 'warn' : 'success');
      ...
      if(sendBtn){ sendBtn.disabled = false; sendBtn.textContent = '发送 Newsletter'; }
    }
  });
...
function clearTbody(tbody){
    ...
    async function runRefreshPayouts(){
      ...
      try{
        ...
        if (resp && resp.error){
          setMessage('Refresh error: ' + (resp.error.message || '未知'), 'error');
          return;
        }
    ...
    async function importStay22CSV(file){
      ...
      for (var i=0;i<rows.length;i+=chunkSize){
        ...
        if (up.error){
          ...
          setMessage('CSV import error: ' + (up.error.message || '未知'), 'error');
          return;
        }
      }
