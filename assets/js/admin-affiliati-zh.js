  ...
  function init(){
    ...
    function ensureModal(){
      ...
      modalEl.innerHTML = [
        ...
        '    <button type="button" class="tva-modal__close" aria-label="关闭" id="tvaModalClose">×</button>',
        '  </div>',
...
function openModal(opts){
    ...
    async function promptAndVerifyTotpCode(client, factorId, promptTitle){
      var code = await promptTotpCodeModal({ title: (promptTitle || '验证 2FA') });
      if (!code) return false;
      var ok = await verifyTotpCode(client, factorId, code);
      if (!ok) showToast('2FA 代码无效。', 'error');
      return ok;
    }
    ...
    function promptTotpCodeModal(opts){
      ...
      return new Promise(function(resolve){
        openModal({
          title: opts.title || '验证 2FA',
          bodyHtml: [
            '<p class="tva-modal__hint">请输入验证器应用生成的 6 位验证码。</p>',
            '<div class="tva-modal__field">',
            '  <label class="tva-modal__label">2FA 验证码</label>',
          ...
          actions: [
            { label: '取消', className: 'btn secondary', onClick: function(){ closeModal(); resolve(null); } },
            { label: '验证', className: 'btn', onClick: function(){
    ...
    async function ensureMfaAal2IfNeeded(client){
      try{
        ...
        if (cur === 'aal1' && next === 'aal2'){
          ...
          if (!factor || !factor.id){
            setMessage('已启用 2FA，但未找到 TOTP 因子。', 'error');
            return false;
          }

          while (true){
            var code = await promptTotpCodeModal({ title: '验证 2FA' });
            if (code === null){
            ...
            if (!/^\d{6}$/.test(code)){
              ...
              if (errEl) errEl.textContent = '请输入有效的 6 位验证码。';
              continue;
            }
            ...
            if (client.auth.mfa.challengeAndVerify){
              ...
              if (!ok){
                ...
                if (e1) e1.textContent = (v1 && v1.error && v1.error.message) ? v1.error.message : '验证码无效。';
              }
            }else{
              var ch = await client.auth.mfa.challenge({ factorId: factor.id });
              if (ch && ch.error){
                var e2 = document.getElementById('tvaMfaErr');
                if (e2) e2.textContent = ch.error.message || '无法创建挑战。';
                continue;
              }
              ...
              if (!ok){
                ...
                if (e3) e3.textContent = (v2 && v2.error && v2.error.message) ? v2.error.message : '验证码无效。';
              }
            }
    ...
    async function open2faSettings(){
      ...
      if (!user){
        showToast('你需要以管理员身份登录后才能管理 2FA。', 'error');
        return;
      }
      ...
      if (verifiedFactor && verifiedFactor.id){
        openModal({
          title: '安全（2FA）',
          bodyHtml: '<p class="tva-modal__subtitle">2FA 已启用。若要禁用，请输入验证器应用生成的验证码进行确认。</p>',
          actions: [
            { label: '关闭', className: 'btn btn-outline', onClick: closeModal },
            { label: '禁用 2FA', className: 'btn btn-danger', onClick: async function(){
                var ok = await promptAndVerifyTotpCode(client, verifiedFactor.id, '禁用 2FA');
                if (!ok) return;

                var un = await client.auth.mfa.unenroll({ factorId: verifiedFactor.id });
                if (un.error){
                  showToast(un.error.message || '禁用 2FA 时出错', 'error');
                  return;
                }
                closeModal();
      ...
      openModal({
        title: '安全（2FA）',
        bodyHtml: '<p class="tva-modal__subtitle">为管理员登录增加第二层安全保护（Authenticator）。</p>',
        actions: [
          { label: '关闭', className: 'btn btn-outline', onClick: closeModal },
          { label: '启用 2FA', className: 'btn btn-primary', onClick: async function(){
              ...
              if (en.error){
                showToast(en.error.message || '启用 2FA 时出错', 'error');
                return;
              }
              ...
              openModal({
                ...
                bodyHtml:
                  '<div class="tva-modal__stack">'+
                    '<p class="tva-modal__subtitle">1）使用验证器应用（Google Authenticator / Authy / Microsoft Authenticator）扫描二维码。</p>'+
                    '<div class="mfa-qr" id="tvaMfaQrWrap"></div>'+
                    '<p class="tva-muted" id="tvaMfaSecretWrap" style="margin-top:10px; display:none;">如果无法扫描： <code style="user-select:all; word-break:break-all;" id="tvaMfaSecret"></code></p>'+
                    '<p class="tva-modal__subtitle" style="margin-top:12px;">2）输入生成的验证码：</p>'+
                    '<div class="tva-modal__field">'+
                      '<label class="tva-modal__label">2FA 验证码</label>'+
                  ...
                  '</div>',
                actions: [
                  { label: '取消', className: 'btn btn-outline', onClick: cleanupEnroll },
                  { label: '验证并启用', className: 'btn btn-primary', onClick: async function(){
                      ...
                      if (!code){
                        if (errEl){ errEl.style.display = 'block'; errEl.textContent = '请输入 2FA 验证码。'; }
                        return;
                      }

                      var ok = await verifyTotpCode(client, factorId, code);
                      if (!ok){
                        if (errEl){ errEl.style.display = 'block'; errEl.textContent = '验证码无效。请重试。'; }
                        return;
                      }
    ...
    function renderChart(monthly){
      ...
      state.chart = new Chart(ctx, {
        ...
        data: {
          labels: labels,
          datasets: [
            { label: '联盟结算', data: affValues, tension: 0.25, fill: false },
  ...
  try{
    ...
    try{
      setNewsletterStatus('加载邮件通讯统计时出错：' + (err?.message || String(err)), 'error');
    }catch(_){}
  }
}
  ...
  form.addEventListener('submit', async (e)=>{
    ...
    if(missing.length){
      const msg = '请填写所有字段后继续。';
      setNewsletterStatus(`${msg} 缺少：${missing.join(', ')}`, 'error');
      try{
...
`+
      `回退语言：ITA
`+
      `已填写语言： ${totalFilled}/${NL_LANGS.length}
...
`+
    ...
    try{
      if(sendBtn){ sendBtn.disabled = true; sendBtn.textContent = '正在发送…'; }
      setNewsletterStatus('正在发送…', 'info');

      const client = refreshClient();
      if(!client) throw new Error('Supabase 客户端不可用');
      ...
      if(data?.by_language && typeof data.by_language === 'object'){
        ...
        for(const [code, obj] of Object.entries(data.by_language)){
          ...
          if(t>0) parts.push(`${code}: ${s}/${t}${f?`（失败 ${f}）`:''}`);
        }
        if(parts.length) extra = `
明细：${parts.join(' · ')}`;
      }

      setNewsletterStatus(`邮件通讯已发送。
总计：${total} · 已发送：${sent} · 错误：${failed}${extra}`, failed ? 'warn' : 'success');

      // refresh stats
      await loadNewsletterStats();
    }catch(err){
      setNewsletterStatus('发送邮件通讯出错：' + (err?.message || String(err)), 'error');
    }finally{
      if(sendBtn){ sendBtn.disabled = false; sendBtn.textContent = '发送邮件通讯'; }
    }
  });
    ...
    const clearBtn = q('#admin-newsletter-clear-btn');
      if(clearBtn){
        clearBtn.addEventListener('click', () => {
          ...
          setNewsletterStatus('草稿已清除。', 'success');
          updatePreview();
        });
      }
...
function clearTbody(tbody){
    ...
    async function updatePayoutRequestStatus(id, newStatus){
      ...
      setMessage('正在更新申请…', null);
      try{
        var resp = await client.from('payout_requests').update(patch).eq('id', id);
        if (resp.error){
          console.error(resp.error);
          setMessage('更新申请时出错：' + (resp.error.message || '未知'), 'error');
          return;
        }
        setMessage('申请已更新。', 'success');
        await loadAll();
      }catch(e){
        console.error(e);
        setMessage('更新申请时发生意外错误。', 'error');
      }
    }
    ...
    async function runImportTravelpayouts(){
      ...
      setMessage('正在导入（Aviasales）…', null);
      try{
        // days = 45 default
        var resp = await client.functions.invoke('import-travelpayouts', { body: { days: 45, campaign_id: 100 } });
        if (resp.error){
          console.error(resp.error);
          setMessage('导入出错：' + (resp.error.message || '未知'), 'error');
          return;
        }
        setMessage('导入完成。正在更新仪表盘…', 'success');
        await loadAll();
      }catch(e){
        console.error(e);
        setMessage('导入时发生意外错误。', 'error');
      }
    }
    ...
    async function runRefreshPayouts(){
      ...
      setMessage('正在刷新提现记录…', null);
      try{
        var resp = await client.rpc('refresh_monthly_affiliate_payouts');
        if (resp && resp.error){
          setMessage('刷新出错：' + (resp.error.message || '未知'), 'error');
          return;
        }
        setMessage('提现已刷新。', 'success');
        await loadAll();
      }catch(e){
        console.error(e);
        setMessage('刷新时发生意外错误。', 'error');
      }
    }
    ...
    async function importStay22CSV(file){
      ...
      if (!rows.length){
        setMessage('不支持的 CSV：未找到有效行。请确保 CSV 至少包含 Campaign ID（或 affiliate_slug）以及 Transaction ID（或 partner_reference）。', 'error');
        return;
      }
      setMessage('正在导入 Stay22 CSV：正在写入/更新数据…', null);
      ...
      for (var i=0;i<rows.length;i+=chunkSize){
        ...
        if (up.error){
          ...
          setMessage('CSV 导入出错：' + (up.error.message || '未知'), 'error');
          return;
        }
      }
      ...
      setMessage('CSV 已导入。仪表盘已更新。', 'success');
      await loadAll();
    }
    ...
    if (loginForm){
      loginForm.addEventListener('submit', async function(e){
        ...
        if (!client || !client.auth){
          setMessage('Supabase Auth 不可用。', 'error');
          return;
        }
        ...
        try{
          ...
          if (res.error){
            setMessage(res.error.message || '登录出错。', 'error');
            return;
          }
