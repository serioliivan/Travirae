...
(function(){
  ...
  function init(){
    ...
    var elTotalWidgetClicks = $('#admin-total-widget-clicks');
    var elTotalAvailableBalance = $('#admin-total-available-balance');
    ...
    function renderChart(monthly){
      ...
      state.chart = new Chart(ctx, {
        ...
        data: {
          labels: labels,
          datasets: [
            { label: affDatasetLabel, data: affValues, tension: 0.25, fill: false },
...
function clearTbody(tbody){
    ...
    function renderOperationalTable(rows){
      ...
      rows.forEach(function(r){
        ...
        var widgetClicksDisplay = Number(r && r.widget_clicks_total || 0);
        if (!isFinite(widgetClicksDisplay) || widgetClicksDisplay <= 0){
          var widgetClicksSite = Number(r && r.widget_clicks_site_total || 0);
          var widgetClicksCreator = Number(r && r.widget_clicks_creator_total || 0);
          widgetClicksDisplay = (isFinite(widgetClicksSite) ? widgetClicksSite : 0) + (isFinite(widgetClicksCreator) ? widgetClicksCreator : 0);
        }
        tr.appendChild(td(String(widgetClicksDisplay)));
    ...
    function updateOperationalTopKpis(rows){
      var list = Array.isArray(rows) ? rows : [];
      var totalWidgetClicks = 0;
      var totalAvailableBalance = 0;
      list.forEach(function(r){
        var widgetClicks = Number(r && r.widget_clicks_total || 0);
        if (!isFinite(widgetClicks) || widgetClicks <= 0){
          var widgetClicksSite = Number(r && r.widget_clicks_site_total || 0);
          var widgetClicksCreator = Number(r && r.widget_clicks_creator_total || 0);
          widgetClicks = (isFinite(widgetClicksSite) ? widgetClicksSite : 0) + (isFinite(widgetClicksCreator) ? widgetClicksCreator : 0);
        }
        if (isFinite(widgetClicks)) totalWidgetClicks += widgetClicks;
        var available = Number(r && r.available_balance_usd || 0);
        if (isFinite(available)) totalAvailableBalance += available;
      });
      safeSetText(elTotalWidgetClicks, String(totalWidgetClicks));
      safeSetText(elTotalAvailableBalance, formatUsd(totalAvailableBalance));
    }
    ...
    async function loadOperationalOverview(){
      ...
      if (resp.error){
        ...
        updateOperationalTopKpis([]);
        return;
      }
      var operationalRows = resp.data || [];
      renderOperationalTable(operationalRows);
      updateOperationalTopKpis(operationalRows);
    }
