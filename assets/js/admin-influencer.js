...
(function(){
    ...
    view.innerHTML = '' +
      '<div class="if-admin-shell if-admin-shell--accordion">' +
        ...
        '<section class="if-admin-directory-card if-admin-detail-section">' +
          ...
          '<div class="if-admin-search-row"><div class="form-group"><label for="if-admin-creator-search">' + escapeHtml(L('searchCreators')) + '</label><input id="if-admin-creator-search" type="search" placeholder="' + escapeHtml(L('searchPlaceholder')) + '"/></div></div>' +
  ...
  function renderCreatorPagination(totalItems){
    ...
    state.creatorPage = Math.min(totalPages, Math.max(1, state.creatorPage || 1));
    el.innerHTML = '' +
      '<button class="btn secondary if-pagination-arrow" type="button" data-admin-page="' + (state.creatorPage - 1) + '" aria-label="Previous"' + (state.creatorPage <= 1 ? ' disabled' : '') + '>←</button>' +
      '<span class="muted small">' + escapeHtml(t('page')) + ' ' + escapeHtml(String(state.creatorPage)) + ' / ' + escapeHtml(String(totalPages)) + '</span>' +
      '<button class="btn secondary if-pagination-arrow" type="button" data-admin-page="' + (state.creatorPage + 1) + '" aria-label="Next"' + (state.creatorPage >= totalPages ? ' disabled' : '') + '>→</button>';
  }
