...
(function(){
  ...
  function renderSkeleton(){
    ...
    view.innerHTML = '' +
      '<div class="if-admin-shell">' +
        '<div class="if-admin-topbar">' +
          '<div class="if-admin-topbar__copy"><span class="chip">Travirae Creator</span><h2>' + escapeHtml(L('title')) + '</h2><p class="muted small">Travirae + creator moderation</p></div>' +
    ...
    if (logout) logout.addEventListener('click', async function(){ try{ await client.auth.signOut(); }catch(_){ } state.user = null; setViews(false); redirectToAdminLogin(); });
  }
