      ...
      if (toastEl){
        ...
        toastEl.innerHTML = ''+
          '<div class="tva-toast__inner" role="alert" aria-live="polite">'+
            ...
            '<button type="button" class="tva-toast__close" aria-label="Закрыть">×</button>'+
          '</div>';
      ...
      modalEl.innerHTML = ''+
        '<div class="tva-modal__card" role="dialog" aria-modal="true">'+
          '<div class="tva-modal__header">'+
            ...
            '<button type="button" class="tva-modal__close" aria-label="Закрыть">×</button>'+
          '</div>'+
...
function openModal(opts){
    ...
    async function ensureMfaAal2IfNeeded(client, opts = {}){
      try{
        ...
        var ok = await promptAndVerifyTotpCode(client, factor.id, 'Проверка 2FA');
        if(!ok) return false;

        showToast('2FA подтверждена.', 'success');
        return true;
      }catch(e){
        console.warn(e);
        return true;
...
// Richiede una re‑autenticazione (password) e, se presente, anche un codice 2FA.
	    ...
	    async function promptPasswordThen2FA(client, opts){
	      ...
	      try{
	        ...
	        if (!email){
	          showToast('Сессия недействительна. Пожалуйста, войдите снова.', 'error');
	          return false;
	        }
	        ...
	        var password = await (function(){
	          return new Promise(function(resolve){
	            var modalId = openModal({
	              title: opts.passwordTitle || 'Подтвердите пароль',
	              bodyHtml: [
	                '<div class="tva-modal__hint">',
	                  (opts.passwordHint || 'Чтобы продолжить, введите пароль от вашего аккаунта.'),
	                '</div>',
	                ...
	                },
	                {
	                  ...
	                  onClick: async function(id){
	                    ...
	                    try{
	                      ...
	                      console.warn(e);
	                      if (errEl){ errEl.textContent = 'Ошибка проверки. Пожалуйста, попробуйте ещё раз.'; errEl.style.display = 'block'; }
	                      if (modal){
	        ...
	        console.warn(e);
	        showToast('Ошибка проверки. Пожалуйста, попробуйте ещё раз.', 'error');
	        return false;
	      }
	    }
    ...
    async function promptAndVerifyTotpCode(client, factorId, promptTitle){
      var code = await promptTotpCodeModal(promptTitle || 'Проверка 2FA');
          ...
          buttons: [
            ...
            },
            {
              ...
              onClick: async function(){
                ...
                if (!/^\d{6}$/.test(code)){
                  if (errEl){ errEl.textContent = 'Введите корректный 6‑значный код.'; errEl.style.display = 'block'; }
                  return;
                }
                ...
                try{
                  ...
                  if (errEl){ errEl.textContent = (err && err.message) ? err.message : 'Неверный код. Пожалуйста, попробуйте ещё раз.'; errEl.style.display = 'block'; }
                }
              }
            }
          ]
    ...
    async function promptTotpCodeModal(promptTitle){
      promptTitle = promptTitle || 'Проверка 2FA';
      return new Promise(function(resolve){
        var done = false;
        function finish(val){
          if (done) return;
        ...
        openModal({
          title: promptTitle,
          bodyHtml: ''+
            '<p class="tva-modal__subtitle">Введите 6‑значный код, сгенерированный вашим приложением Authenticator.</p>'+
            '<div class="tva-field">'+
              '<label class="tva-label">Код 2FA</label>'+
            ...
            '</div>',
          buttons: [
            { label: 'Отмена', className: 'btn btn-outline', onClick: function(){ closeModal(); finish(null); } },
            { label: 'Подтвердить', className: 'btn btn-primary', onClick: function(){
    ...
    async function open2faSettings(){
      ...
      if (!user){
        showToast('Чтобы управлять 2FA, нужно войти в аккаунт.', 'error');
        return;
      }
      ...
      if (verifiedFactor && verifiedFactor.id){
        openModal({
          ...
          bodyHtml: '<p class="tva-modal__subtitle">2FA включена. Чтобы отключить её, подтвердите кодом из вашего приложения.</p>',
          buttons: [
      ...
      openModal({
        title: 'Безопасность (2FA)',
        bodyHtml: '<p class="tva-modal__subtitle">Добавьте второй уровень защиты к вашему аккаунту (Authenticator).</p>',
        buttons: [
          { label: 'Закрыть', className: 'btn btn-outline', onClick: closeModal },
              ...
              openModal({
                ...
                bodyHtml:
                  '<div class="tva-modal__stack">'+
                    '<div style="margin:10px 0 14px; padding:12px; border:1px solid #fde68a; background:#fffbeb; border-radius:12px; font-size:13px; line-height:1.35; color:#111;"><strong>Важно:</strong> если вы потеряете доступ к приложению Authenticator, вы можете больше не суметь <strong>войти</strong> в аккаунт и, возможно, не сможете <strong>сбросить пароль</strong> («Забыли пароль?»), пока не восстановите доступ к 2FA. Убедитесь, что у вас остаётся доступ к устройству/приложению, и при возможности настройте 2FA на втором устройстве.</div>'+
                    '<p class="tva-modal__subtitle">1) Отсканируйте QR‑код в приложении Authenticator.</p>'+
                ...
                buttons: [
                  ...
                  { label: 'Подтвердить и включить', className: 'btn btn-primary', onClick: async function(){
                      ...
                      closeModal();
                      showToast('2FA включена! Важно: не теряйте доступ к приложению Authenticator, иначе вы можете не восстановить аккаунт.', 'success');
                    } }
                ],
    ...
    function openChangeLangModal(){
      var client = refreshClient();
      if (!client){
        showToast('Supabase не готов. Попробуйте снова через несколько секунд.', 'error');
        return;
      }
      ...
      client.auth.getUser().then(function(res){
        ...
        if (!user){
          showToast('Чтобы изменить язык, нужно войти в аккаунт.', 'error');
          return;
        }
        ...
        openModal({
          ...
          bodyHtml:
            ...
            '</div>',
          buttons: [
            { label: 'Отмена', className: 'btn btn-outline', onClick: function(){ closeModal(); } },
            { label: 'Сохранить', className: 'btn btn-primary', onClick: async function(){
              ...
              try{
                ...
                closeModal();
                showToast('Язык успешно обновлён.', 'success');
              }catch(e){
                console.warn(e);
                showToast((e && e.message) ? e.message : 'Ошибка при изменении языка.', 'error');
              }
            } }
          ]
        });
        ...
        console.warn(e);
        showToast('Не удалось получить профиль пользователя.', 'error');
      });
    }
    ...
    function bindAuthForms(){
      ...
      if (loginForm){
        loginForm.addEventListener('submit', async function(e){
          ...
          if (!client || !client.auth){
            setMessage('Сервис аутентификации недоступен. Пожалуйста, попробуйте позже.', 'error');
            return;
          }
          ...
          try{
            ...
            if (!state.user){
              setMessage('Вход выполнен, но не удалось получить данные пользователя.', 'error');
              return;
            }
            ...
            console.error(err);
            setMessage('Ошибка входа. Пожалуйста, попробуйте позже.', 'error');
          }
        });
      }
      ...
      if (registerForm){
        registerForm.addEventListener('submit', async function(e){
          ...
          if (!email || !password){
            setMessage('Пожалуйста, введите email и пароль.', 'error');
            return;
          }
          if (password !== confirm){
          ...
          try{
            ...
            if (result.error){
              ...
              if(msg.includes('already') && msg.includes('registered')){
                setMessage('Этот e-mail уже зарегистрирован. Пожалуйста, попробуйте другой адрес.', 'error');
              }else{
                setMessage(result.error.message || 'Ошибка при регистрации.', 'error');
              }
              return;
            }
            ...
            setMessage('Регистрация завершена! Проверьте почту и подтвердите адрес перед входом.', 'success');
            showView('login');
          }catch(err){
            console.error(err);
            setMessage('Непредвиденная ошибка при регистрации.', 'error');
          }
        });
      }
      ...
      if (forgotForm){
        forgotForm.addEventListener('submit', async function(e){
          ...
          if (!email){
            setMessage('Введите ваш e-mail.', 'error');
            return;
          }
          ...
          try{
            ...
            if (result.error){
              setMessage(result.error.message || 'Ошибка при сбросе пароля.', 'error');
              return;
            }

            setMessage('Если адрес есть в нашей системе, вы получите письмо со ссылкой для сброса пароля.', 'success');
            showView('login');
          }catch(err){
            console.error(err);
            setMessage('Непредвиденная ошибка при сбросе пароля.', 'error');
          }
        });
      }
      ...
      if (recoveryForm){
        recoveryForm.addEventListener('submit', async function(e){
          ...
          if (!password){
            setMessage('Введите новый пароль.', 'error');
            return;
          }
          if (password !== confirm){
            setMessage('Пароли не совпадают.', 'error');
...
try {
  var mfaOk = await ensureMfaAal2IfNeeded(client, { force: true });
  if (!mfaOk){
    setMessage('Чтобы обновить пароль, требуется подтверждение 2FA (код Authenticator).', 'error');
    return;
  }
} catch (e) {
...
if (result.error){
  var em = result.error.message || 'Ошибка при обновлении пароля.';
  if (String(em).includes('AAL2') || String(em).toLowerCase().includes('aal2 session')){
    em = 'Чтобы изменить пароль при включённой 2FA, нужно также ввести код Authenticator (2FA). Пожалуйста, попробуйте ещё раз и завершите проверку 2FA.';
  }
  setMessage(em, 'error');
  return;
}

            setMessage('Пароль обновлён! Теперь вы можете войти с новыми данными.', 'success');
            showView('login');
          }catch(err){
            console.error(err);
            setMessage('Непредвиденная ошибка при обновлении пароля.', 'error');
          }
        });
      }
      ...
      if (logoutBtn){
        logoutBtn.addEventListener('click', async function(){
          ...
          setMessage('Вы вышли из партнёрского кабинета.', 'success');
          showView('login');
        });
      }
        ...
        elCopyBtn.addEventListener('click', function(){
          ...
          if (navigator.clipboard && navigator.clipboard.writeText){
            navigator.clipboard.writeText(elLinkInput.value)
              .then(function(){ setMessage('Ссылка скопирована в буфер обмена.', 'success'); })
              .catch(function(){ setMessage('Не удалось скопировать. Выделите и скопируйте вручную.', 'error'); });
          }else{
            try{
              var ok = document.execCommand('copy');
              setMessage(ok ? 'Ссылка скопирована в буфер обмена.' : 'Не удалось скопировать. Выделите и скопируйте вручную.', ok ? 'success' : 'error');
            }catch(e){
              setMessage('Выберите ссылку и скопируйте вручную.', 'error');
            }
          }
        });
      }
    ...
    function updatePayoutsTable(payouts){
      ...
      if (!payouts || !payouts.length){
        ...
        td0.textContent = 'Выплат пока нет.';
        tr0.appendChild(td0);
    ...
    function setTierUI(tier){
      ...
      if (elTierNext){
        if (tier.nextAt == null){
          elTierNext.textContent = 'Вы достигли максимального уровня (60%).';
        } else {
          elTierNext.textContent = 'Следующий уровень: ' + (tier.nextPercent * 100).toFixed(0) + '% при ' + tier.nextAt + ' продажах (осталось ' + tier.missing + ').';
        }
      }
    }
    ...
    function renderPayoutRequestsTable(rows){
      ...
      if (!rows || !rows.length){
        ...
        td0.colSpan = 4;
        td0.className = 'muted small';
        td0.textContent = 'Запросов пока нет.';
        tr0.appendChild(td0);
      ...
      rows.forEach(function(r){
        ...
        var date = '';
        try{ date = new Date(r.requested_at).toLocaleString('ru-RU'); }catch(e){ date = String(r.requested_at || ''); }

        var status = String(r.status || 'pending');
        var statusLabel = status;
        if (status === 'pending') statusLabel = 'В ожидании';
        if (status === 'approved') statusLabel = 'Одобрено';
        if (status === 'paid') statusLabel = 'Выплачено';
        if (status === 'rejected') statusLabel = 'Отклонено';
      ...
      if (btnOpenPayoutForm){
        btnOpenPayoutForm.addEventListener('click', async function(){
          ...
          if ((Number(state.availableBalance || 0) || 0) < 50){
            setMessage('Недостаточно средств: для запроса выплаты нужно минимум 50 USD.', 'error');
            return;
          }
	          ...
	          var okAuth = await promptPasswordThen2FA(refreshClient(), {
	            title: 'Подтвердите запрос выплаты',
	            message: 'Чтобы продолжить, введите пароль от вашего аккаунта. Если включена 2FA, также потребуется код.',
	            passwordTitle: 'Подтвердите пароль'
	          });
	          if (!okAuth){
	            return;
	          }
      ...
      if (payoutForm){
        payoutForm.addEventListener('submit', async function(e){
          ...
          if (!client || !client.functions){
            setMessage('Сервис выплат недоступен (Functions).', 'error');
            return;
          }
          ...
          if (amount > availableNow){
            setMessage('Недостаточно средств: запрошенная сумма превышает доступный баланс.', 'error');
            return;
          }

          if (amount < 50){
            setMessage('Ошибка: минимальный порог выплаты — 50 USD.', 'error');
            return;
          }

          if (confirmEl && !confirmEl.checked){
            setMessage('Необходимо подтвердить, что данные верны.', 'error');
            return;
          }
          ...
          if (!fiscal.legal_name || !fiscal.tax_residence_country || !fiscal.address_country || !fiscal.address_city){
            setMessage('Пожалуйста, заполните обязательные налоговые данные.', 'error');
            return;
          }

          if (method === 'paypal'){
            if (!payout.paypal_email){
              setMessage('Пожалуйста, укажите e-mail PayPal.', 'error');
              return;
            }
          } else {
            if (!payout.beneficiary_name || !payout.bank_country){
              setMessage('Укажите владельца счёта и страну банка.', 'error');
              return;
            }
            // IBAN è richiesto per SEPA; per SWIFT puro potrebbe essere diverso, ma per semplicità lo chiediamo.
            if (!payout.iban){
              setMessage('Укажите IBAN (для переводов SEPA).', 'error');
              return;
            }
          }
          ...
          try{
            ...
            setMessage('✅ Запрос отправлен! Он появится в истории, а администратор также получит письмо.', 'success');
            if (payoutFormWrap) payoutFormWrap.hidden = true;
            ...
            setMessage('❌ Ошибка запроса выплаты: ' + msg, 'error');
          }finally{
    ...
    async function initDashboard(){
      ...
      if (!aff){
        setMessage('Не удалось получить партнёрский профиль. Пожалуйста, попробуйте позже.', 'error');
        showView('login');
        return;
      }
    ...
    if (mode === 'recovery'){
      showView('recovery');
      setMessage('Установите новый пароль для вашего аккаунта.', null);
      return;
    }
