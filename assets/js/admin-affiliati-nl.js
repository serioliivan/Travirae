  ...
  function init(){
    ...
    function ensureModal(){
      ...
      modalEl.innerHTML = [
        ...
        '    <button type="button" class="tva-modal__close" aria-label="Sluiten" id="tvaModalClose">×</button>',
        '  </div>',
...
function openModal(opts){
    ...
    async function promptAndVerifyTotpCode(client, factorId, promptTitle){
      var code = await promptTotpCodeModal({ title: (promptTitle || '2FA verifiëren') });
      if (!code) return false;
      var ok = await verifyTotpCode(client, factorId, code);
      if (!ok) showToast('Ongeldige 2FA-code.', 'error');
      return ok;
    }
    ...
    function promptTotpCodeModal(opts){
      ...
      return new Promise(function(resolve){
        openModal({
          title: opts.title || '2FA verifiëren',
          bodyHtml: [
            '<p class="tva-modal__hint">Enter the 6-digit code from your authenticator app.</p>',
            '<div class="tva-modal__field">',
            '  <label class="tva-modal__label">2FA code</label>',
          ...
          actions: [
            { label: 'Annuleren', className: 'btn secondary', onClick: function(){ closeModal(); resolve(null); } },
            { label: 'Verifiëren', className: 'btn', onClick: function(){
    ...
    async function ensureMfaAal2IfNeeded(client){
      try{
        ...
        if (cur === 'aal1' && next === 'aal2'){
          ...
          if (!factor || !factor.id){
            setMessage('2FA is ingeschakeld maar er is geen TOTP-factor gevonden.', 'error');
            return false;
          }

          while (true){
            var code = await promptTotpCodeModal({ title: '2FA verifiëren' });
            if (code === null){
            ...
            if (!/^\d{6}$/.test(code)){
              ...
              if (errEl) errEl.textContent = 'Voer een geldige 6-cijferige code in.';
              continue;
            }
            ...
            if (client.auth.mfa.challengeAndVerify){
              ...
              if (!ok){
                ...
                if (e1) e1.textContent = (v1 && v1.error && v1.error.message) ? v1.error.message : 'Ongeldige code.';
              }
            }else{
              var ch = await client.auth.mfa.challenge({ factorId: factor.id });
              if (ch && ch.error){
                var e2 = document.getElementById('tvaMfaErr');
                if (e2) e2.textContent = ch.error.message || 'Challenge kon niet worden gemaakt.';
                continue;
              }
              ...
              if (!ok){
                ...
                if (e3) e3.textContent = (v2 && v2.error && v2.error.message) ? v2.error.message : 'Ongeldige code.';
              }
            }
    ...
    async function open2faSettings(){
      ...
      if (!user){
        showToast('Je moet als beheerder zijn ingelogd om 2FA te beheren.', 'error');
        return;
      }
      ...
      if (verifiedFactor && verifiedFactor.id){
        openModal({
          title: 'Security (2FA)',
          bodyHtml: '<p class="tva-modal__subtitle">2FA is enabled. To disable it, confirm with a code from your app.</p>',
          actions: [
            { label: 'Sluiten', className: 'btn btn-outline', onClick: closeModal },
            { label: '2FA uitschakelen', className: 'btn btn-danger', onClick: async function(){
                var ok = await promptAndVerifyTotpCode(client, verifiedFactor.id, '2FA uitschakelen');
                if (!ok) return;
      ...
      openModal({
        title: 'Security (2FA)',
        bodyHtml: '<p class="tva-modal__subtitle">Beveilig de admin-toegang met een tweede factor (Authenticator).</p>',
        actions: [
          { label: 'Sluiten', className: 'btn btn-outline', onClick: closeModal },
          { label: '2FA inschakelen', className: 'btn btn-primary', onClick: async function(){

              // Ensure no verified factor already
              var existing = await getVerifiedTotpFactor(client);
              if (existing && existing.id){
                showToast('2FA is al ingeschakeld.', 'success');
                open2faSettings();
                return;
              }
              ...
              openModal({
                title: '2FA inschakelen',
                bodyHtml:
                  '<div class="tva-modal__stack">'+
                    '<p class="tva-modal__subtitle">1) Scan the QR code with Google Authenticator / Authy / Microsoft Authenticator.</p>'+
                  ...
                  '</div>',
                actions: [
                  { label: 'Annuleren', className: 'btn btn-outline', onClick: cleanupEnroll },
                  { label: 'Verifiëren & inschakelen', className: 'btn btn-primary', onClick: async function(){
                      ...
                      if (!code){
                        if (errEl){ errEl.style.display = 'block'; errEl.textContent = 'Voer de 2FA-code in.'; }
                        return;
                      }

                      var ok = await verifyTotpCode(client, factorId, code);
                      if (!ok){
                        if (errEl){ errEl.style.display = 'block'; errEl.textContent = 'Ongeldige code. Probeer het opnieuw.'; }
                        return;
                      }
                ...
                try{
                  ...
                  if (qrWrap){
                    ...
                    if (qrUrl){
                      ...
                      p.textContent = 'QR-code niet beschikbaar. Gebruik de sleutel hieronder.';
    ...
    function renderChart(monthly){
      ...
      state.chart = new Chart(ctx, {
        ...
        data: {
          labels: labels,
          datasets: [
            { label: 'Partneruitbetalingen', data: affValues, tension: 0.25, fill: false },
            { label: 'Travirae-marge', data: travValues, tension: 0.25, fill: false },
          ]
        },
        options: {
          responsive: true,
  ...
  try{
    ...
    try{
      setNewsletterStatus('Error loading newsletter: ' + (err?.message || String(err)), 'error');
    }catch(_){}
  }
}
  ...
  form.addEventListener('submit', async (e)=>{
    ...
    if(missing.length){
      const msg = 'Vul alle velden in om door te gaan.';
      setNewsletterStatus(`${msg} Ontbreekt: ${missing.join(', ')}`, 'error');
      try{
...
`+
      `Fallback: ITA
`+
      `Ingevulde talen: ${totalFilled}/${NL_LANGS.length}

`+
      `Onderwerp (ITA): ${ita.subject}`
    );
    if(!ok) return;

    try{
      if(sendBtn){ sendBtn.disabled = true; sendBtn.textContent = 'Bezig met verzenden...'; }
      setNewsletterStatus('Bezig met verzenden...', 'info');
...
Details: ${parts.join(' · ')}`;
      ...
      setNewsletterStatus(`Nieuwsbrief verzonden.
Totale: ${total} · Inviate: ${sent} · Errori: ${failed}${extra}`, failed ? 'warn' : 'success');

      // refresh stats
      await loadNewsletterStats();
    }catch(err){
      setNewsletterStatus('Fout bij het verzenden van de nieuwsbrief: ' + (err?.message || String(err)), 'error');
    }finally{
      if(sendBtn){ sendBtn.disabled = false; sendBtn.textContent = 'Nieuwsbrief verzenden'; }
    }
  });
    ...
    function buildPreviewEmailHtml(language, subject, bodyRaw){
      ...
      const footer = `
        ...
        <p style="font-size:12px;color:#6b7280;margin:0;">
          Als je deze e-mails niet langer wilt ontvangen, kun je je <a href="${safeUnsub}" target="_blank" rel="noopener">afmelden</a>.
        </p>
      `;
      ...
      // HTML frammento → render diretto. Tekst semplice → render come testo (senza template fisso).
      let contentHtml = '';
...
<html>
  ...
  <body style="margin:0;padding:16px;font-family:system-ui,-apple-system,Segoe UI,Roboto,Ubuntu,Cantarell,Noto Sans,Arial;">
    ...
    function updatePreview(){
      ...
      subjectPreviewEl.textContent = subject || '(geen onderwerp)';
    ...
    const clearBtn = q('#admin-newsletter-clear-btn');
      if(clearBtn){
        clearBtn.addEventListener('click', () => {
          ...
          setNewsletterStatus('Concepten gewist.', 'success');
          updatePreview();
        });
      }
...
function clearTbody(tbody){
    ...
    function renderMonthlyTable(rows){
      ...
      if (!rows || !rows.length){
        ...
        td0.textContent = 'No data available.';
        tr0.appendChild(td0);
    ...
    function renderAffiliatesSummaryTable(rows){
      ...
      rows.forEach(function(r){
        ...
        if (slugLabel === 'direct') slugLabel = 'Direct traffic (no affiliate)';
    ...
    function renderPayoutRequestsTable(rows){
      ...
      if (!rows || !rows.length){
        ...
        td0.colSpan = 7;
        td0.className = 'muted small';
        td0.textContent = 'No payout requests.';
        tr0.appendChild(td0);
      ...
      rows.forEach(function(r){
        ...
        if (s === 'pending'){
          ...
          actionsCell.appendChild(makeAction('Mark as paid', 'paid'));
        } else if (s === 'approved'){
          actionsCell.appendChild(makeAction('Mark as paid', 'paid'));
          actionsCell.appendChild(makeAction('Reject', 'rejected'));
        } else {
    ...
    async function updatePayoutRequestStatus(id, newStatus){
      ...
      setMessage('Aanvraag wordt bijgewerkt...', null);
      try{
        var resp = await client.from('payout_requests').update(patch).eq('id', id);
        if (resp.error){
          console.error(resp.error);
          setMessage('Fout bij het bijwerken van de aanvraag: ' + (resp.error.message || 'unknown'), 'error');
          return;
        }
        setMessage('Aanvraag bijgewerkt.', 'success');
        await loadAll();
      }catch(e){
        console.error(e);
        setMessage('Onverwachte fout bij het bijwerken van de aanvraag.', 'error');
      }
    }
    ...
    async function runImportTravelpayouts(){
      ...
      setMessage('Import bezig (Aviasales)...', null);
      try{
        // days = 45 default
        var resp = await client.functions.invoke('import-travelpayouts', { body: { days: 45, campaign_id: 100 } });
        if (resp.error){
          console.error(resp.error);
          setMessage('Importfout: ' + (resp.error.message || 'unknown'), 'error');
          return;
        }
        setMessage('Import voltooid. Dashboard wordt bijgewerkt...', 'success');
        await loadAll();
      }catch(e){
        console.error(e);
        setMessage('Onverwachte fout tijdens import.', 'error');
      }
    }
    ...
    async function runRefreshPayouts(){
      ...
      setMessage('Uitbetalingen worden ververst...', null);
      try{
        var resp = await client.rpc('refresh_monthly_affiliate_payouts');
        if (resp && resp.error){
          setMessage('Verversfout: ' + (resp.error.message || 'unknown'), 'error');
          return;
        }
        setMessage('Uitbetalingen ververst.', 'success');
        await loadAll();
      }catch(e){
        console.error(e);
        setMessage('Onverwachte fout tijdens verversen.', 'error');
      }
    }
    ...
    async function importStay22CSV(file){
      ...
      setMessage('CSV wordt ingelezen...', null);
      ...
      if (!parsed.rows.length){
        setMessage('Lege of ongeldige CSV.', 'error');
        return;
      }
      ...
      if (!rows.length){
        setMessage('Niet-ondersteunde CSV: geen geldige rijen gevonden. Zorg dat de CSV minstens Campaign ID (of affiliate_slug) en Transaction ID (of partner_reference) bevat.', 'error');
        return;
      }
      setMessage('Stay22 CSV importeren: upsert bezig...', null);
      ...
      for (var i=0;i<rows.length;i+=chunkSize){
        ...
        if (up.error){
          ...
          setMessage('CSV-importfout: ' + (up.error.message || 'unknown'), 'error');
          return;
        }
      }
      ...
      setMessage('CSV geïmporteerd. Dashboard bijgewerkt.', 'success');
      await loadAll();
    }
    ...
    if (loginForm){
      loginForm.addEventListener('submit', async function(e){
        ...
        if (!client || !client.auth){
          setMessage('Supabase Auth niet beschikbaar.', 'error');
          return;
        }
        ...
        try{
          ...
          if (res.error){
            setMessage(res.error.message || 'Login error.', 'error');
            return;
          }
          ...
          if (!u || !isAdminEmail(u.email)){
            ...
            setMessage('Dit account is geen beheerdersaccount.', 'error');
            return;
          }
          state.user = u;
          ...
          showDashboard(true);
          setMessage('Succesvol ingelogd als beheerder.', 'success');
          await loadAll();
          startRealtime();
          startAutoRefresh();
        }catch(err){
          console.error(err);
          setMessage('Onverwachte fout tijdens inloggen.', 'error');
        }
      });
    }
