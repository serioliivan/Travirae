...
function corsHeaders(origin?: string): HeadersInit {
  // In fase test, teniamo CORS permissivo.
  // Se vuoi restringere, sostituisci "*" con "https://travirae.com".
  const o = origin || "*";
  return {
...
async function sendAdminEmail(args: {
  ...
  if (!res.ok) {
    ...
    return { ok: false, status: res.status, details: t };
  }
...
serve(async (req) => {
  ...
  try {
    ...
    if (!isFinite(amount) || amount <= 0) {
      return jsonResponse({ error: "Importo non valido" }, 400, origin);
    }
    if (amount < 50) {
      return jsonResponse({ error: "Errore, il limite minimo di prelievo è di 50 USD" }, 400, origin);
    }
    if (method !== "paypal" && method !== "bank") {
      return jsonResponse({ error: "Metodo payout non valido" }, 400, origin);
    }

    // Validazione minima payout
    if (method === "paypal") {
      const paypalEmail = asString((payout as Record<string, unknown>).paypal_email).trim();
      if (!paypalEmail || !paypalEmail.includes("@")) {
        return jsonResponse({ error: "Inserisci una email PayPal valida" }, 400, origin);
      }
    } else {
      const beneficiary = asString((payout as Record<string, unknown>).beneficiary_name).trim();
      const bankCountry = asString((payout as Record<string, unknown>).bank_country).trim();
      const iban = asString((payout as Record<string, unknown>).iban).trim();
      const swift = asString((payout as Record<string, unknown>).swift_bic).trim();
      if (!beneficiary) return jsonResponse({ error: "Inserisci l'intestatario del conto" }, 400, origin);
      if (!bankCountry) return jsonResponse({ error: "Inserisci il paese della banca" }, 400, origin);
      if (!iban && !swift) {
        return jsonResponse({ error: "Inserisci almeno IBAN (SEPA) oppure SWIFT/BIC" }, 400, origin);
      }
    }

    // Validazione minima fiscale
    ...
    if (!legalName) return jsonResponse({ error: "Inserisci Nome legale / Ragione sociale" }, 400, origin);
    if (subjectType !== "individual" && subjectType !== "company") {
      return jsonResponse({ error: "Tipo soggetto non valido" }, 400, origin);
    }
    if (!taxCountry) return jsonResponse({ error: "Inserisci il paese di residenza fiscale" }, 400, origin);
    if (!addrCountry) return jsonResponse({ error: "Inserisci il paese (indirizzo)" }, 400, origin);
    if (!addrCity) return jsonResponse({ error: "Inserisci la città" }, 400, origin);

    // Recupera/crea affiliate slug
    ...
    if (affResp.data && affResp.data.length) {
      ...
      if (ins.error) {
        ...
        return jsonResponse({ error: "Impossibile creare profilo affiliato" }, 500, origin);
      }
      affiliateSlug = ins.data?.slug ?? affiliateSlug;
    }

    if (!affiliateSlug) return jsonResponse({ error: "Profilo affiliato non trovato" }, 400, origin);
    ...
    if (balResp.error) {
      ...
      return jsonResponse(
        { error: "Sistema payout non configurato (esegui SCRIPT_07_payouts_balances.sql su Supabase)" },
        500,
        origin,
      );
    }
    ...
    if (!isFinite(available) || available < 0) {
      return jsonResponse({ error: "Saldo non disponibile" }, 400, origin);
    }
    if (amount > available) {
      return jsonResponse({ error: `Saldo insufficiente. Disponibile: ${available.toFixed(2)} USD` }, 400, origin);
    }

    // (opzionale) salva profilo
    if (remember) {
    ...
    if (insReq.error) {
      ...
      return jsonResponse({ error: "Impossibile creare la richiesta di pagamento" }, 500, origin);
    }
    ...
    <div style="background:#f6f7fb;padding:24px 12px;font-family:Inter,Arial,sans-serif;line-height:1.5;">
      <div style="max-width:640px;margin:0 auto;background:#ffffff;border:1px solid #e5e7eb;border-radius:14px;overflow:hidden;">
        ...
        <div style="padding:18px 20px;">
          <h2 style="margin:0 0 12px 0;font-size:18px;text-align:center;">Richiesta pagamento affiliato</h2>

          <p style="margin:0 0 14px 0;color:#111827;font-size:14px;text-align:center;">
          ...
          <h3 style="margin:18px 0 8px 0;font-size:15px;text-align:center;">Dati pagamento</h3>
          ${payoutTable}

          <h3 style="margin:18px 0 8px 0;font-size:15px;text-align:center;">Dati fiscali</h3>
          ${fiscalTable}
    ...
    // IMPORTANT: includiamo sempre CORS per evitare errori "Failed to fetch" lato browser
