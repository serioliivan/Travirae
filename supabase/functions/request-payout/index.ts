...
// - RESEND_API_KEY
// - (opzionale) RESEND_FROM (default: payouts@travirae.com)
...
async function sendAdminEmail(args: {
  ...
  const { resendKey, from, to, subject, html, replyTo } = args;
  if (!resendKey) {
...
serve(async (req) => {
  ...
  const RESEND_FROM = Deno.env.get("RESEND_FROM") ?? "payouts@travirae.com";
  ...
  const html = `
    <div style="font-family:Inter,Arial,sans-serif;line-height:1.5;">
      <h2>Richiesta pagamento affiliato</h2>
      <p><strong>Affiliate ID:</strong> ${safeSlug}<br/>
      ...
      <pre style="background:#f8fafc;border:1px solid #e5e7eb;padding:10px;border-radius:10px;white-space:pre-wrap;">${escapeHtml(JSON.stringify(payout, null, 2))}</pre>

      <h3>Dati fiscali</h3>
      <pre style="background:#f8fafc;border:1px solid #e5e7eb;padding:10px;border-radius:10px;white-space:pre-wrap;">${escapeHtml(JSON.stringify(fiscal, null, 2))}</pre>

      <p style="color:#6b7280;font-size:12px;margin-top:16px;">Email generata automaticamente da Travirae.</p>
    </div>
  `;

  const subject = `Travirae - Richiesta pagamento ${affiliateSlug} (${amount.toFixed(2)} USD)`;
