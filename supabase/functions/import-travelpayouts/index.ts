...
async function resolveAffiliateAttribution(admin: any, rawSubId: unknown): Promise<ResolvedAttribution> {
  const normalized = sanitizeSlug(rawSubId);
  if (!normalized) {
    return {
      affiliateSlug: "direct",
      matchType: "direct",
      metadata: {
        raw_partner_sub_id: rawSubId == null ? null : String(rawSubId),
        tracking_match_type: "direct",
      },
    };
  }

  try {
    const legacy = await admin.from("affiliates").select("slug").eq("slug", normalized).maybeSingle();
    if (legacy?.data?.slug) {
      return {
        affiliateSlug: String(legacy.data.slug),
        matchType: "legacy_affiliate",
        metadata: {
          raw_partner_sub_id: normalized,
          tracking_match_type: "legacy_affiliate",
        },
      };
    }
  } catch (_legacyErr) {
    // non-blocking, provo RPC
  }

  try {
    const rpc = await admin.rpc("resolve_influencer_tracking_code", { p_tracking_code: normalized });
    const row = Array.isArray(rpc?.data) ? rpc.data[0] : rpc?.data;
    if (row?.affiliate_slug) {
      return {
        affiliateSlug: String(row.affiliate_slug),
        matchType: String(row.match_type || "influencer_widget"),
        metadata: {
          source: "influencer_post",
          creator_slug: row.creator_slug || row.affiliate_slug || null,
          post_id: row.post_id || null,
          post_slug: row.post_slug || null,
          widget_id: row.widget_id || null,
          widget_short_id: row.widget_short_id || null,
          widget_tracking_code: row.tracking_code || normalized,
          raw_partner_sub_id: normalized,
          tracking_match_type: row.match_type || "influencer_widget",
          resolved_tracking_metadata: row.metadata || {},
        },
      };
    }
  } catch (_rpcErr) {
    // fallback raw code -> affiliate_slug grezzo (backward compatible)
  }

  return {
    affiliateSlug: normalized,
    matchType: "fallback_raw",
    metadata: {
      raw_partner_sub_id: normalized,
      tracking_match_type: "fallback_raw",
    },
  };
}
...
Deno.serve(async (req) => {
  ...
  const campaignId = Number(payload?.campaign_id ?? 100);
  ...
  for (const r of rows) {
    ...
    const resolved = await resolveAffiliateAttribution(admin, r?.sub_id);
    ...
    upserts.push({
      affiliate_slug: resolved.affiliateSlug || "direct",
      partner: "aviasales",
      status,
      booked_at: bookedAt,
