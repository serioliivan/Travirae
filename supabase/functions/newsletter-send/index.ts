...
Deno.serve(async (req: Request) => {
  ...
  try {
    ...
    for (const sub of list) {
      ...
      // -----------------------------
      // Admin can now write real HTML (buttons/images/layout) in the newsletter body.
      // If the provided body *doesn't* look like HTML, we keep the old safe behavior:
      // escape + preserve new lines.
      const looksLikeHtml = (s: string) => /<\s*\/?\s*[a-z][\s\S]*>/i.test(s);
