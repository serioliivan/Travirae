...
async function sendResendEmail(args: {
  ...
  // Read body once (Edge runtime streams cannot be re-read).
  ...
  if (!res.ok) {
    ...
    err.retryAfter = ra ? Number.parseInt(ra, 10) || 0 : 0;
    throw err;
  }
  ...
  for (let attempt = 0; attempt < maxAttempts; attempt++) {
    try {
      ...
      const retryable =
        ...
        typeof status !== "number"; // network/unknown

      const isLast = attempt >= maxAttempts - 1;
      if (!retryable || isLast) throw e;

      // Exponential backoff (+ small jitter). Respect Retry-After when present.
      const base = 600 * Math.pow(2, attempt);
      const waitMs = Math.max(base, retryAfter * 1000) + Math.floor(Math.random() * 200);
      await sleep(waitMs);
    }
  }
  throw lastErr;
...
Deno.serve(async (req: Request) => {
  ...
  try {
    ...
    if (!total) {
      return json(req, { ok: true, total: 0, sent: 0, failed: 0, errors: 0, message: "Nessun iscritto attivo." }, 200);
    }
  ...
  <body style="font-family:Arial,sans-serif;line-height:1.5;margin:0;padding:0">
    ...
    ${footer}
      ...
      try {
        ...
        failedRecipients.push({ email, error: msg });
        console.error("newsletter-send error", { email, error: msg });
      }
    }
