...
- `supabase/sql/SCRIPT_07_payouts_balances.sql`  ✅ (NUOVO: prelievi + viste balance)
- `supabase/sql/SCRIPT_08_site_traffic.sql`  ✅ (NUOVO: visite totali sito)
...
- (consigliato) `RESEND_FROM` = `info@travirae.com`
- (opzionale) `ADMIN_EMAIL` = `serioliivan@gmail.com`
...
2) **Email “operative” del sito** (es. richiesta pagamento affiliato)
   ...
   - si configurano con i Secrets:
     - `RESEND_API_KEY`
     - `RESEND_FROM` (es. `info@travirae.com`)
     - `ADMIN_EMAIL`
