...
grant execute on function public.resolve_influencer_tracking_code(text) to anon, authenticated, service_role;
...
grant execute on function public.review_influencer_profile(uuid, text, text) to authenticated, service_role;
...
grant execute on function public.review_influencer_post(uuid, text, text) to authenticated, service_role;
...
grant execute on function public.get_influencer_public_profile(text) to anon, authenticated, service_role;
...
grant execute on function public.get_influencer_public_posts(integer, integer, text) to anon, authenticated, service_role;
...
grant execute on function public.get_influencer_public_post(text) to anon, authenticated, service_role;
...
with event_rows as (
  select
    coalesce(nullif(affiliate_slug, ''), nullif(metadata->>'ref_affiliate_slug', '')) as affiliate_slug,
    event_type
...
creator_ids as (
  ...
  union
  select affiliate_slug from event_rows where affiliate_slug is not null
  union
...
-- ============================================================
-- 7) V9 CONSOLIDATION — WIDGET OUTBOUND + ADMIN OVERVIEW
-- ============================================================
-- This makes a fresh 01→12 deploy consistent with the current frontend:
-- booking-click metrics are deprecated and the admin overview uses widget_click events.
...
grant execute on function public.get_affiliate_widget_outbound_totals(text) to authenticated, service_role;

drop view if exists public.admin_affiliate_overview;

create view public.admin_affiliate_overview with (security_invoker = true) as
with clicks as (
