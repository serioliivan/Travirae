...
create or replace view public.monthly_affiliate_stats with (security_invoker = true) as
with base as (
  select
    b.affiliate_slug,
...
select
  ...
  sales_count,
  net_commissions,
  case when affiliate_slug = 'direct' then 0::numeric else public.travirae_share_percent(sales_count) end as share_percent,
  (net_commissions * (case when affiliate_slug = 'direct' then 0::numeric else public.travirae_share_percent(sales_count) end))::numeric as affiliate_earnings
from base;

-- GRANT esplicito sulla view usata da dashboard e funzioni.
...
grant execute on function public.refresh_monthly_affiliate_payouts() to authenticated, service_role;
