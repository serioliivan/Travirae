-- SCRIPT 14 — Widget outbound totals for affiliate/admin dashboards
--
-- Run this script in Supabase SQL Editor.
-- It adds a SECURITY DEFINER RPC for affiliate dashboards and updates the
-- admin_affiliate_overview view so admin can see outbound widget clicks
-- attributed to each affiliate across the whole site (including creator posts).
--
-- IMPORTANT HOTFIX:
-- If your database still has the old admin_affiliate_overview with legacy
-- booking_clicks_* columns, PostgreSQL will reject CREATE OR REPLACE VIEW
-- when the script tries to rename columns. For that reason this version uses
...
widget_clicks as (
  select
    ...
    count(*)::bigint as widget_clicks_total,
    count(*) filter (where coalesce(e.metadata->>'source', '') = 'site_widget')::bigint as widget_clicks_site_total,
    count(*) filter (where coalesce(e.metadata->>'source', '') = 'influencer_post')::bigint as widget_clicks_creator_total,
