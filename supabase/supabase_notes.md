...
per far funzionare l'area affiliati, il tracking dei referral e le statistiche di payout.
...
- `select('id', { count: 'exact' }).eq('affiliate_slug', slug)` per calcolare i click totali.
...
- `select('id', { count: 'exact' }).eq('affiliate_slug', slug)` per mostrare i **booking totali** in dashboard.

> Come vengono popolati i booking dipende dall'integrazione server-side con i partner (webhook, import, ecc.).  
> Il frontend li legge soltanto.
...
### 5. Autenticazione (`auth.users`)

L'area affiliati usa:

- `auth.signUp` – registrazione (`email`, `password`).
- `auth.signInWithPassword` – login.
- `auth.resetPasswordForEmail` – recupero password (link con `mode=recovery`).
- `auth.updateUser` – aggiornamento password una volta aperto il link di recovery.
- `auth.getUser` – recupero sessione esistente.
- `auth.signOut` – logout.
...
<!-- script inline affiliate portal -->
```
