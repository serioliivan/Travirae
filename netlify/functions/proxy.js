...
// Netlify Function: /api/* for Travirae
...
function json(body,status=200){ return { statusCode:status, headers:{ 'Content-Type':'application/json; charset=utf-8', 'Access-Control-Allow-Origin':'*', 'Access-Control-Allow-Methods':'GET,OPTIONS' }, body: JSON.stringify(body) }; }
...
exports.handler = async (event, context) => {
  try{
    ...
    const LOCALE = (qs.get('locale') || 'it').slice(0,2);
    ...
    if(path.endsWith('/api/flights/search')){
      const origin = (qs.get('origin')||'').toUpperCase().slice(0,3);
      const destination = (qs.get('destination')||'').toUpperCase().slice(0,3);
      const depart = qs.get('depart') || '';
      const ret = qs.get('return') || '';
      const currency = (qs.get('currency')||'EUR').toUpperCase();
      ...
      u.searchParams.set('page','1'); u.searchParams.set('limit', String(limit));
      u.searchParams.set('sorting','price'); u.searchParams.set('trip_class','0');
      u.searchParams.set('token', TP_API_KEY);
      let r = await follow(u.toString(), headers);
      let data=[]; try{ data = (JSON.parse(r.text)||{}).data || []; }catch{ data=[]; }

      // Fallback month matrix
      if(!data || data.length===0){
        const d=new Date(depart); const month = isNaN(+d) ? '' : `${d.getUTCFullYear()}-${String(d.getUTCMonth()+1).padStart(2,'0')}-01`;
        const u2 = new URL(base+'/prices_for_month');
        u2.searchParams.set('origin', origin); u2.searchParams.set('destination', destination);
        u2.searchParams.set('month', month); u2.searchParams.set('currency', currency);
        u2.searchParams.set('token', TP_API_KEY);
        const r2 = await follow(u2.toString(), headers);
        try{ data = (JSON.parse(r2.text)||{}).data || []; }catch{ data=[]; }
      }

      // Normalize + affiliate deeplink (Aviasales with marker)
      const out = (data||[]).slice(0,limit).map(it=>{
        ...
        avia.searchParams.set('depart_date', dep.split('T')[0]);
        if(retD) avia.searchParams.set('return_date', String(retD).split('T')[0]);
        avia.searchParams.set('adults','1'); avia.searchParams.set('with_request','true');
        avia.searchParams.set('currency', currency);
        return {
          price: it.value, origin: it.origin, destination: it.destination,
          depart_date: dep ? String(dep).split('T')[0] : '', return_date: retD ? String(retD).split('T')[0] : null,
          changes: it.number_of_changes || it.transfers || 0, deeplink: avia.toString()
        };
      });
      return json({ ok:true, currency, results: out });
    }

    // Any other path
    return json({ ok:false, error:'unknown_api' }, 404);
