...
module.exports = async (req, res) => {
  const K=process.env.TP_API_KEY||''; const M=process.env.TP_PARTNER_MARKER||''; const REF=process.env.TP_REFERER||'https://travirae.com/';
  const url = new URL('http://x'+req.url); const path=url.pathname; const q=url.searchParams;
  const J=(o,s=200)=>{ res.statusCode=s; res.setHeader('Content-Type','application/json; charset=utf-8'); res.setHeader('Access-Control-Allow-Origin','*'); res.end(JSON.stringify(o)); };
  try{
    if(path.endsWith('/api/ping')) return J({ok:true});
    if(path.endsWith('/api/airports/suggest')){
      ...
      const r=await follow(U.toString(), {'Referer':REF,'User-Agent':'travirae/1.0'}); let arr=[]; try{arr=JSON.parse(r.text)}catch{}; const items=(Array.isArray(arr)?arr:[]).slice(0,10).map(x=>({name:x.name,country:x.country_name||x.country_code,iata:x.code||x.iata,type:x.type})).filter(x=>x.iata); return J({ok:true,items});
    }
