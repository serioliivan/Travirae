...
(function(){
  ...
  function ensureFields({ originIata, destIata, depart }){
    if (!originIata || !destIata){
      alert('Seleziona origine e destinazione dall'autocomplete o inserisci codici IATA (3 lettere).');
      return false;
    }
    if (!depart){ alert('Seleziona la data di partenza.'); return false; }
    return true;
  }
  ...
  function updateStay22Embed(address, checkin, checkout){
    ...
    openStay22Btn.onclick = ()=> {
      window.openStay22({ address, checkin, checkout: checkout || checkin });
    };
  }
